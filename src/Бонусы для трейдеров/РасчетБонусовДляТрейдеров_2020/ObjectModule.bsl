...
Функция ПолучитьРезультатДляВывода()
	...
	|	1041.67 * (1 + РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(&ДатаОтчета, ГОД), &ДатаОтчета, МЕСЯЦ)) КАК БонусПоПлану
	...
	|ВЫБРАТЬ
	|	&Чернигов,
	|	558.32 * (1 + РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(&ДатаОтчета, ГОД), &ДатаОтчета, МЕСЯЦ))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&Николаев,
	|	166.67 * (1 + РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(&ДатаОтчета, ГОД), &ДатаОтчета, МЕСЯЦ))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&Одеса,
	|	1366.68 * (1 + РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(&ДатаОтчета, ГОД), &ДатаОтчета, МЕСЯЦ))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	&Харьков,
	|	1141.66 * (1 + РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(&ДатаОтчета, ГОД), &ДатаОтчета, МЕСЯЦ))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
