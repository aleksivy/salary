Перем ПодвалНапечатан;
Перем ОбластьПодвала;
Перем мСколькоСтрокДобавили;

Процедура ПроверитьПеренос(ТабДок, Область)
	ПроверкаПечатиПодвала = Новый Массив;
	ПроверкаПечатиПодвала.Добавить(Область);
	ПроверкаПечатиПодвала.Добавить(ОбластьПодвала);
	
	Если ПодвалНапечатан или ТабДок.ПроверитьВывод(ПроверкаПечатиПодвала)тогда 
		мСколькоСтрокДобавили = мСколькоСтрокДобавили + 1;
		ТабДок.Вывести(Область);
	Иначе
		ТабДок.Вывести(ОбластьПодвала);
		ПодвалНапечатан = Истина;
	КонецЕсли;
КонецПроцедуры
...
Функция Печать() Экспорт
	...
	// получаем макет
	Если лЭтотОбъект.Дата < '20210505' Тогда
		Макет = ПолучитьМакет("Макет");
	Иначе
		Макет = ПолучитьМакет("МакетВайтерра");
	КонецЕсли;
	...
	мСколькоСтрокДобавили = 0;
	ОбластьПодвала = Макет.ПолучитьОбласть("КолонтитулВитСнизу");
    Пока Не ПодвалНапечатан Цикл
		Область = Макет.ПолучитьОбласть("ПустаяСтрока");
		ПроверитьПеренос(ТабДокумент, Область);
	КонецЦикла;
