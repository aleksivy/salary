...
Перем мРасчетСреднегоДляПособияРезультат; 
Перем мРасчетСреднегоДляПособияРезультатПолный;
Перем мРасчетСреднегоДляПособияСуммаПредела;
Перем мРасчетСреднегоДляПособияОтработаноДней;
Перем мРасчетСреднегоДляПособияОтработаноЧасов;
Перем мРасчетСреднегоДляПособияКалендарныеДни;
...
Процедура ОбновитьВидимостьЭлементовФормы()
	...
	Если мЕстьГодоваяПремия Тогда
		...
		ЭлементыФормы.РамкаЕстьГодоваяПремия1.Видимость= Истина;
		ЭлементыФормы.НадписьГод1.Видимость= Истина;
		ЭлементыФормы.ГодоваяПремияБазовыйПериодНачало1.Видимость= Истина;
		ЭлементыФормы.ГодоваяПремия1.Видимость= Истина;
		ЭлементыФормы.РассчитатьПремияЗаГод1.Видимость= Истина;
	...
	Иначе
		...
		ЭлементыФормы.РамкаЕстьГодоваяПремия1.Видимость= Ложь;
		ЭлементыФормы.НадписьГод1.Видимость= Ложь;
		ЭлементыФормы.ГодоваяПремияБазовыйПериодНачало1.Видимость= Ложь;
		ЭлементыФормы.ГодоваяПремия1.Видимость= Ложь;
		ЭлементыФормы.РассчитатьПремияЗаГод1.Видимость= Ложь;
	...
	Если ПризнакКомпенсацииОтпуска Тогда
		ЭлементыФормы.НадписьПервичныйОтпуск.Видимость = Ложь;
		ЭлементыФормы.ПервичныйОтпуск.Видимость = Ложь;
		
		ЭлементыФормы.КоманднаяПанельРасчетСреднего.Видимость = Истина;
		ЭлементыФормы.РасчетСреднего.Видимость = Истина;
	Иначе	
		ЭлементыФормы.НадписьПервичныйОтпуск.Видимость = Истина;
		ЭлементыФормы.ПервичныйОтпуск.Видимость = Истина;
		
		ЭлементыФормы.КоманднаяПанельРасчетСреднего.Видимость = Ложь;
		ЭлементыФормы.РасчетСреднего.Видимость = Ложь;
	КонецЕсли;
...
Перем строкаБолееПозднихДокументов;
		
	Если ПроверитьНачисления(строкаБолееПозднихДокументов) Тогда
		Сообщить(НСтр("ru='Нельзя производить расчеты, пока проведены документы: ';uk='Не можна проводити розрахунки, поки проведені документи: '") + строкаБолееПозднихДокументов, СтатусСообщения.Внимание);
		Возврат Ложь;
	КонецЕсли;
...
Процедура УстановитьКнопкиПодменюЗаполненияТЧ()
	...
	СоответствиеТЧ.Вставить(ЭлементыФормы.РасчетСреднегоДляПособия,ЭлементыФормы.КоманднаяПанельРасчетСреднего1);
...
Процедура ОбновитьОписаниеСостоянияДокумента()
	...
	НеЗаполнен = НеЗаполнен Или (ЧислоМесяцевДляПособия = 0 И ДнейКомпенсацииУдержанияОтпуска = 0);
...
Процедура ПриОткрытии()
	...
	Если ЭтоНовый() Тогда
		...
		Если ДнейКомпенсацииУдержанияОтпуска = 0 Тогда
			ПризнакКомпенсацииОтпуска = Истина;
		КонецЕсли;	
		
		Если НЕ ЗначениеЗаполнено(КоэффициентПовышенияОкладов) Тогда
...
Процедура ДействияФормыРассчитатьВсе(Кнопка)
	...
	Если РасчетСреднего.Количество()>0 
		...
		ИЛИ РасчетСреднегоДляПособия.Количество()>0
		Тогда
		...
		РасчетСреднегоДляПособия.Очистить(); 
		Записать();
	...
	РасчитатьПараметрыСреднейДляПособия();
	Записать();
...
Процедура РасчитатьПараметрыСреднейДляПособия()
	Если НЕ ЗначениеЗаполнено(ДатаУвольнения) Тогда
		ЧислоМесяцевДляПособия = 0;
		ПериодРасчетаСреднегоДляПособияНачало = Дата(1,1,1);
		ПериодРасчетаСреднегоДляПособияКонец = Дата(1,1,1);
		ВидУчетаВремениДляПособия = Перечисления.ВидыУчетаВремениДляСредней.ПустаяСсылка();
		ГодоваяПремияБазовыйПериодНачало = Дата(1,1,1);
		ГодоваяПремия = 0;	
		
		Возврат;
	КонецЕсли;            	
	
	ЧислоМесяцевДляПособия = 2;
	
	Выборка = ПолучитьПериодПоРаботникуДляПособия().Выбрать();
	Выборка.Следующий();
	
	ПериодРасчетаСреднегоДляПособияНачало	= Выборка.ПериодРасчетаСреднегоНачало;
	ПериодРасчетаСреднегоДляПособияКонец	= Выборка.ПериодРасчетаСреднегоКонец;
...
Процедура РассчитатьСреднююДляПособияНажатие(Элемент)
...
//		Должность
//
Функция ПодборДанныхДокументОснованиеПоСотруднику(Форма, ЭлементФормы, Сотрудник, Организация, ОбязательныйПодбор = Ложь) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               |	РаботникиОрганизации.Ссылка КАК Увольнение,
	               |	РаботникиОрганизации.ДатаУвольнения КАК ДатаУвольнения,
	               |	РаботникиОрганизации.КомпенсироватьДнейЕжегодногоОтпуска КАК ДнейОтпуска
	               |ИЗ
	               |	Документ.УвольнениеИзОрганизаций.РаботникиОрганизации КАК РаботникиОрганизации
	               |ГДЕ
	               |	РаботникиОрганизации.Сотрудник В(&Сотрудник)
	               |	И РаботникиОрганизации.Ссылка.Организация В(&СписокСтруктурныхПодразделений)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ДатаУвольнения";
	...
	Если НаборыДанных.Количество() = 0 Тогда
		Если ОбязательныйПодбор Тогда
			...
			СписокНаборовДанных.Добавить(Неопределено, НСтр("ru='Нет приказов об увольнении';uk='Немає наказів про звільнення'"));
			Форма.ВыбратьИзСписка(СписокНаборовДанных, ЭлементФормы);
		КонецЕсли;
		Возврат НеОпределено;
	// Если нашли один набор данных то используем его
	ИначеЕсли НаборыДанных.Количество() = 1 И НЕ ОбязательныйПодбор Тогда
		Возврат НаборыДанных[0];
	// Если набоов данных несколько - предложим выбрать из списка.
	Иначе
		СписокНаборовДанных = Новый СписокЗначений;
		Для Каждого Строка Из НаборыДанных Цикл
			СписокНаборовДанных.Добавить(Строка, "" + Строка.Увольнение + " (" + Строка.ДатаУвольнения + ")");
		КонецЦикла;
...
КонецФункции  // ПодборДанныхДокументОснованиеПоФизлицу

Процедура ДокументОснованиеНачалоВыбора(Элемент, СтандартнаяОбработка)
	
    СтандартнаяОбработка = Ложь;
	НаборДанных = ПодборДанныхДокументОснованиеПоСотруднику(ЭтаФорма, Элемент, Сотрудник, Организация, Истина);
	Если НаборДанных = НеОпределено Тогда
		Возврат;
	КонецЕсли;

	ДокументОснование = НаборДанных.Увольнение;
	ДатаУвольнения = НаборДанных.ДатаУвольнения;
	ДнейКомпенсацииУдержанияОтпуска = НаборДанных.ДнейОтпуска;
...
Процедура НачисленияПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если ДанныеСтроки.Сторно Тогда
		...
		ОформлениеСтроки.Ячейки.ВидРасчета.ТолькоПросмотр = Истина;
...
Процедура РасчетСреднегоПередНачаломДобавления(Элемент, Отказ, Копирование)
	...
	Если НЕ ЗначениеЗаполнено(ЭлементыФормы.Сотрудник) Тогда
		Предупреждение(НСтр("ru='Перед началом заполнения табличной части необходимо выбрать работника';uk='Перед початком заповнення табличної частини необхідно вибрати працівника'"));
		Отказ = Истина;
	КонецЕсли; 
...
Процедура РасчетСреднегоДляПособияПриАктивизацииЯчейки(Элемент)
	
	ТекущаяСтрока = ЭлементыФормы.РасчетСреднегоДляПособия.ТекущаяСтрока;
	Если ТекущаяСтрока <> Неопределено Тогда
		мРасчетСреднегоДляПособияРезультат			= ТекущаяСтрока.Результат;
		мРасчетСреднегоДляПособияРезультатПолный	= ТекущаяСтрока.РезультатПолный;
		мРасчетСреднегоДляПособияСуммаПредела		= ТекущаяСтрока.СуммаПредела;
		мРасчетСреднегоДляПособияОтработаноДней		= ТекущаяСтрока.ОтработаноДней;
		мРасчетСреднегоДляПособияОтработаноЧасов	= ТекущаяСтрока.ОтработаноЧасов;
		мРасчетСреднегоДляПособияКалендарныеДни		= ТекущаяСтрока.КалендарныеДни;
	КонецЕсли;
	
КонецПроцедуры

Процедура РасчетСреднегоДляПособияПередНачаломДобавления(Элемент, Отказ, Копирование)
...
Процедура РасчетСреднегоДляПособияПриНачалеРедактирования(Элемент, НоваяСтрока)
	Если НоваяСтрока Тогда
		Элемент.ТекущаяСтрока.КоэффициентПовышенияОкладов = 1;
	КонецЕсли; 
КонецПроцедуры

Процедура РасчетСреднегоДляПособияПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
...
Процедура РасчетСреднегоДляПособияРезультатПриИзменении(Элемент)
		
	Если Элемент.Значение <> мРасчетСреднегоДляПособияРезультат Тогда
		ЭлементыФормы.РасчетСреднегоДляПособия.ТекущаяСтрока.Авторасчет = Ложь;
		мРасчетСреднегоДляПособияРезультат = Элемент.Значение;
	КонецЕсли;
	
КонецПроцедуры

Процедура РасчетСреднегоДляПособияРезультатПолныйПриИзменении(Элемент)
		
	Если Элемент.Значение <> мРасчетСреднегоДляПособияРезультатПолный Тогда
		ЭлементыФормы.РасчетСреднегоДляПособия.ТекущаяСтрока.Авторасчет = Ложь;
		мРасчетСреднегоДляПособияРезультатПолный = Элемент.Значение;
	КонецЕсли;
	
КонецПроцедуры

Процедура РасчетСреднегоДляПособияОтработаноДнейПриИзменении(Элемент)
		
	Если Элемент.Значение <> мРасчетСреднегоДляПособияОтработаноДней Тогда
		ЭлементыФормы.РасчетСреднегоДляПособия.ТекущаяСтрока.Авторасчет = Ложь;
		мРасчетСреднегоДляПособияОтработаноДней = Элемент.Значение;
	КонецЕсли;
	
КонецПроцедуры

Процедура РасчетСреднегоДляПособияОтработаноЧасовПриИзменении(Элемент)
	
	Если Элемент.Значение <> мРасчетСреднегоДляПособияОтработаноЧасов Тогда
		ЭлементыФормы.РасчетСреднегоДляПособия.ТекущаяСтрока.Авторасчет = Ложь;
		мРасчетСреднегоДляПособияОтработаноЧасов = Элемент.Значение;
	КонецЕсли;
	
КонецПроцедуры

Процедура РасчетСреднегоДляПособияКалендарныеДниПриИзменении(Элемент)
		
	Если Элемент.Значение <> мРасчетСреднегоДляПособияКалендарныеДни Тогда
		ЭлементыФормы.РасчетСреднегоДляПособия.ТекущаяСтрока.Авторасчет = Ложь;
		мРасчетСреднегоДляПособияКалендарныеДни = Элемент.Значение;
	КонецЕсли;
...
Процедура КоманднаяПанельРасчетСреднегоДляПособияЗаполнить(Кнопка)
	Если Не ЗначениеЗаполнено(Сотрудник) Тогда
		Предупреждение(НСтр("ru='Перед началом заполнения табличной части необходимо выбрать работника';uk='Перед початком заповнення табличної частини необхідно вибрати працівника'"));
		Возврат;
	КонецЕсли;
	Если РасчетСреднегоДляПособия.Количество()>0 Тогда
...
Процедура КоманднаяПанельРасчетСреднегоДляПособияРассчитать(Кнопка)
	...
	ОбработкаКомментариев = глЗначениеПеременной("глОбработкаСообщений");
	ОбработкаКомментариев.УдалитьСообщения();
	РассчитатьРасчетСреднегоДляПособия( КомментироватьРасчет );
	ОбработкаКомментариев.ПоказатьСообщения();
...
Процедура КоманднаяПанельРасчетСреднегоДляПособияРассчитатьВсе(Кнопка)
...
Процедура ПервичныйОтпускНачалоВыбора(Элемент, СтандартнаяОбработка)

	СтандартнаяОбработка=Ложь;
	мВедомость = Элемент.Значение;

	ФормаВыбора = Документы["НачислениеОтпускаРаботникамОрганизаций"].ПолучитьФормуВыбора(,Элемент,);
	...
	Запрос.Текст="
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Основной.Номер КАК НомерДок
	|ИЗ
	|	Документ.НачислениеОтпускаРаботникамОрганизаций КАК Основной
	|
	|ГДЕ	Основной.Организация = &Организация 		
	|	И	Основной.Сотрудник = &Сотрудник
	|";
	
	СписокОтбора=Новый СписокЗначений;
	СписокОтбора.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("НомерДок"));
...
Процедура ПервичныйОтпускПриИзменении(Элемент)
	СуммаСредней = Элемент.Значение.СуммаСредней;
	ВидУчетаВремениДляСредней = Элемент.Значение.ВидУчетаВремениДляСредней;
	ПериодРасчетаСреднегоНачало = Элемент.Значение.ПериодРасчетаСреднегоНачало;
	ПериодРасчетаСреднегоКонец = Элемент.Значение.ПериодРасчетаСреднегоКонец;
	ЧислоМесяцев = Элемент.Значение.ЧислоМесяцев;
	ГодоваяПремияБазовыйПериодНачало = Элемент.Значение.ГодоваяПремияБазовыйПериодНачало;
	ГодоваяПремия = Элемент.Значение.ГодоваяПремия;
КонецПроцедуры
