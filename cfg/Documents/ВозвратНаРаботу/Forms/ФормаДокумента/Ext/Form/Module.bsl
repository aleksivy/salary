...
// Процедура вызывается при выборе пункта подменю "ВозвратНаРаботуОрганизаций" меню "ВводНаОсновании"
//
Процедура ДействияФормыВозвратНаРаботуОрганизацийВводНаОсновании(Кнопка)
...
Процедура КоманднаяПанельРаботникиОрганизацииЗаполнить(Кнопка)
	...
	ПроцедурыУправленияПерсоналом.ОткрытьФормуОтбораСпискаРаботников(ЭтаФорма, Дата, "Работники", , Подразделение);
...
Процедура РаботникиДатаВозвратаПриИзменении(Элемент)
	Если Элемент.Значение > РабочаяДата Тогда
		Ответ = Вопрос(НСтр("ru='Вы действительно хотите вернуть работника на будущую дату?';uk='Ви дійсно хочете повернути працівника на майбутню дату?'"), РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Нет);
