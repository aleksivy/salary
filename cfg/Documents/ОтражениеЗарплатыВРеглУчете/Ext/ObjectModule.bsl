...
// Серна - Начало
		ИначеЕсли (Выборка.СчетДт.Код = "471") и (Выборка.СчетКт.Код = "661") Тогда
			СтрокаДанных2 = ОтражениеВУчете.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаДанных2, Выборка);
			СтрокаДанных2.СчетДт = ПланыСчетов.Хозрасчетный.АдминистративныеРасходы;
			СтрокаДанных2.СубконтоДт1 = СтрокаДанных.СубконтоДт2;
			СтрокаДанных2.СубконтоДт2 = СтрокаДанных.СубконтоДт1;
			СтрокаДанных2.СубконтоДт3 = Справочники.НоменклатурныеГруппы.НайтиПоНаименованию("Для распределения по культурам");
			СтрокаДанных2.СчетКт = ПланыСчетов.Хозрасчетный.ОбеспечениеВыплатОтпусков;
			СтрокаДанных2.СубконтоКт1 = СтрокаДанных.СубконтоДт1;
			СтрокаДанных2.СубконтоКт2 = Неопределено;
			СтрокаДанных2.СубконтоКт3 = Неопределено;
...
// Серна - Начало
		|ВЫБОР
		|	КОГДА Основной.ПодразделениеОрганизации.Родитель.Родитель <> ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
		|		ТОГДА Основной.ПодразделениеОрганизации.Родитель.Родитель
		|	КОГДА Основной.ПодразделениеОрганизации.Родитель <> ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
		|		ТОГДА Основной.ПодразделениеОрганизации.Родитель
		|			ИНАЧЕ Основной.ПодразделениеОрганизации
...
//		|			Основной.ДоходНДФЛ.СпособОтраженияВРеглУчете.СубконтоКт1	КАК СубконтоКт1,
		...
		|				КОГДА Основной.Сотрудник.ТекущееПодразделениеОрганизации.Родитель.Родитель <> ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
		|					ТОГДА Основной.Сотрудник.ТекущееПодразделениеОрганизации.Родитель.Родитель
...
// Серна - Начало
	// Очистим данные по сотрудникам по счету 661 в проводках
	Для каждого лСтрока Из ОтражениеВУчете Цикл
		Если лСтрока.СчетДт = ПланыСчетов.Хозрасчетный.РасчетыПоЗаработнойПлате Тогда
			Если Не лСтрока.СубконтоДт1 = Справочники.ФизическиеЛица.ПустаяСсылка() Тогда лСтрока.СубконтоДт1 = Справочники.ФизическиеЛица.ПустаяСсылка(); КонецЕсли;
		КонецЕсли;
		Если лСтрока.СчетКт = ПланыСчетов.Хозрасчетный.РасчетыПоЗаработнойПлате Тогда
			Если Не лСтрока.СубконтоКт1 = Справочники.ФизическиеЛица.ПустаяСсылка() Тогда лСтрока.СубконтоКт1 = Справочники.ФизическиеЛица.ПустаяСсылка(); КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	лтзПроводки = ОтражениеВУчете.Выгрузить();
	лтзПроводки.Свернуть("СчетДт,СубконтоДт1,СубконтоДт2,СубконтоДт3,СчетКт,СубконтоКт1,СубконтоКт2,СубконтоКт3,СтатьяВаловыхРасходов,НалоговоеНазначение,НалоговоеНазначениеНДС,НеОтражатьВБУ", "Сумма");
	ОтражениеВУчете.Загрузить( лтзПроводки );
// Серна - Конец
КонецПроцедуры
