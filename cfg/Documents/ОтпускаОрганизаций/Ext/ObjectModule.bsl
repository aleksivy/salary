...
// Серна - Начало
	// Движения по регистру СернаДниОтпуска
	Если (ВидОперации = Перечисления.ВидыОперацийОтпускаОрганизаций.Отпуск) и (РаботникиОрганизации.Итог("КвоДней") > 0) Тогда
		Для каждого лСтрока Из РаботникиОрганизации Цикл
			Если Дата < '2016-09-01' Тогда
				Если Не (лСтрока.ВидОтпуска = Справочники.ВидыОтпусков.ЕжегодныйОсновной) или (лСтрока.КвоДней = 0) Тогда Продолжить; КонецЕсли;
			Иначе
				Если Не лСтрока.ВидОтпуска.ВедетсяУчетОстатков или (лСтрока.КвоДней = 0) Тогда Продолжить; КонецЕсли;
				Если лСтрока.ВидОтпуска.ОтпускНеКомпенсируется или лСтрока.ВидОтпуска.ОтпускНеПереноситсяНаСледующийГод Тогда Продолжить; КонецЕсли;
			КонецЕсли;
			лНовоеДвижение = Движения.СернаДниОтпуска.Добавить();
			лНовоеДвижение.Активность = Истина;
			лНовоеДвижение.ВидДвижения = ВидДвиженияНакопления.Расход;
			лНовоеДвижение.Организация = Организация;
			лНовоеДвижение.Период = Дата;
			лНовоеДвижение.Регистратор = Ссылка;
			лНовоеДвижение.КвоДней = лСтрока.КвоДней;
			лНовоеДвижение.Сотрудник = лСтрока.Сотрудник;
			лНовоеДвижение.ВидОтпуска = лСтрока.ВидОтпуска;
		КонецЦикла;
	КонецЕсли;
// Серна - Конец
