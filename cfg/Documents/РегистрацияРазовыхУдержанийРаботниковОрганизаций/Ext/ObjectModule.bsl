...
// Серна - Начало
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.РегистрацияРазовыхНачисленийРаботниковОрганизаций") Тогда
		// Заполнение шапки
		...
		лВидРасчетаМедСтраховка = ПланыВидовРасчета.УдержанияОрганизаций.НайтиПоНаименованию("01. Медицинская страховка");
		Для Каждого ТекСтрокаОсновныеНачисления Из Основание.ОсновныеНачисления Цикл
			Если Не ТекСтрокаОсновныеНачисления.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.МедицинскаяСтраховка Тогда Продолжить; КонецЕсли;
			НоваяСтрока = Удержания.Добавить();
			НоваяСтрока.ВидРасчета = лВидРасчетаМедСтраховка;
			НоваяСтрока.Авторасчет = Истина;
			НоваяСтрока.ДатаНачала = ТекСтрокаОсновныеНачисления.ДатаНачала;
			НоваяСтрока.ДатаОкончания = ТекСтрокаОсновныеНачисления.ДатаОкончания;
			НоваяСтрока.Результат = ТекСтрокаОсновныеНачисления.Результат;
			НоваяСтрока.Показатель1 = ТекСтрокаОсновныеНачисления.Результат;
			НоваяСтрока.Сотрудник = ТекСтрокаОсновныеНачисления.Сотрудник;
			НоваяСтрока.Физлицо = ТекСтрокаОсновныеНачисления.Физлицо;
		КонецЦикла;
// Серна - Конец
	КонецЕсли
