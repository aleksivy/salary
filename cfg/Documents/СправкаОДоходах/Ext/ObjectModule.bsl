...
// Серна - Начало
	...
	|		СУММА(Начисления.НачисленоПрочее) КАК НачисленоПрочее
	|	ИЗ
	|		(ВЫБРАТЬ
	|			СУММА(ОсновныеНачисленияРаботниковОрганизаций.Результат) КАК Результат,
	|			СУММА(ВЫБОР
	|					КОГДА ОсновныеНачисленияРаботниковОрганизаций.ВидРасчета.ЯвляетсяДоходомВНатуральнойФорме
	...
	|			ОсновныеНачисленияРаботниковОрганизаций.ВидРасчета.ЯвляетсяДоходомВНатуральнойФорме) КАК Начисления
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|			И НЕ ВзносыВФонды.Налог В ИЕРАРХИИ (ЗНАЧЕНИЕ(Справочник.Налоги.ВзносыФОТ))
	|		
	|		СГРУППИРОВАТЬ ПО
...
// Серна - Начало
		Строка.СуммаКВыплате	= Выборка.СуммаКВыплате - Выборка.НачисленоПрочее;
		Строка.НачисленоПрочее	= Выборка.НачисленоПрочее;
// Серна - Конец
	КонецЦикла;
...
// Серна - Начало
	...
	ФиоСотрудника= РезультатФИО.Фамилия +" "+РезультатФИО.Имя+" "+РезультатФИО.Отчество;
	Попытка		Объект = Новый COMОбъект("Padeg_ua.Declension");
	Исключение	Сообщить("Не загружена библиотека Padeg_ua.dll");
	Конецпопытки;
// Серна - Конец
...
//	ОбластьМакетаШапка.Параметры.Физлицо = РезультатФИО.Фамилия +" "+РезультатФИО.Имя+" "+РезультатФИО.Отчество;
	ОбластьМакетаШапка.Параметры.Физлицо = Объект.GetFIOPadegFS(ФиоСотрудника, "", 3);
// Серна - Конец
...
// Серна - Начало
	Попытка 	Объект = Новый COMОбъект("Padeg_ua.Declension");
	Исключение	Сообщить("Не загружена библиотека Padeg_ua.dll"); Возврат ТабДокумент;
	Конецпопытки;
//	ОбластьМакетаШапка.Параметры.Физлицо = РезультатФИО.Фамилия +" "+РезультатФИО.Имя+" "+РезультатФИО.Отчество;
	ФиоСотрудника = РезультатФИО.Фамилия + " " + РезультатФИО.Имя + " " + РезультатФИО.Отчество;
	ОбластьМакетаШапка.Параметры.Физлицо = Объект.GetFIOPadegFS(ФиоСотрудника, "", 3);
// Серна - Конец
	...
	КонецЕсли;
// Серна - Начало
	ЗапросУвольнения = Новый Запрос;
	ЗапросУвольнения.УстановитьПараметр("Сотрудник", Сотрудник);
	ЗапросУвольнения.Текст =
	"ВЫБРАТЬ
	...
	|";
	РезультатУвольнения = ЗапросУвольнения.Выполнить().Выбрать();
	Пока РезультатУвольнения.Следующий() Цикл
		Статья = РезультатУвольнения.СтатьяКЗОТ;
	КонецЦикла;	
	Если Строка(Статья)="п. 1 ст. 40 КЗоТ Сокращение " Тогда
		ОбластьМакета.Параметры.Закон="на підставі Кодексу Законів про Працю України (КЗпПУ) п.1 ст.40 у зв`язку із скороченням";
		ОбластьМакета.Параметры.Периодвыплат="одноразово";
	КонецЕсли;
// Серна - Конец
...
Функция ПечатьДоходыИНалоги()
	...
	//ОбластьМакетаШапка.Параметры.ДатаКон					= Формат(?(НЕ ЗначениеЗаполнено(Сотрудник.ДатаУвольнения),РезультатПериодДоходов.ОкончаниеПериодаДоходов,МИН(Сотрудник.ДатаУвольнения,РезультатПериодДоходов.ОкончаниеПериодаДоходов)), "ДЛФ=Д");
// Серна - Начало
	ОбластьМакетаШапка.Параметры.ДатаНач					= Формат(ДатаНач, "ДЛФ=Д");
	ОбластьМакетаШапка.Параметры.ДатаКон					= Формат(ДатаКон, "ДЛФ=Д");
// Серна - Конец
...
// Серна - Начало
	Если НЕ(Сотрудник.ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.Подряда И (ВидСправки = Перечисления.ВидыСправокОДоходах.ДоходыИНалоги ИЛИ ВидСправки = Перечисления.ВидыСправокОДоходах.Произвольная ИЛИ ВидСправки = Перечисления.ВидыСправокОДоходах.СернаБанк))Тогда
// Серна - Конец
		ТекстЗапроса =
		"ВЫБРАТЬ ПЕРВЫЕ 1
...
// Серна - Конец
	...
	|ГДЕ
// Серна - Начало
	|	СправкаОДоходах.ВидСправки = &ВидСправки
// Серна - Конец
	|
	|СГРУППИРОВАТЬ ПО
...
//
Процедура АвтозаполнениеБезработицаСОграничениями() Экспорт
	...
	// Проверим ЕСВ для импортированных данных
	Для каждого лСтрока Из ТЗ Цикл
		Если Не лСтрока.РезультатВзносы = 0 или (лСтрока.Период > Дата(2014, 01, 31, 23, 59, 59)) Тогда Продолжить; КонецЕсли;
		лСтрока.РезультатВзносы = Окр( 0.036 * лСтрока.РезультатВсего, 2);
	КонецЦикла;
	
	Доходы.Загрузить(ТЗ);
	
КонецПроцедуры //  АвтозаполнениеБезработицаСОграничениями

// ПечатьСернаБанк
//
Функция ПечатьСернаБанк()
	...
	//Выводим шапку
	ОбластьМакетаШапка = Макет.ПолучитьОбласть("Шапка");
	Рез=Объект.GetFIOPadegFS(ФиоСотрудника, "", 3);	
	ОбластьМакетаШапка.Параметры.Физлицо = Рез	;
		
	Если  Сотрудник.Физлицо.Пол = Перечисления.ПолФизическихЛиц.Мужской Тогда
		ОбластьМакетаШапка.Параметры.он                     =НСтр("ru='він';uk='він'",КодЯзыкаПечать);
		ОкончаниеРаботал = НСтр("ru='в';uk='в'",КодЯзыкаПечать);
	Иначе
		ОбластьМакетаШапка.Параметры.он                     =НСтр("ru='вона';uk='вона'",КодЯзыкаПечать);
		ОкончаниеРаботал = НСтр("ru='ла';uk='ла'",КодЯзыкаПечать);;
	КонецЕсли;	
		
	ТекстДоговор = "";
		
	Если Сотрудник.ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.Подряда Тогда
		ТекстРаботает = НСтр("ru='працював';uk='працюва'",КодЯзыкаПечать)+ОкончаниеРаботал+" в організації "+Организация.НаименованиеПолное +
		ТекстДоговор + НСтр("ru=' за цивільно-правовим договором';uk=' за цивільно-правовим договором'",КодЯзыкаПечать);
	Иначе	
		Если НЕ ЗначениеЗаполнено(Сотрудник.ДатаУвольнения) Тогда
			ТекстРаботает = НСтр("ru='працює в ';uk='працює в організації '",КодЯзыкаПечать) + Организация.НаименованиеПолное +НСтр("ru=' з ';uk=' з '",КодЯзыкаПечать) +
			Формат(Сотрудник.ДатаПриемаНаРаботу, "ДЛФ=Д")+НСтр("ru=' р';uk=' р'",КодЯзыкаПечать) + ТекстДоговор + НСтр("ru='. На даний момент займає посаду ';uk='. На даний момент займає посаду '",КодЯзыкаПечать) + РезультатДолжность.Должность.Наименование;
		Иначе
			ТекстРаботает = НСтр("ru='працював';uk='працюва'",КодЯзыкаПечать)+ОкончаниеРаботал+" в організації "+Организация.НаименованиеПолное +НСтр("ru=' з ';uk=' з '",КодЯзыкаПечать) +
			Формат(Сотрудник.ДатаПриемаНаРаботу, "ДЛФ=Д")+ НСтр("ru=' р.';uk=' р.'",КодЯзыкаПечать) + " по " + Формат(Сотрудник.ДатаУвольнения-86400, "ДЛФ=Д")+ НСтр("ru=' р.';uk=' р.'",КодЯзыкаПечать)  + ТекстДоговор + НСтр("ru=' на посаді ';uk=' на посаді '",КодЯзыкаПечать) + РезультатДолжность.Должность.Наименование;
		КонецЕсли;
	КонецЕсли;
	ОбластьМакетаШапка.Параметры.Номер						= Номер;
	ОбластьМакетаШапка.Параметры.Дата						= Формат(Дата, "ДЛФ=Д");
	ОбластьМакетаШапка.Параметры.ДатаНач					= Формат(ДатаНач, "ДЛФ=Д");
	ОбластьМакетаШапка.Параметры.ДатаКон					= Формат(ДатаКон, "ДЛФ=Д");
	ОбластьМакетаШапка.Параметры.СуммаДоход					= Формат(Доходы.Итог("ДоходНДФЛ"),	"ЧЦ=10; ЧДЦ=2");
	//ОбластьМакетаШапка.Параметры.ДатаНач					= Формат(МАКС(Сотрудник.ДатаПриемаНаРаботу,РезультатПериодДоходов.НачалоПериодаДоходов), "ДЛФ=Д");
	//ОбластьМакетаШапка.Параметры.ДатаКон					= Формат(?(НЕ ЗначениеЗаполнено(Сотрудник.ДатаУвольнения),РезультатПериодДоходов.ОкончаниеПериодаДоходов,МИН(Сотрудник.ДатаУвольнения,РезультатПериодДоходов.ОкончаниеПериодаДоходов)), "ДЛФ=Д");
	...
	Для Каждого ДоходыКолонка Из НайденныеСтроки Цикл
		СписокВыводимыхКолонок.Добавить(ДоходыКолонка.ИмяКолонки);
	КонецЦикла;
	
	ВыводимыеКолонки = СтрЗаменить(Строка(СписокВыводимыхКолонок),"; ",",");
	ТаблицаПечати = Доходы.Выгрузить();
	ТаблицаПечати = ТаблицаПечати.Скопировать(,ВыводимыеКолонки);
	ОбластьПериод = Макет.ПолучитьОбласть("ОбластьСтрокаПериод"); 
	ОбластьМакета = Макет.ПолучитьОбласть("ОбластьСтрока"); 
	...
	Для Каждого СтрокаТЧ Из ТаблицаПечати Цикл
		Для Каждого СтолбецТЧ из ТаблицаПечати.Колонки Цикл
			ИндексКолонки = ТаблицаПечати.Колонки.Индекс(СтолбецТЧ);
			Если ИндексКолонки = 0 Тогда
				ОбластьПериод.Параметры.Период = Формат(СтрокаТЧ.Период,	"ДФ=""ММММ ` гггг""; Л=""uk""");
			Иначе
				ОбластьМакета.Параметры.СтрокаДанные = Формат(СтрокаТЧ[ИндексКолонки],	"ЧЦ=10; ЧДЦ=2");
			КонецЕсли;	
			Если ИндексКолонки = 0 Тогда
