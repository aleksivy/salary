...
// Выполняет аворасчет реквизитов шапки
//
...
// Обновляет список выбора элемента управления ДатаВыхода
//
Процедура ОбновитьСписокПраздничныхДнейМесяца()
	
	СписокПраздников = СформироватьСписокПраздников();
	ЭлементыФормы.Начисления.Колонки.ДатаВыхода.ЭлементУправления.СписокВыбора = СписокПраздников;
	ЭлементыФормы.Начисления.Колонки.ДатаВыхода.ЭлементУправления.ВысотаСпискаВыбора = СписокПраздников.Количество();
	
КонецПроцедуры // ОбновитьСписокПраздничныхДнейМесяца()
...
Процедура ОбновитьВидимостьПоПерерассчитываемомуДокументу()
	...
	Если ЗначениеЗаполнено(ПерерассчитываемыйДокумент) Тогда
		...
		УдалитьКнопкуИзНабора("Разделитель",			КнопкиЗаполнить);	
		УдалитьКнопкуИзНабора("РаботавшимиВПраздники",	КнопкиЗаполнить);	
		Если КнопкиЗаполнить.Найти("ПоПерерассчитываемомуДокументу") = Неопределено Тогда
			...
			НоваяКнопка.Подсказка = НСтр("ru='По перерассчитываемому документу';uk='По документу, що перераховується'");
			НоваяКнопка.Пояснение = НСтр("ru='По перерассчитываемому документу';uk='По документу, що перераховується'");
		КонецЕсли;
		
	Иначе
		УдалитьКнопкуИзНабора("ПоПерерассчитываемомуДокументу", КнопкиЗаполнить);
		Если КнопкиЗаполнить.Найти("РаботавшимиВПраздники") = Неопределено Тогда
			НоваяКнопка = КнопкиЗаполнить.Вставить(0, "",ТипКнопкиКоманднойПанели.Действие,НСтр("ru='Работавшими в праздники';uk='Що працювали у свята'"), ДействиеПоКнопке);
			НоваяКнопка.Подсказка = НСтр("ru='Заполнить работниками, работавшими в праздники';uk='Заповнити працівниками, які працювали у свята'");
			НоваяКнопка.Пояснение = НСтр("ru='Заполнить работниками, рабочие дни которых пришлись на праздники';uk='Заповнити працівниками, робочі дні яких припали на свята'");
		КонецЕсли;
		Если КнопкиЗаполнить.Найти("СписокРаботников") = Неопределено Тогда
			КнопкиЗаполнить.Вставить(0, "Разделитель",ТипКнопкиКоманднойПанели.Разделитель);
			...
			НоваяКнопка.Подсказка = НСтр("ru='Заполнить списком работников';uk='Заповнити списком працівників'");
			НоваяКнопка.Пояснение = НСтр("ru='Заполнить списком работников, отобранных по произвольным условиям';uk='Заповнити списком працівників, відібраних по довільних умовах'");
		КонецЕсли;
...
Процедура ПриОткрытии()
	...
	СтруктураКолонок.Вставить("ОтработаноЧасов");
	...
	// Список доступных видов расчета
	ЭлементыФормы.Начисления.Колонки.ВидРасчета.ЭлементУправления.СписокВыбора.Добавить(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПраздничныхИВыходных);
	ЭлементыФормы.Начисления.Колонки.ВидРасчета.ЭлементУправления.СписокВыбора.Добавить(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаПраздничныеИВыходные);
...
Процедура ДействияФормыЗаполнить(Кнопка)
	...
	Если Кнопка.Имя = "СписокРаботников" Тогда
		ПроцедурыУправленияПерсоналом.ОткрытьФормуОтбораСпискаРаботников(ЭтаФорма, ПериодРегистрации, , Организация, , Ложь, ,"ОплатаПраздничныхИВыходныхДнейОрганизаций", Новый Структура("ДатаВыхода,ОтработаноЧасов,ЧасовДвойных",Дата(1,1,1),0));
	ИначеЕсли Кнопка.Имя = "ПоПерерассчитываемомуДокументу" Тогда
		...
		КонецЕсли;
	ИначеЕсли Кнопка.Имя = "РаботавшимиВПраздники" Тогда
		Если Не ЗаполнитьРаботавшимиВПраздники() Тогда
			СтрокаТекущегоМесяца = Формат(ПериодРегистрации, "ДФ=ММ");
			ЕстьПраздники = Ложь;
			СписокПраздников = СформироватьСписокПраздников();
			...
			Если ЕстьПраздники Тогда
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Данные для заполнения докумета не обнаружены! Проверьте заполнение производственного календаря на ';uk='Дані для заповнення документа не виявлені! Перевірте заповнення виробничого календаря на '") + Формат(Год(ПериодРегистрации), "ЧЦ=4; ЧГ=0") + НСтр("ru=' год!';uk=' рік!'"));
			Иначе
				ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='В текущем месяце нет общегосударственных праздников!';uk='У поточному місяці немає загальнодержавних свят!'"));
			КонецЕсли;
			
		ИначеЕсли Начисления.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьОбОшибке(НСтр("ru='Не обнаружено работников, рабочие дни которых пришлись на праздники!';uk='Не виявлено працівників, робочі дні яких припали на свята!'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнительныеДействияСписокСотрудников(Кнопка)
	...
	Если Проведен Тогда
		...
		Форма.Заголовок = НСтр("ru='Сотрудники документа Оплата праздничных и выходных дней №';uk='Співробітники документа Оплата святкових і вихідних днів №'") + Номер + НСтр("ru=' от ';uk=' від '") + Формат(Дата,"ДФ=dd.MM.yyyy");
		Форма.Открыть();
...
Процедура НачисленияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СтрокиРаботника = Начисления.НайтиСтроки(Новый Структура("Сотрудник", ВыбранноеЗначение));
	Если СтрокиРаботника.Количество() = 0 Тогда
		...
		ПроведениеРасчетов.ПроставитьДанныеСтроки(ОснНазначение, ВыбранноеЗначение, Дата, ПериодРегистрации, ЭлементыФормы.Начисления.ТекущаяСтрока.ВидРасчета, ЭлементыФормы.Начисления.ТекущаяСтрока);
			
	ИначеЕсли СтрокиРаботника.Количество() = 1 Тогда
		ВидРасчета = СтрокиРаботника[0].ВидРасчета;
		Если Не ВидРасчета.Пустая() Тогда
			...
			ОснНазначение = ?(ЗначениеЗаполнено(ВыбранноеЗначение.ОсновноеНазначение), ВыбранноеЗначение.ОсновноеНазначение, ВыбранноеЗначение);
			ПроведениеРасчетов.ПроставитьДанныеСтроки(ОснНазначение, ВыбранноеЗначение, Дата, ПериодРегистрации, ЭлементыФормы.Начисления.ТекущаяСтрока.ВидРасчета, ЭлементыФормы.Начисления.ТекущаяСтрока);
			СтрокаТабличнойЧасти.ВидРасчета = ?(ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаПраздничныеИВыходные, ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПраздничныхИВыходных, ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаПраздничныеИВыходные);
		КонецЕсли;
...
Процедура НачисленияСотрудникПриИзменении(Элемент)
	...
	ПроведениеРасчетов.ПроставитьДанныеСтроки(ОснНазначение, ДанныеСтроки.Сотрудник, ДатаСобытия, ПериодРегистрации, ЭлементыФормы.Начисления.ТекущаяСтрока.ВидРасчета, ЭлементыФормы.Начисления.ТекущаяСтрока);
	ДанныеСтроки.Физлицо = Элемент.Значение.Физлицо;
	                                       
	//Предполагаем, что если работник повторяется, то после оплаты вводится доплата
	СтрокаЭтогоЖеРаботника = Начисления.Найти(ДанныеСтроки.Сотрудник, "Сотрудник");
	Если СтрокаЭтогоЖеРаботника <> Неопределено Тогда
		Если ДанныеСтроки.ВидРасчета.Пустая() Или СтрокаЭтогоЖеРаботника.ВидРасчета = ДанныеСтроки.ВидРасчета Тогда // попробуем угадать вид расчета
			ДанныеСтроки.ВидРасчета = ?(СтрокаЭтогоЖеРаботника.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаПраздничныеИВыходные, ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПраздничныхИВыходных, ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаПраздничныеИВыходные);
		КонецЕсли;
	КонецЕсли;
...
Процедура НачисленияСотрудникОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	//Предполагаем, что если работник повторяется, то после оплаты вводится доплата
	ДанныеСтроки = ЭлементыФормы.Начисления.ТекущиеДанные;
	СтрокаЭтогоЖеРаботника = Начисления.Найти(ВыбранноеЗначение, "Сотрудник");
	Если СтрокаЭтогоЖеРаботника <> Неопределено Тогда
		Если ДанныеСтроки.ВидРасчета.Пустая() ИЛИ СтрокаЭтогоЖеРаботника.ВидРасчета = ДанныеСтроки.ВидРасчета Тогда // попробуем угадать вид расчета
			ДанныеСтроки.ВидРасчета = ?(СтрокаЭтогоЖеРаботника.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаПраздничныеИВыходные, ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПраздничныхИВыходных, ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаПраздничныеИВыходные);
		КонецЕсли;
...
// Процедура - обработчик события "НачалоВыбора" вида расчета
//
...
// Процедура - обработчик события "ПриИзменении" отработанных часов
//
...
// Процедура - обработчик события "ПриИзменении" размера часовой ставка
//
