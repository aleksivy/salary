...
Процедура ОбновитьВидимостьЭлементовФормы()
	...
	ЭлементыФормы.ПанельПериодаОплаты.ТекущаяСтраница = ?(ЭтоЦелосменныйНевыход,
	...
	Если ВидРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаДоСреднегоЗаработка Тогда
		ЭлементыФормы.Панель.Страницы.Начисления.Видимость = Ложь;
		ЭлементыФормы.Назначение.Видимость = Истина;
		ЭлементыФормы.НадписьНазначение.Видимость = Истина;
		
	Иначе
		ЭлементыФормы.Панель.Страницы.Начисления.Видимость = Истина;
		ЭлементыФормы.Назначение.Видимость = Ложь;
		ЭлементыФормы.НадписьНазначение.Видимость = Ложь;
		
	КонецЕсли;

	
	// страница расчета средней
...
Процедура УстановитьЗначенияКолонкиТаблицыНачислений(ИмяКолонки, Значение)
...
Процедура ОбновитьОписаниеСостоянияДокумента()
	...
	НеЗаполнен = НеЗаполнен Или Не ЗначениеЗаполнено(ДатаНачала);
...
Процедура ПриОткрытии()
	...
	Если ЭтоНовый() Тогда
		...
		Если ВидУчетаВремениДляСредней.Пустая() Тогда
	...
	// Список видов расчета по среднему
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |
	                      |ГДЕ
	                      |	ОсновныеНачисленияОрганизаций.СпособРасчета В(&МассивСпоcобовРасчета) 	                      
  						  |	И ОсновныеНачисленияОрганизаций.Ссылка <> ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ПростойПоСредней)
						  |	И ОсновныеНачисленияОрганизаций.Ссылка <> ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ПочасовойПростойПоСредней)
						  |	И ОсновныеНачисленияОрганизаций.Ссылка <> ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ВыходноеПособие)
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	ОсновныеНачисленияОрганизаций.Наименование");
				   
	МассивСпоcобовРасчета = Новый Массив;			   
	МассивСпоcобовРасчета.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработку);
	МассивСпоcобовРасчета.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ДоплатаДоСреднегоЗаработка);
	Запрос.УстановитьПараметр("МассивСпоcобовРасчета", МассивСпоcобовРасчета);
	...
	ЭлементыФормы.ВидРасчета.СписокВыбора = СписокВидовРасчетов;
...
Процедура ДействияФормыРассчитатьВсе(Кнопка)
	...
	Если ВидРасчета <> ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаДоСреднего Тогда
...
Процедура ВидРасчетаПриИзменении(Элемент)
	...
	УстановитьЗначенияКолонкиТаблицыНачислений("ВидРасчета", ВидРасчета);
...
Процедура ДатаНачалаПриИзменении(Элемент)
	...
	ЭтоЦелосменныйНевыход = СпособРегистрацииВремени = Перечисления.СпособыРегистрацииВремени.РегистрацияДляЦелойСмены;
	ДатаОкончания = ?(ЭтоЦелосменныйНевыход, Макс(ДатаНачала,ДатаОкончания), ДатаНачала);
...
Процедура РасчитатьПараметрыСредней()
	...
	Если ДатаНачала = Дата('00010101') ИЛИ НЕ ЗначениеЗаполнено(Сотрудник) Тогда
		ПериодРасчетаСреднегоНачало	= Дата('00010101');
		ПериодРасчетаСреднегоКонец	= Дата('00010101');
		ВидУчетаВремениДляСредней = Перечисления.ВидыУчетаВремениДляСредней.ПоРабочимДням;
		
	Иначе
		//ВыборкаПериодПоРаботнику = ПолучитьПериодПоРаботнику();
		//
		//ПериодРасчетаСреднегоНачало	= ВыборкаПериодПоРаботнику.ПериодРасчетаСреднегоНачало;
		//ПериодРасчетаСреднегоКонец	= ВыборкаПериодПоРаботнику.ПериодРасчетаСреднегоКонец;
		//
		//Если ВыборкаПериодПоРаботнику.ГрафикРаботы.СуммированныйУчетРабочегоВремени 
		//	ИЛИ СпособРегистрацииВремени = Перечисления.СпособыРегистрацииВремени.РегистрацияДляЧастиСмены
		//		
		//		ИЛИ ВыборкаПериодПоРаботнику.ОсновнойВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОкладПоЧасам
		//		ИЛИ ВыборкаПериодПоРаботнику.ОсновнойВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ТарифЧасовой
		//	Тогда
...
Процедура НачисленияПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	//ПроведениеРасчетов.ПриВыводеСтрокиНачислений(Элемент, ОформлениеСтроки, ДанныеСтроки, мСведенияОВидахРасчета);
