...
// Серна - Начало
	...
	лЗапрос.УстановитьПараметр("ДатаУвольнения", ДатаУвольнения);
	лЗапрос.УстановитьПараметр("Сотрудник", Сотрудник);
	лЗапрос.Текст =
	"ВЫБРАТЬ
	|	РегОстатки.Сотрудник,
	|	СУММА(РегОстатки.КвоДнейОстаток) КАК КвоДнейОстаток
	|ПОМЕСТИТЬ РегОстатки
	|ИЗ
	|	РегистрНакопления.СернаДниОтпуска.Остатки(
	|			НАЧАЛОПЕРИОДА(&ДатаУвольнения, ДЕНЬ),
	|			Сотрудник = &Сотрудник
	|				И НЕ ВидОтпуска.ОтпускНеПереноситсяНаСледующийГод = ЗНАЧЕНИЕ(Справочник.ВидыОтпусков.ЕжегодныйОсновной)) КАК РегОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	РегОстатки.Сотрудник
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(РегПериод.Период) КАК Период,
	|	ЕСТЬNULL(РегОстатки.КвоДнейОстаток, 0) КАК КвоДнейОстаток,
	|	ЕСТЬNULL(НормаОтпуска.КвоДней, НормаОтпускаДляВсех.КвоДней) КАК НормаДней
	|ПОМЕСТИТЬ Врем
	|ИЗ
	|	РегистрНакопления.СернаДниОтпуска КАК РегПериод
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегОстатки КАК РегОстатки
	|		ПО РегПериод.Сотрудник = РегОстатки.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ггуНормыОтпусков.СрезПоследних(
	|				&ДатаУвольнения,
	|				ВидОтпуска = ЗНАЧЕНИЕ(Справочник.ВидыОтпусков.ЕжегодныйОсновной)
	|					И Сотрудник = &Сотрудник) КАК НормаОтпуска
	|		ПО РегПериод.Сотрудник = НормаОтпуска.Сотрудник,
	|	РегистрСведений.ггуНормыОтпусков.СрезПоследних(
	|			&ДатаУвольнения,
	|			ВидОтпуска = ЗНАЧЕНИЕ(Справочник.ВидыОтпусков.ЕжегодныйОсновной)
	|				И Сотрудник = ЗНАЧЕНИЕ(Справочник.СотрудникиОрганизаций.ПустаяСсылка)) КАК НормаОтпускаДляВсех
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(РегПериод.Регистратор) = ТИП(Документ.ДниОтпускаОрганизаций)
	|	И НЕ РегПериод.ЭтоКорректировка
	|	И РегПериод.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И РегПериод.ВидОтпуска = ЗНАЧЕНИЕ(Справочник.ВидыОтпусков.ЕжегодныйОсновной)
	|	И РегПериод.Сотрудник = &Сотрудник
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(НормаОтпуска.КвоДней, НормаОтпускаДляВсех.КвоДней),
	|	ЕСТЬNULL(РегОстатки.КвоДнейОстаток, 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫРАЗИТЬ(Врем.НормаДней * РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Врем.Период, ДЕНЬ), НАЧАЛОПЕРИОДА(КОНЕЦПЕРИОДА(&ДатаУвольнения, ДЕНЬ), ДЕНЬ), ДЕНЬ) / (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(&ДатаУвольнения, МЕСЯЦ), НАЧАЛОПЕРИОДА(КОНЕЦПЕРИОДА(&ДатаУвольнения, МЕСЯЦ), ДЕНЬ), ДЕНЬ) + 1) / 12 КАК ЧИСЛО(12, 0)) КАК ДоначисленоДней
	|ИЗ
	|	Врем КАК Врем";
	...
	Если лВыборка.Следующий() Тогда
		Если лВыборка.КвоДнейОстаток = Неопределено или лВыборка.КвоДнейОстаток = null Тогда
			лКолвоДней = 0;
		Иначе
			лКолвоДней = лВыборка.КвоДнейОстаток + лВыборка.ДоначисленоДней;
		КонецЕсли;
	КонецЕсли;
	Возврат лКолвоДней;
// Серна - Конец
