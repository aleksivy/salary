...
// Серна - Начало
Процедура РаботникиОрганизацииБанкПриИзменении(Элемент)
	Если Элемент.Значение = Перечисления.СпособыВыплатыЗарплаты.ЧерезКассу Тогда
		ЭлементыФормы.РаботникиОрганизации.ТекущиеДанные.Банк = Справочники.Контрагенты.ПустаяСсылка(); 
		ЭлементыФормы.РаботникиОрганизации.ТекущиеДанные.НомерКарточки = "";
	Иначе
		ТекущаяСтрока = ЭлементыФормы.РаботникиОрганизации.ТекущаяСтрока;
		Если ТекущаяСтрока.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк Тогда
			Если ЗначениеЗаполнено(ТекущаяСтрока.Сотрудник)Тогда
				Запрос = Новый Запрос;
				Запрос.УстановитьПараметр("парамДата",		Дата);
				Запрос.УстановитьПараметр("парамСотрудник", ТекущаяСтрока.Сотрудник);
				...
				Запрос.Текст = ТекстЗапроса;
				Выборка = Запрос.Выполнить().Выбрать();
				лБанк = ТекущаяСтрока.Банк;
				лНомерКарточки = "";
				Пока Выборка.Следующий() Цикл
					Если Не Выборка.Банк = лБанк Тогда Продолжить; КонецЕсли;
					лНомерКарточки = СокрЛП(Выборка.НомерКарточки);
					Прервать;
				КонецЦикла;
				Если ЗначениеЗаполнено(лНомерКарточки) Тогда
					ТекущаяСтрока.НомерКарточки = лНомерКарточки;
				Иначе
					Сообщить("Не найден номер кредитной карточки в банке - " + лБанк + " для сотрудника - " + ТекущаяСтрока.Сотрудник);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
