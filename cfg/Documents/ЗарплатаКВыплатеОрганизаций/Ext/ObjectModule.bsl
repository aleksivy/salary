...
// Серна - Начало
	|	АвансыРаботникамОрганизаций.Валюта,
// Серна - Конец
...
// Серна - Начало
		Если Выборка.Валюта = глЗначениеПеременной("ВалютаРегламентированногоУчета") Тогда
			лСумма = Выборка.СуммаАванса;
		Иначе
			лстрКурс = РегистрыСведений.КурсыВалютДляРасчетовСПерсоналом.ПолучитьПоследнее(Дата, Новый Структура("Валюта", Выборка.Валюта));
			лСумма = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(Выборка.СуммаАванса, Выборка.Валюта, глЗначениеПеременной("ВалютаРегламентированногоУчета"), лстрКурс.Курс, 1, лстрКурс.Кратность, 1);
		КонецЕсли;
		НоваяСтрока.Сумма					= лСумма;
		НоваяСтрока.СуммаГрязными			= лСумма * ?(ВидВыплаты.РасчетОтОбратного,1.5,1);
		НоваяСтрока.ВыплаченностьЗарплаты	= Перечисления.ВыплаченностьЗарплаты.Выплачено;
		НоваяСтрока.СпособВыплаты			= ?(Выборка.СпособВыплаты= NULL, Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк, Выборка.СпособВыплаты);
// Серна - Конец
