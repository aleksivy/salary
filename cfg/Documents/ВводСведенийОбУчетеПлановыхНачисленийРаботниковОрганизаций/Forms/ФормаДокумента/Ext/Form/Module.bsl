...
// Процедура - обработчик события "ПриИзменении" поля ввода основания.
//
Процедура ДокументОснованиеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Элемент.Значение) Тогда
		ЗаполнитьДокумент(Элемент.Значение);
	КонецЕсли;
...
Процедура КоманднаяПанельРаботникиОрганизацииСписокРаботников(Кнопка)
	...
	ПроцедурыУправленияПерсоналом.ОткрытьФормуОтбораСпискаРаботников(ЭтаФорма, Дата, , Организация,,,,"СведенияОбУчетеПлановыхНачислений", Новый Структура("ВидРасчетаДляУчетаПлановыхНачислений,СпособОтраженияВБухУчетеДляУчетаПлановыхНачислений, ДатаДляУчетаПлановыхНачислений",,,ТекущаяДата()));
...
Процедура ОбработкаВыбора(ЗначениеВыбора, Источник)
	Если ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		...
		Если ЗначениеВыбора.Свойство("Команда",Команда) и Команда = "ЗаполнитьСписокРаботников" Тогда
			...
			Реквизиты = ЗначениеВыбора.Реквизиты;
			
			ВидРасчетаДляУчетаПлановыхНачислений = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПустаяСсылка();
			СпособОтраженияВБухУчетеДляУчетаПлановыхНачислений = Справочники.СпособыОтраженияЗарплатыВРеглУчете.ПустаяСсылка();
			ДатаДляУчетаПлановыхНачислений = Дата(1,1,1);
			
			Для Каждого Элемент Из Реквизиты Цикл
				Если Элемент.Ключ = "ВидРасчетаДляУчетаПлановыхНачислений" Тогда
					ВидРасчетаДляУчетаПлановыхНачислений = Элемент.Значение;
				ИначеЕсли Элемент.Ключ = "СпособОтраженияВБухУчетеДляУчетаПлановыхНачислений" Тогда
					СпособОтраженияВБухУчетеДляУчетаПлановыхНачислений = Элемент.Значение;
				ИначеЕсли Элемент.Ключ = "ДатаДляУчетаПлановыхНачислений" Тогда
					ДатаДляУчетаПлановыхНачислений = Элемент.Значение;
				КонецЕсли;
			КонецЦикла;	
			...
			Для Каждого СтрокаТЗ Из ЗначениеВыбора.Данные.Выгрузить() Цикл
				...
				СтрокаТабличнойЧасти.Сотрудник = 	СтрокаТЗ.Сотрудник;
				СтрокаТабличнойЧасти.ВидРасчета 				= ВидРасчетаДляУчетаПлановыхНачислений;
				СтрокаТабличнойЧасти.СпособОтраженияВБухучете 	= СпособОтраженияВБухУчетеДляУчетаПлановыхНачислений;
				СтрокаТабличнойЧасти.ДатаНачала 				= ДатаДляУчетаПлановыхНачислений;			
