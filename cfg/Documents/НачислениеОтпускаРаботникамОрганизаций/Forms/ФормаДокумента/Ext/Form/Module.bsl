...
Процедура ОбновитьВидимостьЭлементовФормы()
	...
	// УстановитьСоответсвиеСтраницОтпуска
	Если ВидДокументаОтпуск = Перечисления.ВидыДокументаОтпуск.Отпускные Тогда
		ЭлементыФормы.ПанельВидовДокумента.ТекущаяСтраница =  ЭлементыФормы.ПанельВидовДокумента.Страницы.Отпускные;
		
	ИначеЕсли ВидДокументаОтпуск = Перечисления.ВидыДокументаОтпуск.КомпенсацияОтпуска Тогда
		ЭлементыФормы.ПанельВидовДокумента.ТекущаяСтраница =  ЭлементыФормы.ПанельВидовДокумента.Страницы.КомпенсацияОтпуска;
		
	ИначеЕсли ВидДокументаОтпуск = Перечисления.ВидыДокументаОтпуск.ОтзывИзОтпуска Тогда
		ЭлементыФормы.ПанельВидовДокумента.ТекущаяСтраница =  ЭлементыФормы.ПанельВидовДокумента.Страницы.ОтзывИзОтпуска;
...
Процедура ОбновитьОписаниеСостоянияДокумента()
	...
	НеЗаполнен = Не ЗначениеЗаполнено(Организация) Или Не ЗначениеЗаполнено(Сотрудник) Или Не ЗначениеЗаполнено(ВидДокументаОтпуск);
	ЗаполненСОшибками = Ложь;
	
	Если ВидДокументаОтпуск = Перечисления.ВидыДокументаОтпуск.Отпускные
	 или ВидДокументаОтпуск = Перечисления.ВидыДокументаОтпуск.ОтзывИзОтпуска Тогда
	 	НеЗаполнен = НеЗаполнен Или (Отпуска.Количество() = 0);
		Для Каждого Отпуск из Отпуска Цикл
			НеЗаполнен = НеЗаполнен Или НЕ ЗначениеЗаполнено(Отпуск.ВидРасчета);
			ЗаполненСОшибками = ЗаполненСОшибками Или НЕ ЗначениеЗаполнено(Отпуск.ДатаНачала) Или НЕ ЗначениеЗаполнено(Отпуск.ДатаОкончания);
			ЗаполненСОшибками = ЗаполненСОшибками Или (Отпуск.ДатаНачала > Отпуск.ДатаОкончания);
		КонецЦикла;	
	ИначеЕсли ВидДокументаОтпуск = Перечисления.ВидыДокументаОтпуск.КомпенсацияОтпуска Тогда	
		НеЗаполнен = НеЗаполнен Или (ДнейЧасовКомпенсацииОтпуска = 0); 
	КонецЕсли; 
...
Процедура ПриОткрытии()
	...
	Если ЭтоНовый() Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВидДокументаОтпуск) Тогда
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |
	                      |ГДЕ
	                      |	ОсновныеНачисленияОрганизаций.СпособРасчета В(&МассивСпопобовРасчета) И
	                      |	ОсновныеНачисленияОрганизаций.Ссылка <> &ВидРасчетаОтпускОсновной
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	ОсновныеНачисленияОрганизаций.Наименование");
				   
	////МассивСпопобовРасчета = Новый Массив;			   
	////МассивСпопобовРасчета.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуДляОтпуска);
	////Запрос.УстановитьПараметр("МассивСпопобовРасчета", МассивСпопобовРасчета);
	////Запрос.УстановитьПараметр("ВидРасчетаОтпускОсновной", ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаПоСреднемуОтп);
	////
	////СписокВидовРасчетов = Новый СписокЗначений;
	////СписокВидовРасчетов.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ВидРасчета"));
	////ЭлементыФормы.ВидРасчетаДополнительногоОтпуска.СписокВыбора = СписокВидовРасчетов;
	////
...
Процедура ДействияФормыРассчитатьВсе(Кнопка)
	...
	Если НЕ(ЗначениеЗаполнено(ВидДокументаОтпуск)) Тогда
		Предупреждение(НСтр("ru='Перед заполнением документа необходимо выбрать вид документа';uk='Перед заповненням документа необхідно вибрати вид документа'"));
		Возврат
	КонецЕсли;
...
Процедура ПриказОтпускОрганизацииПриИзменении(Элемент)
	ОбработкаЗаполненияПоСотруднику();
	ОбновитьВидимостьЭлементовФормы();
КонецПроцедуры
...
Процедура ПервичныйОтпускПриИзменении(Элемент)
	...
	Если Вопрос(НСтр("ru='Список времени в отпуске будет очищен и перезаполнен';uk='Список часу у відпустці буде очищений і перезаповнений'"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		...
		ЧислоМесяцев				= ПервичныйОтпуск.ЧислоМесяцев;
		ВидУчетаВремениДляСредней	= ПервичныйОтпуск.ВидУчетаВремениДляСредней;
		
		Отпуска.Очистить();	
		
		Для каждого СтрокаОтпуска Из ПервичныйОтпуск.Отпуска Цикл
			
			СтрокаОтзыва = Отпуска.Добавить();
...
Процедура РасчетСреднегоПередНачаломДобавления(Элемент, Отказ, Копирование)
	...
	Если ЭлементыФормы.Сотрудник=Неопределено Тогда
		Предупреждение(НСтр("ru='Перед началом заполнения табличной части необходимо выбрать работника';uk='Перед початком заповнення табличної частини необхідно вибрати працівника'"));
		Отказ = Истина;
	КонецЕсли; 
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЧНОЙ ЧАСТИ Отпуска

Процедура ОтпускаВидРасчетаНачалоВыбора(Элемент, СтандартнаяОбработка)
...
Процедура ПервичныйОтпускОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Если ВыбранноеЗначение.ПериодРегистрации >= ПериодРегистрации Тогда
		Предупреждение(НСтр("ru='Период регистрации в выбираемом документе должен быть меньше текущего!';uk='Період реєстрації в документі, що вибирається, повинен бути менше поточного!'"));
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
