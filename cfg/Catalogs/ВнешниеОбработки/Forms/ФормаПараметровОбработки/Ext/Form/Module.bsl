...
Процедура ПараметрыОбработкиПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ПустаяСтрока(ИмяПараметра) Тогда
			...
			Предупреждение(НСтр("ru='Имя параметра не может быть пустым!';uk=""Ім'я параметра не може бути порожнім!"""));
		...
		Исключение
			...
			Предупреждение(НСтр("ru='Задано неправильное имя параметра!';uk=""Задано неправильне ім'я параметра!"""));
		...
		Если КоличествоПараметров > 1 Тогда
			...
			Ответ = Вопрос(НСтр("ru='Параметр с именем ""';uk='Параметр із ім''ям ""'") + ИмяПараметра + НСтр("ru='"" уже сужествует. Отменить ввод?';uk='"" вже існує. Скасувати введення?'"), РежимДиалогаВопрос.ДаНет);
