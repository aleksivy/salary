...
Процедура ПолеТабличногоДокументаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТекущаяОбласть.Низ = ТекущаяОбласть.Верх Тогда
		...
		Если НЕ Ссылка.Пустая() Тогда
			Вопрос = НСтр("ru='В справочнике ""Валюты"" уже существует элемент с кодом ""';uk='В довіднику ""Валюти"" вже існує елемент з кодом ""'") + КодЧисловой + НСтр("ru='""! Открыть существующий?';uk='""! Відкрити існуючий?'");
	...
	Иначе
		...
		Для ТекущаяСтрока = ТекущаяОбласть.Верх по ТекущаяОбласть.Низ Цикл
			...
			ЕСли ЭтоЧисло Тогда
				...
				СтрокаПодбора.Видимость = Справочники.Валюты.НайтиПоКоду(КодЧисловой).Пустая();
