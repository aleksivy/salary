...
Процедура ОбновлениеОтображения()
	...
	СформироватьНадписиПолейПрописиНаУкраинском();
...
Процедура СписокКурсыВалютПередНачаломДобавления(Элемент, Отказ, Копирование)
	...
	Если ЭтоНовый() Тогда
		Ответ = Вопрос(НСтр("ru='Элемент еще не записан. Записать?';uk='Елемент ще не записаний. Записати?'"), РежимДиалогаВопрос.ОКОтмена);
...
Процедура НаименованиеПолноеПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ЭлементыФормы.ПолеПрописи1наУкраинском.Значение) Тогда
		ЭлементыФормы.ПолеПрописи1наУкраинском.Значение = Нрег(НаименованиеПолное);
	КонецЕсли;
...
// Обработчик события ПриИзменении элементов ПолеПрописи1наУкраинском, ..., ПолеПрописи8наУкраинском.
//
Процедура ПолеПрописиНаУкраинскомПриИзменении(Элемент)

	СформироватьПараметрыПрописиНаУкраинском();
...
Процедура ПрочитатьПараметрыПрописи()
	...
	СтрокаПараметров = СтрЗаменить(ПараметрыПрописиНаУкраинском, ",", Символы.ПС);

	ЭлементыФормы.ПолеПрописи1наУкраинском.Значение        = СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 1));
	ЭлементыФормы.ПолеПрописи2наУкраинском.Значение        = СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 2));
	ЭлементыФормы.ПолеПрописи3наУкраинском.Значение        = СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 3));
	ЭлементыФормы.ПолеПрописи4наУкраинском.Значение        = СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 4));
	ЭлементыФормы.ПолеПрописи5наУкраинском.Значение        = СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 5));
	ЭлементыФормы.ПолеПрописи6наУкраинском.Значение        = СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 6));
	ЭлементыФормы.ПолеПрописи7наУкраинском.Значение        = СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 7));
	ЭлементыФормы.ПолеПрописи8наУкраинском.Значение        = СокрЛП(СтрПолучитьСтроку(СтрокаПараметров, 8));
	...
	Если ЭлементыФормы.ПолеПрописи4наУкраинском.Значение = Неопределено Тогда
		ЭлементыФормы.ПолеПрописи4наУкраинском.Значение = ЭлементыФормы.ПолеПрописи4наУкраинском.СписокВыбора[0].Значение;
	КонецЕсли;

	Если ЭлементыФормы.ПолеПрописи8наУкраинском.Значение = Неопределено Тогда
		ЭлементыФормы.ПолеПрописи8наУкраинском.Значение = ЭлементыФормы.ПолеПрописи8наУкраинском.СписокВыбора[0].Значение;
	КонецЕсли;
...
// Процедура формирует параметры прописи для украинского языка из соответствующих полей диалога.
//
// Параметры:
//  Нет.
//
Процедура СформироватьПараметрыПрописиНаУкраинском()

	ПараметрыПрописиНаУкраинском = ЭлементыФормы.ПолеПрописи1наУкраинском.Значение + ", "
	                             + ЭлементыФормы.ПолеПрописи2наУкраинском.Значение + ", "
	                             + ЭлементыФормы.ПолеПрописи3наУкраинском.Значение + ", "
	                             + ЭлементыФормы.ПолеПрописи4наУкраинском.Значение + ", "
	                             + ЭлементыФормы.ПолеПрописи5наУкраинском.Значение + ", "
	                             + ЭлементыФормы.ПолеПрописи6наУкраинском.Значение + ", "
	                             + ЭлементыФормы.ПолеПрописи7наУкраинском.Значение + ", "
	                             + ЭлементыФормы.ПолеПрописи8наУкраинском.Значение + ", "
...
// Процедура формирует надписи к полям прописи для украинского языка.
//
// Параметры:
//  Нет.
//
Процедура СформироватьНадписиПолейПрописиНаУкраинском()

	Надписи = Новый Структура("м1, м2, м3, ж1, ж2, ж3, с1, с2, с3");

	Надписи.м1 = "один:"; Надписи.м2 = "два:"; Надписи.м3 = "п'ять:";
	Надписи.ж1 = "одна:"; Надписи.ж2 = "дві:"; Надписи.ж3 = "п'ять:";
	Надписи.с1 = "одне:"; Надписи.с2 = "два:"; Надписи.с3 = "п'ять:";

	Род = ЭлементыФормы.ПолеПрописи4наУкраинском.Значение;

	ЭлементыФормы.НадписьПрописи1наУкраинском.Заголовок = Надписи[Род + "1"];
	ЭлементыФормы.НадписьПрописи2наУкраинском.Заголовок = Надписи[Род + "2"];
	ЭлементыФормы.НадписьПрописи3наУкраинском.Заголовок = Надписи[Род + "3"];

	Род = ЭлементыФормы.ПолеПрописи8наУкраинском.Значение;

	ЭлементыФормы.НадписьПрописи5наУкраинском.Заголовок = Надписи[Род + "1"];
	ЭлементыФормы.НадписьПрописи6наУкраинском.Заголовок = Надписи[Род + "2"];
	ЭлементыФормы.НадписьПрописи7наУкраинском.Заголовок = Надписи[Род + "3"];
...
Процедура СформироватьНадписиПолейПрописиНаРусском()
	...
	ЭлементыФормы.НадписьПрописи1наРусском.Заголовок = Надписи[Род + "1"];
	ЭлементыФормы.НадписьПрописи2наРусском.Заголовок = Надписи[Род + "2"];
	ЭлементыФормы.НадписьПрописи3наРусском.Заголовок = Надписи[Род + "3"];

	Род = ЭлементыФормы.ПолеПрописи8наРусском.Значение;

	ЭлементыФормы.НадписьПрописи5наРусском.Заголовок = Надписи[Род + "1"];
	ЭлементыФормы.НадписьПрописи6наРусском.Заголовок = Надписи[Род + "2"];
	ЭлементыФормы.НадписьПрописи7наРусском.Заголовок = Надписи[Род + "3"];
...
// Род целой части на украинском языке
ЭлементыФормы.ПолеПрописи4наУкраинском.СписокВыбора.Очистить();
ЭлементыФормы.ПолеПрописи4наУкраинском.СписокВыбора.Добавить("м", "Мужской");
ЭлементыФормы.ПолеПрописи4наУкраинском.СписокВыбора.Добавить("ж", "Женский");
ЭлементыФормы.ПолеПрописи4наУкраинском.СписокВыбора.Добавить("с", "Средний");
...
// Род дробной части на украинском языке
ЭлементыФормы.ПолеПрописи8наУкраинском.СписокВыбора.Очистить();
ЭлементыФормы.ПолеПрописи8наУкраинском.СписокВыбора.Добавить("м", "Мужской");
ЭлементыФормы.ПолеПрописи8наУкраинском.СписокВыбора.Добавить("ж", "Женский");
ЭлементыФормы.ПолеПрописи8наУкраинском.СписокВыбора.Добавить("с", "Средний");
