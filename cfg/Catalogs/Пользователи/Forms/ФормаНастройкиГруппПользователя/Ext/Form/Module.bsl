Процедура ПриОткрытии()
	...
	ЭлементыФормы.НадписьПользователь.Заголовок = НСтр("ru='Пользователь: ';uk='Користувач: '") + СокрЛП(Пользователь);
...
Функция СохранитьЗначения()
	...
	Для каждого СтрокаТЗ Из ДеревоГрупп.Строки.НайтиСтроки(Новый Структура("ВсеСтроки", Истина), Истина) Цикл
		...
		Попытка
			Объект.Записать();
		Исключение
			ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Изменения не сохранены!';uk='Зміни не збережені!'"));
			ОтменитьТранзакцию();
			Возврат Ложь;
		КонецПопытки;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		ОтветНаВопрос = Вопрос(НСтр("ru='Данные были изменены. Сохранить?';uk='Дані були змінені. Зберегти?'"), РежимДиалогаВопрос.ДаНетОтмена);
