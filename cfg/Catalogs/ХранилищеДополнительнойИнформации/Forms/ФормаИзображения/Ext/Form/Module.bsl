...
Процедура ПередЗаписью(Отказ)
	...
	Если ВидДанных = Перечисления.ВидыДополнительнойИнформацииОбъектов.Изображение Тогда
		Если Хранилище.Получить() = Неопределено
		 ИЛИ Хранилище.Получить().Вид = ВидКартинки.Пустая Тогда
			Предупреждение(НСтр("ru='Выберите изображение.';uk='Виберіть зображення.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Объект = Неопределено ИЛИ Объект.Пустая() Тогда
		Предупреждение(НСтр("ru='Укажите владельца изображения.';uk='Вкажіть власника зображення.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ДействияФормыСохранитьИзображение(Кнопка)
	...
	Если ЗаписываемаяКартинка.Вид <> ВидКартинки.Пустая Тогда
		...
		ДиалогЗаписиФайла.Заголовок                   = НСтр("ru='Укажите имя файла';uk=""Вкажіть ім'я файлу""");
		...
		Исключение
			Предупреждение(НСтр("ru='Ошибка при записи файла: ';uk='Помилка при запису файлу: '") + ДиалогЗаписиФайла.ПолноеИмяФайла + Символы.ПС
			              + ОписаниеОшибки() + Символы.ПС + НСтр("ru='Файл не записан.';uk='Файл не записаний.'"));
		КонецПопытки;
