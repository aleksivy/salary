...
Процедура ПриОткрытии()
	...
	ИначеЕсли мКопирование И ЗначениеЗаполнено(мСсылкаОснование) Тогда
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый() Тогда
		...
		Если Запрос.Выполнить().Пустой() Тогда
			Предупреждение(НСтр("ru='Вы не являетесь администратором ни одной учетной записи."
"Группы почтовой рассылки могут создавать и редактировать только администраторы учетных записей.';uk='Ви не є адміністратором  жодного облікового запису."
"Групи поштової розсилки можуть створювати й редагувати тільки адміністратори облікових записів.'"));
			Отказ = Истина;
		КонецЕсли;
...
Процедура ПослеЗаписи()
	...
	Если ТипЗнч(ВладелецФормы) = Тип("Форма") И ЗначениеЗаполнено(Ссылка) Тогда
...
Процедура УчетныеЗаписиПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущаяКолонка = Элемент.Колонки.УчетнаяЗапись И Элемент.ТекущиеДанные <> Неопределено И ЗначениеЗаполнено(Элемент.ТекущиеДанные.УчетнаяЗапись) Тогда
		Отказ = Истина;
