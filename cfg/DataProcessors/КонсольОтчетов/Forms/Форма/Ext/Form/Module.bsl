...
Процедура СкопироватьПараметрыСтроки(ИсходнаяСтрока, КонечнаяСрока)
	...
	КонечнаяСрока.НастройкиПостроителя = ИсходнаяСтрока.НастройкиПостроителя;
	...
	КонечнаяСрока.ПредставленияДляИмен = ИсходнаяСтрока.ПредставленияДляИмен;
	КонечнаяСрока.ФорматыДляИмен = ИсходнаяСтрока.ФорматыДляИмен;
	...
	КонечнаяСрока.Макет = ИсходнаяСтрока.Макет;
...
"Выберите другой файл.';uk='Неможливо завантажити список звітів із зазначеного файлу!"
"Виберіть інший файл.'"));
		   	КонецЕсли;
		   	Возврат Ложь;
		КонецЕсли;
...
Функция СохранитьЗапросыВФайл(ЗапрашиватьСохранение = Ложь, ЗапрашиватьИмяФайла = Ложь)
	...
	Если Не ЗапрашиватьИмяФайла Тогда
		Если ЗапрашиватьСохранение Тогда
			Если Не Модифицированность Тогда
				Возврат Истина;
			Иначе
				Ответ = Вопрос(НСтр("ru='Сохранить текущие отчеты?';uk='Зберегти поточні звіти?'"), РежимДиалогаВопрос.ДаНетОтмена);
	...
	Если ПустаяСтрока(ИмяФайла) или ЗапрашиватьИмяФайла Тогда
		...
		Длг.Каталог = ИмяПути;
		Длг.Заголовок = НСтр("ru='Укажите файл для списка отчетов';uk='Вкажіть файл для списку звітів'");
		Длг.Фильтр = "Файлы отчетов (*.rcf)|*.rcf|Все файлы (*.*)|*.*";
		Длг.Расширение = "rcf";
	...
	Если НЕ ЗначениеВФайл(ИмяФайла, ДеревоЗапросов) Тогда
		Предупреждение(НСтр("ru='Невозможно сохранить файл';uk='Неможливо зберегти файл'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Попытка
		...
		Если Текст <> Неопределено И Текст <> "" Тогда
	        ...
	        Если Строка.ПараметрыЗапроса <> Неопределено Тогда
				Для каждого СтрокаПараметров Из Строка.ПараметрыЗапроса Цикл
					Если СтрокаПараметров.ЭтоВыражение = Истина Тогда
						Попытка
							Построитель.Параметры.Вставить(СтрокаПараметров.ИмяПараметра, Вычислить(СтрокаПараметров.ЗначениеПараметра));
						Исключение
							Предупреждение(НСтр("ru='Ошибка в выражении параметра ';uk='Помилка у виразу параметра '") + СтрокаПараметров.ИмяПараметра);
						КонецПопытки;
					Иначе
		...
		КонецЕсли;
	Исключение
		Предупреждение(НСтр("ru='Ошибка в тексте запроса:';uk='Помилка в тексті запиту:'") + "
			|" + ОписаниеОшибки());
	КонецПопытки;
...
Функция ВыполнитьЗапрос(Куда)
	...
	Если ПостроительОтчетов.Текст = "" ИЛИ ТекущаяСтрока = Неопределено ИЛИ 
		...
		Предупреждение(НСтр("ru='Не введен текст запроса';uk='Не введений текст запиту'"));
		Возврат Ложь;
	КонецЕсли;

	Если ПостроительОтчетов.ПолучитьЗапрос().Текст = "" Тогда
		Предупреждение(НСтр("ru='Необходимо выбрать как минимум одно поле';uk='Необхідно вибрати як мінімум одне поле'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если СтраницаДляВывода = 0 Тогда
		...
		// Заполним меню открытия уровня
		КнопкаУровней = ЭлементыФормы.КоманднаяПанельРасшифровки.Кнопки.Найти("Уровни");
		Если КнопкаУровней <> Неопределено Тогда
			ЭлементыФормы.КоманднаяПанельРасшифровки.Кнопки.Удалить(КнопкаУровней);
		КонецЕсли;
		
		Если ЭлементыФормы.РезультатТабДокСвод.КоличествоУровнейГруппировокСтрок() > 1 Тогда
			КнопкаУровни = ЭлементыФормы.КоманднаяПанельРасшифровки.Кнопки.Добавить("Уровни", ТипКнопкиКоманднойПанели.Подменю, НСтр("ru='Показать уровень';uk='Показати рівень'"), );
			
			Действие = Новый Действие("ОткрытьУровень");
			Для у = 1 По ЭлементыФормы.РезультатТабДокСвод.КоличествоУровнейГруппировокСтрок() Цикл
				ИмяКнопки = "К" + Строка(у);
				КнопкаУровни.Кнопки.Добавить(ИмяКнопки, ТипКнопкиКоманднойПанели.Действие, НСтр("ru='Уровень';uk='Рівень'") + Строка(у), Действие);
			КонецЦикла;
		КонецЕсли;
	...
	ИначеЕсли СтраницаДляВывода = 1 Тогда
		...
		Если ПостроительОтчетов.ИзмеренияСтроки.Количество() = 0 И 
			...
			Предупреждение(НСтр("ru='Для вывода отчета в диаграмму необходимо выбрать как минимум одно измерение';uk='Для виведення звіту в діаграму необхідно вибрати як мінімум один вимір'"));
		Иначе
...
Процедура ОтработатьРасшифровку(Расшифровка, ОтработыватьРасшифровкуПоУмолчанию)
	...
	Если ДоступныеИзмерения.Количество() > 0 Тогда
		...
		КонецЦикла;
	Иначе
		СписокВыбора.Добавить(1, НСтр("ru='Текущий отчет';uk='Поточний звіт'"));
	КонецЕсли;
	СписокВыбора.Добавить(2, НСтр("ru='Другой отчет...';uk='Інший звіт...'"));
	...
	Для Каждого ТекущийЭлементСтруктуры Из Расшифровка Цикл
        СписокВыбора.Добавить(Новый Структура("Значение", ТекущийЭлементСтруктуры.Значение),
							  НСтр("ru='Открыть:';uk='Відкрити:'") + ТекущийЭлементСтруктуры.Значение);
	КонецЦикла; 
    ...
    Если Выбор <> Неопределено Тогда
		...
		ИначеЕсли Выбор.Значение = 1 Тогда
			...
			Если ИмяДобавляемогоИзмерения <> "" Тогда
				ПостроительОтчетов.ИзмеренияСтроки.Добавить(ИмяДобавляемогоИзмерения, ИмяДобавляемогоИзмерения);
			КонецЕсли;
		...
		Иначе
			...
			ПостроительОтчетов.ИзмеренияСтроки.Добавить(Выбор, Выбор);
			ПостроительОтчетов.НастроитьРасшифровку(ПостроительОтчетов, Расшифровка);
			СохранитьПозициюВРезультате();
			ВыполнитьЗапрос(ВыводОтчетаПоНастройкам);
			ЗаписатьИсторию();
...
Процедура ОткрытьФайлОтчетов()
	...
	Если СохранитьЗапросыВФайл(Истина) Тогда
		...
		Длг.ПолноеИмяФайла = ИмяФайла;
		Длг.Каталог = ИмяПути;
		Длг.Заголовок = НСтр("ru='Выберите файл со списком отчетов';uk='Виберіть файл зі списком звітів'");
		Длг.Фильтр = НСтр("ru='Файлы отчетов (*.rcf)|*.rcf|Все файлы (*.*)|*.*';uk='Файли звітів (*.rcf)|*.rcf|Все файли (*.*)|*.*'");
		Длг.Расширение = "rcf";
...
Процедура СохранитьТекущиеНастройкиВСтроку()
	...
	Если Не ОтработанаРасшифровка И ТекущаяСтрока.СохранятьНастройкиАвтоматически = Истина И 
		ТипЗнч(ТекущаяСтрока.НастройкиПостроителя) = Тип("ТаблицаЗначений") Тогда
		Для Каждого Тн ИЗ ТекущаяСтрока.НастройкиПостроителя Цикл
...
Процедура КоманднаяПанельРеультатаСохранитьНастройкиОтчета(Кнопка)
	...
	Если ФормаСохраненияНастроек.Настройки.Количество() = 0 Тогда
		ФормаСохраненияНастроек.НаименованиеНастройки = НСтр("ru='Основная';uk='Основна'");
	КонецЕсли;
			...
			Для Каждого Тн ИЗ ТекущаяСтрока.НастройкиПостроителя Цикл
				...
				Если Найдено Тогда
					...
					Если ФормаСохраненияНастроек.НаименованиеНастройки = Тн.Имя Тогда
						...
						КонецЕсли;
					Иначе
						ДС.НастройкиПостроителя = Тн.НастройкиПостроителя;
...
Процедура ОткрытьУровень(Кнопка)
	...
	У = ЭлементыФормы.РезультатТабДокСвод.КоличествоУровнейГруппировокСтрок() - 1;
	Пока у > Уровень Цикл
		ЭлементыФормы.РезультатТабДокСвод.ПоказатьУровеньГруппировокСтрок(у);
		у = у - 1;
	КонецЦикла;

	ЭлементыФормы.РезультатТабДокСвод.ПоказатьУровеньГруппировокСтрок(Уровень);
КонецПроцедуры

Процедура КоманднаяПанельГруппировокГруппировкиПоУмолчанию(Кнопка)
	Перем Настройки;
...
Процедура КоманднаяПанельПорядкаПоУмолчанию(Кнопка)
	Перем Настройки;
...
Процедура КоманднаяПанельПолейПоУмолчанию(Кнопка)
	Перем Настройки;
...
Процедура ДеревоЗапросовПередНачаломИзменения(Элемент, Отказ)
	...
	ФормаРедактирования = ПолучитьФорму("ФормаРедактированияЗапроса", ЭтаФорма);
	...
	ФормаРедактирования.ПредставленияДляИмен = Элемент.ТекущаяСтрока.ПредставленияДляИмен;
	ФормаРедактирования.ФорматыДляИмен = Элемент.ТекущаяСтрока.ФорматыДляИмен;
	СкопироватьТаблицуПараметров(Элемент.ТекущаяСтрока.ПараметрыЗапроса, ФормаРедактирования.Параметры);
	...
	Если ФормаРедактирования.ОткрытьМодально() = Истина Тогда
		...
		Элемент.ТекущаяСтрока.ПараметрыЗапроса = НоваяТаблицаПараметров();
		Элемент.ТекущаяСтрока.ПредставленияДляИмен = ФормаРедактирования.ПредставленияДляИмен;
		Элемент.ТекущаяСтрока.ФорматыДляИмен = ФормаРедактирования.ФорматыДляИмен;
		...
		СкопироватьТаблицуПараметров(ФормаРедактирования.Параметры, Элемент.ТекущаяСтрока.ПараметрыЗапроса);
...
Процедура ДеревоЗапросовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель)
	ФормаРедактирования = ПолучитьФорму("ФормаРедактированияЗапроса");
	...
	Если ФормаРедактирования.ОткрытьМодально() = Истина Тогда
		...
		НоваяСтрока.ПредставленияДляИмен = ФормаРедактирования.ПредставленияДляИмен;
		НоваяСтрока.ФорматыДляИмен = ФормаРедактирования.ФорматыДляИмен;
		...
		Если Копирование Тогда
			НоваяСтрока.ВыбТипДиаграммы = Элемент.ТекущаяСтрока.ВыбТипДиаграммы;
			НоваяСтрока.ВыбТипСводДиаграммы = Элемент.ТекущаяСтрока.ВыбТипСводДиаграммы;
			НоваяСтрока.НастройкиПостроителя = Элемент.ТекущаяСтрока.НастройкиПостроителя;
			НоваяСтрока.РазмещениеГруппировок = Элемент.ТекущаяСтрока.РазмещениеГруппировок;
			НоваяСтрока.РазмещениеРеквизитов =  Элемент.ТекущаяСтрока.РазмещениеРеквизитов;
			НоваяСтрока.РазмещениеИтогов = Элемент.ТекущаяСтрока.РазмещениеИтогов;
			НоваяСтрока.ТипОформления = Элемент.ТекущаяСтрока.ТипОформления;
			НоваяСтрока.ИспользоватьМакет = Элемент.ТекущаяСтрока.ИспользоватьМакет;
			НоваяСтрока.ФиксированныйЗаголовок = Элемент.ТекущаяСтрока.ФиксированныйЗаголовок;
			НоваяСтрока.МакетСОформлением = Элемент.ТекущаяСтрока.МакетСОформлением;
			НоваяСтрока.Макет = Элемент.ТекущаяСтрока.Макет;
		Иначе
		...
		НоваяСтрока.ПараметрыЗапроса = НоваяТаблицаПараметров();
		СкопироватьТаблицуПараметров(ФормаРедактирования.Параметры, НоваяСтрока.ПараметрыЗапроса);
...
Процедура ПриОткрытии()
	...
	ЭлементыФормы.ВыбТипСводДиаграммы.СписокВыбора.Добавить(ТипСводнойДиаграммы.ГистограммаОбъемная, 
		НСтр("ru='Гистограмма объемная';uk=""Гістограма об'ємна"""));
