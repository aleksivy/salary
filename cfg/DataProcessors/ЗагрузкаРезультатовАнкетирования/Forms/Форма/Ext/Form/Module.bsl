...
Процедура ЗаполнитьТабличноеПолеВходящие()
	...
	Иначе
		...
		ЗапросПоЭлПисьмам.УстановитьПараметр("УчетнаяЗапись",	УчетнаяЗапись);
		ЗапросПоЭлПисьмам.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	И ВложенияЭлектронныхПисем.Объект.СтатусПисьма = &Полученное
		|	И ВложенияЭлектронныхПисем.Объект.Ссылка ССЫЛКА Документ.ЭлектронноеПисьмо
		|	И (ВложенияЭлектронныхПисем.ИмяФайла ПОДОБНО ""%.rps""
		|			ИЛИ ВложенияЭлектронныхПисем.ИмяФайла ПОДОБНО ""%.xml"")" +
		?(ТолькоНепрочитанные, "
		|	И
		|	ВложенияЭлектронныхПисем.Объект.НеРассмотрено = ИСТИНА", "")+?(ПоУчетнойЗаписи,"
		|	И
		|	ВложенияЭлектронныхПисем.Объект.УчетнаяЗапись = &УчетнаяЗапись","");
...
Процедура ЗаполнитьДеревоПоФайлам()
	...
	Если ДеревоАнкет.Строки.Количество() > 0 Тогда
		Вопрос = НСтр("ru='Существующее дерево файлов будет очищено. Продолжить?';uk='Існуюче дерево файлів буде очищено. Продовжити?'");
...
Процедура ОбновитьОтображениеXMLФайлаИзКаталога()
	...
	Если ЭлементыФормы.КоманднаяОтветов.Кнопки.HTML.Пометка тогда
		...
		Если ЭлементыФормы.Дерево.ТекущаяСтрока <> Неопределено И 
			...
			ПолноеИмя	= Каталог + ?(Прав(Каталог,1)<>"\","\","") + ЭлементыФормы.Дерево.ТекущаяСтрока.ИмяФайла;
			...
			ФайлXSL = Новый ТекстовыйДокумент();
			ФайлXSL.УстановитьТекст(СтрокаПреобразования);
			ФайлXSL.Записать(Каталог + ?(Прав(Каталог,1)<>"\","\","") + "style.xsl");
	...
	Иначе
		...
		Если ЭлементыФормы.Дерево.ТекущаяСтрока <> Неопределено Тогда
			...
			Каталог  = ЭлементыФормы.Дерево.ТекущаяСтрока.Родитель.ИмяФайла;
			Файл     = Новый Файл(Каталог + ?(Прав(Каталог,1)<>"\","\","") + ИмяФайла);
...
Процедура ИмяКаталогаНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ДиалогФыбораФайла.Заголовок               = НСтр("ru='Выберите каталог с файлами опросов';uk='Виберіть каталог з файлами опитувань'");
...
Процедура ПредПросмотрonclick(Элемент, pEvtObj)
	...
	УправлениеЭлектроннойПочтой.ОбработкаСобытияOnClickПоляHTML(Элемент, pEvtObj,,, ЭтаФорма);
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЧНОГО ПОЛЯ Входящие
