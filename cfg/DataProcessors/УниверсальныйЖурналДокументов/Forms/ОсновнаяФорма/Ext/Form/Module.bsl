...
Функция мОписаниеОтбора(Отбор)
	...
	Если ПустаяСтрока(ОписаниеОтбора) Тогда
		Возврат НСтр("ru='Отбор не установлен';uk='Відбір не встановлений'");
	Иначе
		Возврат НСтр("ru='Установлен отбор: ';uk='Встановлений відбір: '") + ОписаниеОтбора;
	КонецЕсли;
...
Процедура ОбновитьДанныеПоСсылке(СсылкаДокумента)
	...
	Иначе
		...
		КонецЕсли;
		Предупреждение(НСтр("ru='Данные не соответствуют текущему отбору списка универсального журнала.';uk='Дані не відповідають поточному відбору списку універсального журналу.'"));
...
Процедура СоздатьНовыйДокумент()
	...
	Иначе
		...
		ТипДока=Спс.ВыбратьЭлемент(НСтр("ru='Выбор вида документа';uk='Вибір виду документа'"), ТекущийДокумент);
	КонецЕсли;
...
Процедура ИзменитьДокумент()
	...
	Если ТекущиеДанныеСтроки<>Неопределено Тогда
		...
		ТекДок.ПолучитьФорму(, ЭлементыФормы.СписокДокументов).Открыть();
	КонецЕсли;
...
Процедура УстановитьПометкуУдаления()
	...
	Если ТекущиеДанныеСтроки<>Неопределено Тогда
		...
		Если ТекДок.ПометкаУдаления Тогда
			Если Вопрос(НСтр("ru='Снять с объекта пометку на удаление?';uk=""Зняти з об'єкта позначку на вилучення?"""), РежимДиалогаВопрос.ДаНет)=КодВозвратаДиалога.Да Тогда
				Попытка
			...
			КонецЕсли;
		Иначе
			Если Вопрос(НСтр("ru='Пометить объект на удаление?';uk=""Позначити об'єкт для вилучення?"""), РежимДиалогаВопрос.ДаНет)=КодВозвратаДиалога.Да Тогда
				Попытка
...
Процедура СписокДокументовПередНачаломДобавления(Элемент, Отказ, Копирование)
	...
	Иначе
		...
		Иначе
			Предупреждение(НСтр("ru='Не определен состав документов журнала!';uk='Не визначений склад документів журналу!'"));
		КонецЕсли;
...
Процедура ВвестиДокументПоОснованию(Кнопка) // Подключение данной процедуры-обработчика выполняется из кода конфигурации
	...
	ДокОсн = ЭлементыФормы.СписокДокументов.ТекущиеДанные.Ссылка;
