...
Процедура КоманднаяПанельСписокГрафЗаполнитьАвтоматическиВсеОбщиеГрафы(Кнопка)
	...
	Если СписокГраф.Количество() и Вопрос(НСтр("ru='Существующие графы будут очищены. Продолжить?';uk='Існуючі графи будуть очищені. Продовжити?'"), РежимДиалогаВопрос.ОКОтмена) = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ОтметитьВыбранныйРеквизит()
	...
	Если ТекущиеДанныеСпискаГраф = Неопределено Тогда
		Предупреждение(НСтр("ru='Не выбрана графа!';uk='Не обрана графа!'"));
		Возврат;
	ИначеЕсли ТекущиеДанные = Неопределено или ТекущиеДанные.Уровень() = 0 Тогда
		Предупреждение(НСтр("ru='Не выбран реквизит!';uk='Не обраний реквізит!'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого ЭлементСписка Из СписокОбрабатываемыхДокументов Цикл
		...
		Если Не СтрокаЗначенияСпискаГраф = Неопределено и Не СтрокаЗначенияСпискаГраф.Представление = ИмяРеквизита Тогда
			РезультатВопроса = Вопрос(НСтр("ru='Существуют документы, для которых в составе графы отмечен реквизит с отличным именем."
"Изменять для таких документов состав графы?';uk=""Існують документи, для яких у складі графи відзначений реквізит з іншим ім'ям."
"Змінювати для таких документів склад графи?"""), РежимДиалогаВопрос.ДаНетОтмена);
