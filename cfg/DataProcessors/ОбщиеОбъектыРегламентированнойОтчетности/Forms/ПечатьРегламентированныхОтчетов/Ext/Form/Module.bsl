...
Процедура ОсвежитьВыбранныйЛист()
	...
	ЭлементыФормы.ИтоговаяТаблица.Вывести(ЭлементыФормы.СписокЛистов.ТекущиеДанные.ТабличныйДокумент);
	ЭлементыФормы.ИтоговаяТаблица.ОриентацияСтраницы = ЭлементыФормы.СписокЛистов.ТекущиеДанные.ТабличныйДокумент.ОриентацияСтраницы;
	ЭлементыФормы.ИтоговаяТаблица.АвтоМасштаб = ЭлементыФормы.СписокЛистов.ТекущиеДанные.ТабличныйДокумент.АвтоМасштаб;
	Если ЭлементыФормы.СписокЛистов.ТекущиеДанные.Уровень() = 1 Тогда
		ЭлементыФормы.ИтоговаяТаблица.Вывести(ЭлементыФормы.СписокЛистов.ТекущиеДанные.ТабличныйДокумент);
		ЭлементыФормы.ИтоговаяТаблица.ОриентацияСтраницы = ЭлементыФормы.СписокЛистов.ТекущиеДанные.ТабличныйДокумент.ОриентацияСтраницы;
		ЭлементыФормы.ИтоговаяТаблица.АвтоМасштаб = ЭлементыФормы.СписокЛистов.ТекущиеДанные.ТабличныйДокумент.АвтоМасштаб;
		Если НЕ ЭлементыФормы.ИтоговаяТаблица.АвтоМасштаб Тогда
			ЭлементыФормы.ИтоговаяТаблица.МасштабПечати = ЭлементыФормы.СписокЛистов.ТекущиеДанные.ТабличныйДокумент.МасштабПечати;
		КонецЕсли;
...
Процедура СформироватьТекстПодскзаки()
	...
	ТекстСтатистики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выбрано %1 из %2 %3. Для печати требуется %4 %5 бумаги.';uk='Обрано %1 з %2 %3. Для друку потрібно %4 %5 паперу.'"), КоличествоТаблДок, СписокЛистов.Количество(), ТекстЛистовВсего, КоличествоЛистовТребуется, ТекстЛистовТребуется);
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если СписокПечатаемыхЛистов.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Не выбраны листы для вывода на печать!';uk='Не обрані аркуші для виведення на друк!'"));
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПриОткрытии()
	...
	Заголовок = ВладелецФормы.Заголовок + НСтр("ru=' - предварительный просмотр';uk=' - попередній перегляд'");
...
// Процедура сохраняет выбранные из числа сформированных табличных документов
// в файлы в указанный пользователем каталог.
//
Процедура ОсновныеДействияФормыЗаписать(Кнопка)
	...
	Если КоличествоВыбранныхТабличныхДокументов() = 0 Тогда
		Предупреждение(НСтр("ru='Выберите листы в дереве печатаемых листов!';uk='Оберsnm листи у дереві друкуємих листів!'"));
		Возврат;
	КонецЕсли;
	...
	Для Каждого Эл Из СписокЛистов Цикл
		Если Эл.Пом Тогда
			ИмяФайла = ПутьДляВыгрузки + Эл.Наименование + ".mxl";
			Попытка
				состояние(НСтр("ru='Сохранение ';uk='Збереження '") + ИмяФайла);
				Эл.ТабличныйДокумент.Записать(ИмяФайла, ТипФайлаТабличногоДокумента.MXL);
			Исключение
	...
	Если ПризнакОшибки Тогда
		Предупреждение(НСтр("ru='Один или несколько из выбранных файлов не удалось записать в указанный каталог!';uk='Один або декілька з обраних файлів не вдалося записати до вказаного каталогу!'"), НСтр("ru='Ошибка записи на диск!';uk='Помилка запису на диск!'"));
	Иначе
		Если Вопрос(НСтр("ru='Выбранные файлы успешно записаны на диск. Открыть каталог?';uk='Обрані файли успішно записані на диск. Відкрити каталог?'"), РежимДиалогаВопрос.ДаНет,,, НСтр("ru='Файлы успешно записаны';uk='Файли успішно записані'")) = КодВозвратаДиалога.Да Тогда
			ЗапуститьПриложение(ПутьДляВыгрузки);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОсновныеДействияФормыАктивизироватьРодительскийОтчет(Кнопка)
	...
	Исключение
		Предупреждение(НСтр("ru='Не удалось активизировать родительский отчет! Вероятно, форма отчета закрыта.';uk='Не вдалося активізувати батьківський звіт! Імовірно, форма звіту закрита.'"));
	КонецПопытки;
...
Процедура ИтоговаяТаблицаВыбор(Элемент, Область, СтандартнаяОбработка)
