...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если мСпрашиватьОСохранении <> Ложь И Модифицированность Тогда
		Ответ = Вопрос(НСтр("ru='Данные на форме были изменены. Сохранить изменения?';uk='Дані на формі були змінені. Зберегти зміни?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура ТаблицаДопСтраницПриИзмененииФлажка(Элемент, Колонка)
	...
	Для Каждого Стр Из мТаблицаСтраницРаздела Цикл
		...
		ТекущееПредставлениеСтраницы = ?(РегламентированнаяОтчетность.ПустоеЗначение(ТекущееПредставлениеСтраницы), НСтр("ru='Лист N ';uk='Аркуш N '") + Строка(НомерСтраницы), ТекущееПредставлениеСтраницы + НСтр("ru='. Лист N ';uk='. Аркуш N '") + Строка(НомерСтраницы));
		Если Не ПустаяСтрока(ТекущееПредставлениеСтраницы) Тогда
			Стр.Представление = ТекущееПредставлениеСтраницы;
		Иначе
			Стр.Представление = НСтр("ru='Новая страница';uk='Нова сторінка'");
		КонецЕсли;
