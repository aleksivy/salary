...
Функция ПолучитьДистрибутив(Сервер, Адрес, Ресурс, Каталог)
	...
	Исключение
		Сообщить(НСтр("ru='Каталог для получения комплекта ""';uk='Каталог для отримання комплекту ""'")+Каталог+НСтр("ru='"" занят. Получение невозможно.';uk='"" зайнятий. Отримання неможливо.'"));
		Возврат "";
	КонецПопытки;
...
Процедура Распаковать(Каталог, Архив)
	...
	Исключение
		Сообщить(НСтр("ru='Неудачная попытка удалить файл ';uk='Невдала спроба видалити файл '")+Каталог+Архив);
	КонецПопытки;
КонецПроцедуры
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	Если ПустаяСтрока(СтартоваяСтраница) Тогда
		СтартоваяСтраница = СерверИсточник+"/version_files?nick="+АдресРесурсовОбозревателя+"&ver="+Метаданные.Версия;
	КонецЕсли;
	Попытка
	...
	Если pEvtObj.shiftKey Тогда
		...
		Если Параметр<>Неопределено Тогда
			...
			Если Не Файл.Существует() Тогда
				Ответ = Вопрос(НСтр("ru='Каталог ';uk='Каталог '")+КаталогДополнительнойИнформации +"\"+ ОтносительныйКаталог+НСтр("ru=' не существует. Создать?';uk=' не існує. Створити?'"),РежимДиалогаВопрос.ДаНет,КодВозвратаДиалога.Да);
				...
				КонецЕсли;
			КонецЕсли;
			ЗапуститьПриложение(КаталогДополнительнойИнформации +"\"+ ОтносительныйКаталог);
		Иначе
	...
	Иначе
		...
		Если Параметр<>Неопределено Тогда
			Если Параметр.Представление="UPD" Тогда
				...
				Если Параметр<>Неопределено Тогда
					// проверка соответствия текущей версии конфигурации выложенному компклету обновления
					...
					Иначе
						// комплект не годится
						Предупреждение(НСтр("ru='Указанный комплект предназначен для обновления с версий:  ';uk='Вказаний комплект призначений для оновлення з версій:  '")+СтрЗаменить(Параметр.Представление,";",", ")+Символы.ПС+НСтр("ru='Текущая версия ';uk='Поточна версія '")+Метаданные.Версия);
					КонецЕсли;
				Иначе
					Сообщить(НСтр("ru='Отсутствует информация о версиях, для которых предназначен комплект обновления.';uk='Відсутня інформація про версії, для яких призначений комплект оновлення.'"));
				КонецЕсли;
				pEvtObj.returnValue = Ложь;
			ИначеЕсли Параметр.Представление="ADD" Тогда
				...
				СоставСсылки(Ссылка, Сервер, Адрес, Ресурс);
				Добавить = Истина;
				Если Найти(Ссылка,"downloads.1c.eu")>0 Тогда
					Параметр = СписокПараметров.НайтиПоЗначению("ID");
