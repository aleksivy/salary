...
Процедура КоманднаяПанельТабличногоДокументаПредыдущееПримечание(Кнопка)
	...
	Иначе
		Предупреждение(НСтр("ru='Достигнуто начало документа';uk='Досягнуто початок документа'"));
	КонецЕсли;
...
Процедура КоманднаяПанельТабличногоДокументаСледующееПримечание(Кнопка)
	...
	Если Нашли Тогда
		ТабличныйДокумент.ТекущаяОбласть = Область;
	Иначе
		Предупреждение(НСтр("ru='Достигнут конец документа';uk='Досягнуто кінець документа'"));
	КонецЕсли;
...
Процедура КоманднаяПанельТабличногоДокументаСохранить(Кнопка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ru='Созранить табличный документ в файл';uk='Зберегти табличний документ у файл'");
...
Процедура КоманднаяПанельТабличногоДокументаОткрыть(Кнопка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ru='Прочитать табличный документ из файла';uk='Прочитати табличний документ з файлу'");
...
Процедура КоманднаяПанельФормыСохранитьЗначенияВФайл(Кнопка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ru='Созранить значения настройки в файл';uk='Зберегти значення настройки в файл'");
...
Процедура КоманднаяПанельФормыВосстановитьЗначенияИзФайла(Кнопка)
	...
	ДиалогВыбораФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогВыбораФайла.Заголовок                   = НСтр("ru='Восстановить значения из файла';uk='Відновити значення з файлу'");
	ДиалогВыбораФайла.Фильтр    = "Настройка загрузки в табличный документ (*.mxlz)|*.mxlz|Все файлы (*.*)|*.*|";
...
Процедура ТаблицаЗагружаемыхРеквизитовРежимЗагрузкиПриИзменении(Элемент)
	...
	Если ТекущиеДанные.НомерКолонки = 0 И ТекущиеДанные.РежимЗагрузки = "Искать" И РучнаяНумерацияКолонокТабличногоДокумента Тогда
		Предупреждение(НСтр("ru='Не выбран номер колонки табличного документа';uk='Не обраний номер колонки табличного документа'"));
		ТекущиеДанные.РежимЗагрузки = "Устанавливать"
	КонецЕсли;
