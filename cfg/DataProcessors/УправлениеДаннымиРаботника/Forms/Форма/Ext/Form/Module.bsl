...
Процедура УстановкиПриОткрытии()
	...
	КонецЕсли;
// Серна - Начало
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.АвансыРаботникамОрганизаций) Тогда
		ЭлементыФормы.КоманднаяПанельОсновная.Кнопки.ПлановыеДанные.Доступность		= Ложь;
	КонецЕсли;
	Если Не ПравоДоступа("Чтение", Метаданные.Документы.НачислениеЗарплатыРаботникамОрганизаций) Тогда
		ЭлементыФормы.КоманднаяПанельОсновная.Кнопки.РасчетыЗарплата.Доступность		= Ложь;
	КонецЕсли;
// Серна - Конец
...
// Серна - Начало
	Если ПравоДоступа("Чтение", Метаданные.РегистрыСведений.АвансыРаботникамОрганизаций) Тогда
// Серна - Конец
	Выборка = Запрос.Выполнить().Выбрать();
	ВыходнойДокумент.Вывести(ПустаяСтрока);
