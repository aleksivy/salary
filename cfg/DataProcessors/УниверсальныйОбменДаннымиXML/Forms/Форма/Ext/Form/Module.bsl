...
Процедура ВыборФайла(Элемент, ПроверятьСуществование=Ложь, Знач РасширениеПоУмолчанию = "xml", 
	...
	Если РасширениеПоУмолчанию = "txt" Тогда
		...
		ДиалогВыбораФайла.Фильтр = НСтр("ru='Файл протокола обмена (*.txt)|*.txt';uk='Файл протоколу обміну (*.txt)|*.txt'");
	...
	ИначеЕсли РежимОбмена = "Выгрузка" Тогда
		...
		Если АрхивироватьФайлДанных Тогда
			...
			ДиалогВыбораФайла.Фильтр = НСтр("ru='Архивный файл данных (*.zip)|*.zip';uk='Архівний файл даних (*.zіp)|*.zіp'");
			ДиалогВыбораФайла.Расширение = "zip";
			
		ИначеЕсли ВыборФайлаПравил Тогда
			
			ДиалогВыбораФайла.Фильтр = НСтр("ru='Файл данных (*.xml)|*.xml|Архивный файл данных (*.zip)|*.zip';uk='Файл даних (*.xml)|*.xml|Архівний файл даних (*.zіp)|*.zіp'");
...
Процедура ПриОткрытии()
	...
	ЗаголовокФормы = НСтр("ru='Универсальный обмен данными в формате XML (%Версия%)';uk='Універсальний обмін даними у форматі XML (%Версия%)'");
	...
	Если Не ПустаяСтрока(ИмяФайлаОбмена)
		...
		Если НеЗадаватьВопросПриЗагрузкеДанныхПриОткрытииОбработки = 1 Тогда
			...
			ТекстВопроса = НСтр("ru='Выполнить загрузку данных из файла обмена?';uk='Виконати завантаження даних з файлу обміну?'");
...
Процедура ГлавнаяПанельФормыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = 0 Тогда
		...
		КнопкаВыполнить.Текст = НСтр("ru='Выгрузить данные';uk='Вивантажити дані'");
		
	ИначеЕсли ТекущаяСтраница = 1 Тогда
		
		РежимОбмена = "Загрузка";
		КнопкаВыполнить.Текст = НСтр("ru='Загрузить данные';uk='Завантажити дані'");
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если НЕ НепосредственноеЧтениеВИБПриемнике Тогда
		...
		Если Врег(СокрЛП(ИмяФайлаПравилОбмена)) = Врег(СокрЛП(ИмяФайлаОбмена)) Тогда
			...
			ТекстПредупреждения = НСтр("ru='Файл правил обмена не может совпадать с файлом данных. Выберите другой файл для выгрузки данных.';uk='Файл правил обміну не може збігатися з файлом даних. Виберіть інший файл для вивантаження даних.'");
...
Процедура УдалитьНажатие(Элемент)
	...
	ТекстВопроса = НСтр("ru='Удалить выбранные данные в информационной базе?';uk='Вилучити обрані дані в інформаційній базі?'");
...
Процедура ИмяФайлаПравилПриИзменении(Элемент)
	...
	ТекстВопроса = НСтр("ru='Загрузить правила обмена данными?';uk='Завантажити правила обміну даними?'");
	ОтветПользователя = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, 15, КодВозвратаДиалога.Да);
...
Процедура ТестПодключенияНажатие(Элемент)
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru='Подключение к информационной базе приемнику произошло успешно.';uk='Підключення до інформаційної бази приймачу відбулося успішно.'");
		Предупреждение(ТекстПредупреждения);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ru='Выбор каталога информационной базы';uk='Вибір каталогу інформаційної бази'");
...
Процедура ОбновитьПравилаНажатие(Элемент)
	...
	Если ПустаяСтрока(ИмяФайлаПравилОбмена) Тогда
		...
		ТекстПредупреждения = НСтр("ru='Выберите файл правил обмена.';uk='Виберіть файл правил обміну.'");
		Предупреждение(ТекстПредупреждения);
		Возврат;
