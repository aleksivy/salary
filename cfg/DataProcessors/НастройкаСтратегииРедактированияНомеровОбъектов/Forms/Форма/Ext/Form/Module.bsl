	...
	Для Каждого ОформлениеСтроки ИЗ ОформленияСтрок Цикл
		...
		Иначе
			...
			Если ОформлениеСтроки.Ячейки.СтратегияРедактирования.Значение = Перечисления.СтратегияРедактированияНомеровОбъектов.ПустаяСсылка() Тогда
				ОформлениеСтроки.Ячейки.СтратегияРедактирования.УстановитьТекст(НСтр("ru='Не поддерживается';uk='Не підтримується'"));
				ОформлениеСтроки.Ячейки.СтратегияРедактирования.ТолькоПросмотр = Истина;
			КонецЕсли;
...
Процедура ЗаполнениеДереваОбъекта(ЗаполнятьТолькоПоМетаданным = Ложь)
	...
	ФормаИндикации.КомментарийОбработкиДанных = НСтр("ru='Начальное заполнение дерева объектов метаданных';uk=""Початкове заповнення дерева об'єктів метаданих""");	
	...
	Если Метаданные.Справочники.Количество() > 0 Тогда		
		...
		ФормаИндикации.КомментарийЗначения = НСтр("ru='Анализ форм справочников';uk='Аналіз форм довідників'");
	...
	Если Метаданные.Документы.Количество() > 0 Тогда
		...
		ФормаИндикации.КомментарийЗначения = НСтр("ru='Анализ форм документов';uk='Аналіз форм документів'");
	...
	Если Метаданные.ПланыВидовХарактеристик.Количество() > 0 Тогда
		...
		ФормаИндикации.КомментарийЗначения = НСтр("ru='Анализ форм планов видов характеристик';uk='Аналіз форм планів видів характеристик'");
...
Процедура Записать()
	...
	Если ОбщегоНазначения.ЕстьДругиеПользователиВБазе() Тогда
		Предупреждение(НСтр("ru='Для остальных пользователей новая стратегия доступности редактирования';uk='Для інших користувачів нова стратегія доступності редагування'") + Символы.ПС + НСтр("ru='номеров (кодов) объектов вступит в силу после перезапуска их сеансов работы с программой.';uk=""номерів (кодів) об'єктів набуде чинності після перезапуску їхніх сеансів роботи із програмою."""));
	КонецЕсли;
...
Процедура КоманднаяПанельДереваОбъектовЗаполнитьПоУмолчанию(Кнопка)
	Ответ = Вопрос(НСтр("ru='Для всех объектов метаданных стратегия доступности редактирования номера будет установлена';uk=""Для всіх об'єктів метаданих стратегія доступності редагування номеру буде встановлена""") + Символы.ПС + НСтр("ru='в значение по умолчанию. Все введенные ранее значения будут потеряны. Продолжить?';uk='у значення по умовчанню. Всі уведені раніше значення будуть загублені. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
