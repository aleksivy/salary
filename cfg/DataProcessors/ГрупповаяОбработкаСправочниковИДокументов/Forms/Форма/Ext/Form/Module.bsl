...
Процедура ЗаполнитьСписокДействий() Экспорт
	...
	ИначеЕсли мСформированныйРежим.ИмяВидаОдногоТипа = "Номенклатура" Тогда
		...
		// Конфигурация ЗУП не поддерживает печать ценников
		//МассивТипов = Новый Массив;
		//МассивТипов.Добавить(ТипЗНЧ(Справочники.ТипыЦенНоменклатуры.ПустаяСсылка()));
		//СписокВыбора.Добавить(Новый Структура("Имя,ЭтоВыбор,СписокВыбора","П_ПечатьЦенников","ПечатьЦенников",Новый ОписаниеТипов(МассивТипов)),"Печать ценников");
		
		Если Не Метаданные.Обработки.Найти("ПечатьЭтикеток") = Неопределено Тогда
...
// Функция возвращает печатную форму по умолчанию для документа,
// дублирует аналогичную функцию типовх конфигураций, добавлена для возможности использования
// в любой конфигурации
Функция _ПолучитьПечатнуюФормуПоУмолчаниюЛокально(ИмяОбъекта, СоответствиеМакетов) Экспорт
	
	Перем НазваниеМакета;	
	НазваниеМакета = ВосстановитьЗначение(ИмяОбъекта + "МакетПечати");	
	ИмяМакета = СоответствиеМакетов.НайтиПоЗначению(НазваниеМакета);
	Если ИмяМакета = Неопределено Тогда		
		Для каждого Элемент Из СоответствиеМакетов Цикл
			НазваниеМакета = Элемент.Значение;
			Прервать;
		КонецЦикла; 
	КонецЕсли;
	Возврат НазваниеМакета;
	
КонецФункции // _ПолучитьПечатнуюФормуПоУмолчаниюЛокально()

// Процедура печати документов
Процедура ПечатьДокументов(МассивДокументов,НаПринтер)
	Если НаПринтер Тогда
		
		Если Вопрос(НСтр("ru='При печати документов без предварительного просмотра будут использованы настройки, "
"используемые при последней печати. Продолжить?';uk='При друку документів без попереднього перегляду будуть використані настройки, "
"використані під час останнього друку. Продовжити?'"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Нет Тогда
			Возврат;
		КонецЕсли;
...
Функция ОчищатьРезультаты()
	Если Не мСформированныйРежим = Неопределено И мСформированныйРежим.ДанныеОтобраны Тогда
		Если Вопрос(НСтр("ru='Результат отбора будет очищен. Продолжить?';uk='Результат відбору буде очищений. Продовжити?'"), РежимДиалогаВопрос.ОКОтмена) = КодВозвратаДиалога.Отмена  Тогда
			Возврат Ложь;
		КонецЕсли; 
	КонецЕсли; 
	ОчиститьРезультаты();
	...
	Для каждого Строка Из ТабличноеПолеВидыОбъектов Цикл
		...
		Для каждого КлючИЗначение Из СтруктураРеквизитовШапки Цикл
			...
			Если Не МетаданноеРеквизита = Неопределено И МетаданноеРеквизита.Тип.СодержитТип(Тип("Строка")) и МетаданноеРеквизита.Тип.КвалификаторыСтроки.Длина = 0 Тогда
				ТекстЗапроса = ТекстЗапроса + "," + Символы.ПС + "	ПОДСТРОКА("+ПсевдонимТаблицы+"."+Префикс+КлючИЗначение.Ключ+",1,"+ОграничениеНаСтрокиНеограниченнойДлинны+")";
			Иначе
		...
		Если ОбрабатыватьТабличныеЧасти Тогда
			...
			Для каждого КлючИЗначение Из СтруктураРеквизитовТЧ Цикл
				...
				Если Не МетаданноеРеквизита = Неопределено И МетаданноеРеквизита.Тип.СодержитТип(Тип("Строка")) и МетаданноеРеквизита.Тип.КвалификаторыСтроки.Длина = 0 Тогда
					...
					ТекстЗапроса = ТекстЗапроса + "," + Символы.ПС + "	ПОДСТРОКА("+ПсевдонимТаблицы+"."+КлючИЗначение.Ключ+",1,"+ОграничениеНаСтрокиНеограниченнойДлинны+")";
...
Процедура ОсновныеДействияФормыВыполнитьОтбор(Кнопка)
	...
	КонецЕсли; 
	Если мСформированныйРежим = Неопределено Тогда
		Предупреждение(НСтр("ru='Не выбрано ни одной таблицы';uk='Не обрано жодної таблиці'"));
		Возврат;
	КонецЕсли; 
...
Процедура СписокДействийОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если  Не ВыбранноеЗначение =  Неопределено И ВыбранноеЗначение.Свойство("ЭтоВыбор",ТипВыбора) Тогда
		...
		ИначеЕсли ТипВыбора = "Свойство" Тогда
			...
			Результат = СписокЗначенийВыбора.ВыбратьЭлемент(НСтр("ru='Выбор свойства';uk='Вибір властивості'"));
			Если Не Результат = Неопределено Тогда
		...
		ИначеЕсли ТипВыбора = "Категория" Тогда
			...
			Результат = СписокЗначенийВыбора.ВыбратьЭлемент(НСтр("ru='Выбор категории';uk='Вибір категорії'"));
			Если Не Результат = Неопределено Тогда
...
Процедура ВыполнитьНажатие(Элемент)
	...
	Если СписокДействий = Неопределено Тогда
		Предупреждение(НСтр("ru='Не выбрано действие для выполнения';uk='Не обрана дія для виконання'")); 
		Возврат;
	КонецЕсли; 
	
	
	Ответ = Вопрос(НСтр("ru='Выполнить ""';uk='Виконати ""'") + СписокДействий.ТекущееПредставление+ """?", РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
	...
	Для Каждого СтрокаДанных ИЗ ТаблицаДанных Цикл
		...
		ИначеЕсли ЭтоДействиеСШапкой И ПрошлаяПометка <> СтрокаДанных.Пометка Тогда
			...
			Предупреждение(НСтр("ru='Найдены противоречивые отметки';uk='Знайдено суперечливі позначки'"));
			Возврат;
	...
	ОчиститьСообщения();
	Сообщить(НСтр("ru='Выполняемое действие: ';uk='Дія, що виконується: '") + СписокДействий.ТекущееПредставление, СтатусСообщения.Информация);
	Если Не Лев(СписокДействий.Имя,1) = "П" Тогда
		Сообщить(НСтр("ru='Новое значение: ';uk='Нове значення: '") + ПолеВводаДействия, СтатусСообщения.Обычное);
	ИначеЕсли Не ПолеВводаДействия = Неопределено Тогда
		Сообщить(НСтр("ru='Режим: ';uk='Режим: '") + ПолеВводаДействия, СтатусСообщения.Обычное);
	КонецЕсли;
	Сообщить("-----------------------------------------------------------", СтатусСообщения.БезСтатуса);
	...
	~ОткатТранзакции:
	ОтменитьТранзакцию();
	Сообщить(НСтр("ru='Отмена транзакции. Действие не выполнено!';uk='Скасування транзакції. Дія не виконана!'"), СтатусСообщения.Важное);
