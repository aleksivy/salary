...
Функция Автозаполнение() Экспорт
	...
	Для Каждого СтрокаТаблицы Из Неявки Цикл
		...
		КонецЕсли;	
// Серна - Начало
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ВидОтпуска) Тогда Продолжить; КонецЕсли;
// Серна - Конец
...
Процедура РассчитатьДокументы() Экспорт
	...
	Для Каждого СтрокаТаблицы Из Неявки Цикл
		...
		Если СтрокаТаблицы.Отметка И ЗначениеЗаполнено(СтрокаТаблицы.РасчетныйДокумент) Тогда
			...
			КонецЕсли;
// Серна - Начало
			Если Не РасчетныйДокумент.Метаданные().ТабличныеЧасти.Найти("Начисления")=Неопределено Тогда
				Если (РасчетныйДокумент.Начисления.Количество() > 0) Тогда
					РасчетныйДокумент.Начисления.Очистить();
				КонецЕсли;
			КонецЕсли;
			Если Не РасчетныйДокумент.Метаданные().ТабличныеЧасти.Найти("РасчетСреднего")=Неопределено Тогда
				Если (РасчетныйДокумент.РасчетСреднего.Количество() > 0) Тогда
					РасчетныйДокумент.РасчетСреднего.Очистить();
				КонецЕсли;
			КонецЕсли;
// Серна - Конец
