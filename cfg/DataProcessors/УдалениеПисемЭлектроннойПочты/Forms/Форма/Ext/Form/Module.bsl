...
Процедура НайтиОбъектыПомеченныеНаУдаление()
	...
	Состояние(НСтр("ru='Выполняется поиск помеченных на удаление электронных писем ...';uk='Виконується пошук позначених для вилучення електронних листів ...'"));
	ОбъектыДляУдаления.Загрузить(Запрос.Выполнить().Выгрузить());

	Состояние(НСтр("ru='Выполняется поиск ссылок на удаленные электронные письма ...';uk='Виконується пошук посилань на вилучені електронні листи ...'"));
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ЭтаФорма.ВладелецФормы) <> Тип("Форма") ИЛИ НЕ ЗначениеЗаполнено(УчетнаяЗапись) Тогда
		Предупреждение(НСтр("ru='Обработка удаления электронных писем открывается только из формы учетной записи электронной почты.';uk='Обробка вилучення електронних листів відкривається тільки з форми облікового запису електронної пошти.'"));
		Отказ = Истина;
	КонецЕсли; 
...
Процедура ОсновныеДействияФормыУдалить(Кнопка)
	...
	ОтветНаВопрос = Вопрос(НСтр("ru='Удалить отмеченные объекты?';uk=""Видалити відзначені об'єкти?"""), РежимДиалогаВопрос.ОКОтмена);
	...
	Сообщить(НСтр("ru='Электронных писем к удалению: ';uk='Електронних листів до вилучення: '") + МассивПисем.Количество());
	...
	Пока ЕстьУдаленные Цикл
		...
		Сообщить(НСтр("ru='Итерация удаления №';uk='Ітерація вилучення №'") + ИндексИтерации);
		
		ТекПисьмо = 0;
		Для каждого ЭлектронноеПисьмо Из МассивПисем Цикл
			
			ТекПисьмо = ТекПисьмо + 1;
			Состояние(НСтр("ru='Обрабатывается ';uk='Обробляється '") + ТекПисьмо + НСтр("ru=' из ';uk=' з '") + МассивПисем.Количество());
			...
			Если СтрокиСсылок.Количество() = 0 Тогда
				...
				Если ОбъектЭлектронноеПисьмо <> Неопределено Тогда
					Попытка
						...
						СтрокаУдалений = СтрокаУдалений + Символы.ПС + НСтр("ru='Удалено письмо ';uk='Вилучено лист '") + ПредставлениеПисьма;
						УдаленоПисем = УдаленоПисем + 1;
						ЕстьУдаленные = Истина;
					Исключение
						Сообщить("" + ОбъектЭлектронноеПисьмо + НСтр("ru=' не удалено. ';uk=' не вилучено. '") + ОписаниеОшибки());
				...
				Если МожноУдалять Тогда
					...
					Для каждого СтрокаСсылки Из СтрокиСсылок Цикл
						...
						Попытка
							...
							СтрокаУдалений = СтрокаУдалений + Символы.ПС + НСтр("ru='Удалено вложение ';uk='Вилучене вкладення '") + ПредставлениеВложения;
							ЕстьУдаленные = Истина;
						Исключение
							Сообщить("" + ЭлектронноеПисьмо + НСтр("ru=' не удалено. ';uk=' не вилучено. '") + ОписаниеОшибки());
							ОтменаТранзакции = Истина;
						КонецПопытки;
					КонецЦикла;
	...
	Сообщить(НСтр("ru='Удалено электронных писем: ';uk='Вилучено електронних листів: '") + УдаленоПисем + НСтр("ru=' из ';uk=' з '") + МассивПисем.Количество());
