...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если мРезультатДиалога <> "Отмена" тогда
		...
		МестоРожденияСтрокойПослеРедактировния = ""+СокрЛП(Особое)+","+СокрЛП(Город)+","+СокрЛП(Район)+","+СокрЛП(Область)+","+СокрЛП(Страна);

		Если СокрЛП(МестоРожденияСтрокой) <> СокрЛП(МестоРожденияСтрокойПослеРедактировния) тогда

			Если мРезультатДиалога <> "ОК" тогда
				РезультатВопроса = Вопрос(НСтр("ru='Данные о месте рождения изменены. Сохранить?';uk='Дані про місце народження змінені. Зберегти?'"),РежимДиалогаВопрос.ДаНетОтмена);
