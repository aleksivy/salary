...
Функция ЗаписатьДанные()
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Данные не сохранены';uk='Дані не збережені'"));
		Возврат Ложь;
	КонецПопытки;
	...
	Если НЕ ОбщегоНазначения.ПроверитьСтруктуруЗаполненияПараметровДляОтправкиСообщенияВОтделТехническойПоддержки(СтруктураДанных) Тогда
		ОтветНеВопрос = Вопрос(НСтр("ru='Есть незаполненные поля, обязательные для заполнения! Продолжить?';uk=""Є незаповнені поля, обов'язкові для заповнення! Продовжити?"""), РежимДиалогаВопрос.ОКОтмена,,КодВозвратаДиалога.ОК);
...
Процедура ОсновныеДействияФормыПечать(Кнопка)
	...
	Если Модифицированность Тогда
		...
		ОтветНеВопрос = Вопрос(НСтр("ru='Данные будут сохранены! Продолжить?';uk='Дані будуть збережені! Продовжити?'"), РежимДиалогаВопрос.ОКОтмена,,КодВозвратаДиалога.ОК);
		Если ОтветНеВопрос <> КодВозвратаДиалога.ОК Тогда
			Возврат;
		КонецЕсли;
