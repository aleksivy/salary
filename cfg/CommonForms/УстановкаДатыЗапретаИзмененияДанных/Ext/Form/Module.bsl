...
Процедура ЗаполнитьОсновноеДерево()
	...
	|ГДЕ НЕ Ссылка В (&СписокИсключений)";
...
Процедура ЗаполнитьРеглДерево()
	...
	|
	|ГДЕ НЕ Роли.Ссылка В(&СписокИсключений)
    |
	|АВТОУПОРЯДОЧИВАНИЕ";
...
Процедура СохранитьДанные()
	...
	Исключение
		Предупреждение(НСтр("ru='Установка даты запрета изменения данных должна выполняться в монопольном режиме.';uk='Установка дати заборони зміни даних повинна виконуватися у монопольному режимі.'"));		
		Отказ = Истина;
		Возврат;
	КонецПопытки;
	
	Если Не ПравоДоступа("Изменение", Метаданные.РегистрыСведений.ГраницыЗапретаИзмененияДанных) Тогда
		Предупреждение(НСтр("ru='Недостаточно прав доступа для установки даты запрета изменения данных.';uk='Недостатньо прав для встановлення дати заборони зміни даних.'"));
		Возврат;
	КонецЕсли;
	...
	Попытка
		Набор.Записать();
	Исключение
		ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Не удалось записать данные о датах запрета изменения данных.';uk='Не вдалося записати дані про дати заборони зміни даних.'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Значение Из Перечисления.НаборПравПользователей Цикл
			Если СписокИсключений.НайтиПоЗначению(Значение) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Запись=Набор.Добавить();
...
Процедура ДеревоОсновнойДатыПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	Если ДанныеСтроки.НаборПрав.Пустая() Тогда
		ОформлениеСтроки.Ячейки.НаборПрав.УстановитьТекст(НСтр("ru='Все роли';uk='Всі ролі'"));
...
Процедура ДеревоРеглДатыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель)
	...
	Если Организация = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если ДеревоРеглДаты.Строки.Найти(Организация, "ОрганизацияРоль") <> Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДеревоРеглДатыПриАктивизацииСтроки(Элемент)
	ЭлементыФормы.КоманднаяПанельДереваРеглДаты.Кнопки.УдалитьОрганизацию.Доступность = (Элемент.ТекущиеДанные <> Неопределено И Элемент.ТекущиеДанные.Уровень() = 0);
КонецПроцедуры
...
СписокИсключений.Добавить(Перечисления.НаборПравПользователей.Пользователь);
Для Каждого Роль ИЗ Метаданные.Перечисления.НаборПравПользователей.ЗначенияПеречисления Цикл
	Если Лев(Роль.Имя, 7) = "Удалить" Тогда
		СписокИсключений.Добавить(Перечисления.НаборПравПользователей[Роль.Имя]);
	КонецЕсли;
КонецЦикла;
