...
Процедура ОбновитьТаблицуНапоминаний(НовыеНапоминания) Экспорт
	...
	Если ТекущийДокументТабПоля <> Неопределено И ЗначениеЗаполнено(ТекущийДокументТабПоля) Тогда
...
Процедура ОбновитьРеквизитыФормы()
	...
	Если СтрокаТаблицы = Неопределено Тогда
		...
		УстановитьКартинку(РаботаСДиалогами.ПолучитьКартинкуНапоминания(ДокументНапоминания));
	...
	Иначе
		...
		Если СтрокаТаблицы.ФлагДеньРождения И НЕ мТипыДокументовНапоминаний.СодержитТип(ТипЗнч(СтрокаТаблицы.ДокументНапоминания)) Тогда
			...
			ЭтаФорма.Заголовок = НСтр("ru='Напоминание: День рождения ';uk='Нагадування: День народження '")+Формат(ДатаНапоминания,"ДФ='dd ММММ yyyy';ДЛФ=ДД");
			
			УстановитьКартинку(БиблиотекаКартинок.ДеньРождения);
	
		Иначе
			
			ЭтаФорма.Заголовок = НСтр("ru='Напоминание: ';uk='Нагадування: '")+Формат(ДатаНапоминания,"ДФ='dd ММММ yyyy';ДЛФ=ДД")+", "+Формат(ДатаНапоминания,"ДФ='дддд'")+" "
	...
	Пока а <= ЭлементыФормы.ТаблицаНапоминаний.ВыделенныеСтроки.Количество() - 1 Цикл
		...
		Если ВыделеннаяСтрока <> Неопределено Тогда
			...
			Если ВыделеннаяСтрока.ФлагДеньРождения И НЕ мТипыДокументовНапоминаний.СодержитТип(ТипЗнч(ВыделеннаяСтрока.ДокументНапоминания)) Тогда
				...
				НовоеСобытие.Ответственный      = глЗначениеПеременной("глТекущийПользователь");
				
				СтруктураРабочихВремен = УправлениеКонтактами.ОпределитьНачалоИОкончениеРабочегоДняПользователя(глЗначениеПеременной("глТекущийПользователь"),ТекущаяДата());
				...
				НовоеСобытие.СостояниеСобытия   = Перечисления.СостоянияСобытий.Запланировано;
				НовоеСобытие.Комментарий = НСтр("ru='Сформирован автоматически при напоминании о дне рождения';uk='Сформований автоматично при нагадуванні про день народження'");
				
				Попытка
					НовоеСобытие.Записать(РежимЗаписиДокумента.Проведение);
					ТаблицаНапоминаний.Удалить(ВыделеннаяСтрока);
				Исключение
					ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки(),, НСтр("ru='Документ ""Событие"" не создан.';uk='Документ ""Подія"" не створений.'"));
				КонецПопытки;
...
Процедура ТаблицаНапоминанийПриАктивизацииСтроки(Элемент)	
 	ПодключитьОбработчикОжидания("ОбработчикОжиданияТаблицаНапоминанийПриАктивизацииСтроки", 0.1, Истина);		
...
//
Процедура ОбработчикОжиданияТаблицаНапоминанийПриАктивизацииСтроки()
	ОбновитьРеквизитыФормы();
КонецПроцедуры // ОбработчикОжиданияТаблицаНапоминанийПриАктивизацииСтроки
...
Процедура ТаблицаНапоминанийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	ОформлениеСтроки.Ячейки.Картинка.ИндексКартинки     = ?((ДанныеСтроки.ФлагДеньРождения И НЕ мТипыДокументовНапоминаний.СодержитТип(ТипЗнч(ДанныеСтроки.ДокументНапоминания))), 7, РаботаСДиалогами.ОпределитьИндексКартинкиВидаСобытия(ДанныеСтроки.ДокументНапоминания));
...
ТаблицаНапоминаний.Индексы.Добавить("ДокументНапоминания, КонтактноеЛицо");
