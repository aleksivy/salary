...
Перем ПериодДействия Экспорт;

////////////////////////////////////////////////////////////////////////////////
// ОБЩИЕ ПРОЦЕДУРЫ И ФУНКЦИИ

// Процедура - заполняет таблицу показателей
//
Процедура ФормаАвтозаполнение(ТаблицаЗапроса = Неопределено, Сотрудники = Неопределено, Показатели = Неопределено, ОдинВидРасчета = Ложь, ИзДокумента = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               ...
		               |				И (ЗначенияПоказателейСхемМотивации.Сотрудник В(&Сотрудники) Или ЗначенияПоказателейСхемМотивации.Сотрудник = Значение(Справочник.СотрудникиОрганизаций.ПустаяСсылка))
		               |			
		               |			СГРУППИРОВАТЬ ПО
		               ...
		               |						И (ТаблицаЗапроса.Сотрудник В (&Сотрудники))
		               |			
		               |			СГРУППИРОВАТЬ ПО
		               |				ВЫБОР
		               |					КОГДА ТаблицаЗапроса.ВидПоказателя = ЗНАЧЕНИЕ(Перечисление.ВидыПоказателейСхемМотивации.ПоПодразделению)
		               ...
		               |		ГДЕ
		               |			ПЛАН.Сотрудник В(&Сотрудники)) КАК ВложеннаяТаблица
		               |	
		               |	СГРУППИРОВАТЬ ПО
		               ...
		               |ГДЕ
		               |	ИтоговаяТаблица.Показатель.ВозможностьИзменения = ЗНАЧЕНИЕ(Перечисление.ИзменениеПоказателейСхемМотивации.ВиденНоНеРедактируетсяПриРасчете)
		               |	И ВЫБОР
		               |			КОГДА ИтоговаяТаблица.Показатель.ВидПоказателя = ЗНАЧЕНИЕ(Перечисление.ВидыПоказателейСхемМотивации.ПоПодразделению)
		               |				ТОГДА ИтоговаяТаблица.Подразделение <> НЕОПРЕДЕЛЕНО
		               |						И ИтоговаяТаблица.Подразделение ЕСТЬ НЕ NULL 
		               |						И ИтоговаяТаблица.Подразделение <> ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
		               |			ИНАЧЕ ИСТИНА
		               |		КОНЕЦ
		               |
		               |СГРУППИРОВАТЬ ПО
...
Процедура ПриОткрытии()
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		мМассивЭУ.Добавить(ЭлементыФормы.ТаблицаПоказателей.Колонки.Валюта);
	...
	ЭлементыФормы.ТаблицаПоказателей.ТекущаяКолонка = ЭлементыФормы.ТаблицаПоказателей.Колонки.Значение;
	
	Если мСотрудникДляОткрытия <> Неопределено Тогда
		Для Каждого СтркаНабора Из ТаблицаПоказателей Цикл
			ТекущийСотрудник = СтркаНабора.Сотрудник;
			Если ТекущийСотрудник <> Неопределено И ТекущийСотрудник <> Null Тогда
				Если ТекущийСотрудник = мСотрудникДляОткрытия Или ТекущийСотрудник.Физлицо = мСотрудникДляОткрытия Тогда
					ЭлементыФормы.ТаблицаПоказателей.ТекущаяСтрока = СтркаНабора;
					Прервать;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ т.п. ТаблицаПоказателей

// Процедура - обработчик события "ПриВыводеСтроки"
//
Процедура ТаблицаПоказателейПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	...
	Если СведенияОПоказателе.ВидПоказателя = Перечисления.ВидыПоказателейСхемМотивации.ПоПодразделению Тогда
		...
		ЭлементыФормы.ТаблицаПоказателей.Колонки.по.ТекстШапки = НСтр("ru='Сотрудник (подразделение)';uk='Співробітник (підрозділ)'");
	Иначе
	...
	//
КонецПроцедуры //ТаблицаПоказателейПриВыводеСтроки

// Процедура - обработчик события "ПередНачаломДобавления"
//
Процедура ТаблицаПоказателейПередНачаломДобавления(Элемент, Отказ, Копирование)
	Отказ = Истина;
КонецПроцедуры
...
Процедура ТаблицаПоказателейПриАктивизацииСтроки(Элемент)
	...
	Показатель = ЭлементыФормы.ТаблицаПоказателей.ТекущиеДанные.Показатель;
	СведенияОПоказателе = ПолучитьСведенияОПоказателе(мСведенияОПоказателях, Показатель);
	Если СведенияОПоказателе.ТипПоказателя = Перечисления.ТипыПоказателейСхемМотивации.Денежный Тогда
		ЭлементыФормы.ТаблицаПоказателей.Колонки["Значение"].ЭлементУправления.Формат = "ЧДЦ=2";
	КонецЕсли;
...
//
Процедура ТаблицаПоказателейНачалоВыбора(Элемент, СтандартнаяОбработка)
...
// Процедура - обработчик события "АвтоПодборТекста"
//
Процедура ТаблицаПоказателейПоказательАвтоПодборТекста(Элемент, Текст, ТекстАвтоПодбора, СтандартнаяОбработка)
...
//
Процедура ТаблицаПоказателейППоказательОкончаниеВводаТекста(Элемент, Текст, Значение, СтандартнаяОбработка)
...
// Процедура обрабатывает нажание на кнопку "ОК"
//
Процедура ОсновныеДействияФормыДействие(Кнопка)
	
	Оповестить("ЗаписьПоказателя", ТаблицаПоказателей);
	
	Для Каждого СтрокаТабПоказателей Из ТаблицаПоказателей Цикл
		МенеджерЗаписи = РегистрыСведений.ЗначенияПоказателейСхемМотивации.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.Организация		= Организация;
		МенеджерЗаписи.ПериодДействия	= ПериодДействия;
		МенеджерЗаписи.Период			= ПериодДействия;
		МенеджерЗаписи.Подразделение	= СтрокаТабПоказателей.Подразделение;
		МенеджерЗаписи.Сотрудник		= СтрокаТабПоказателей.Сотрудник;
		МенеджерЗаписи.Показатель		= СтрокаТабПоказателей.Показатель;
		МенеджерЗаписи.Валюта			= СтрокаТабПоказателей.Валюта;
		МенеджерЗаписи.Значение			= СтрокаТабПоказателей.Значение;
