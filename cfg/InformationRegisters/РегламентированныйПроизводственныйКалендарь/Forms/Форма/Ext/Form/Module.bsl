...
Процедура ОбновитьСтрокуСведенийОПеренесенныхДнях(ОбновитьЗаголовок = Ложь)
	...
	Если ОбновитьЗаголовок Тогда
		ЭлементыФормы.ЗаголовокПеренесенныеДни.Значение = НСтр("ru='Изменения за ';uk='Зміни за '")+Формат(ГодВФорме, "ЧГ=0")+ НСтр("ru=' год';uk=' рік'");
	КонецЕсли;
	ТекстЗаголовка = "";
	...
	Для Каждого ДатаРегиональногоПраздника Из МассивРегиональныхПраздников Цикл
		...
		Если ДеньНедели = 1 или ДеньНедели = 2 или ДеньНедели = 4 Тогда
			СловоНазначен = НСтр("ru=' назначен ';uk=' призначений '");
		ИначеЕсли ДеньНедели = 7 Тогда
			СловоНазначен = НСтр("ru=' назначено ';uk=' призначено '");
		Иначе
			СловоНазначен = НСтр("ru=' назначена ';uk=' призначена '");
		КонецЕсли;
		
		ВидНазначенногоДня = НСтр("ru=' региональным праздником';uk=' регіональним святом'");
		...
		Если СтрокаТаблицыНазначенныхДней <> Неопределено Тогда
			Если СтрокаТаблицыНазначенныхДней.ВидДня = ВидДняПредпраздничный Тогда
				ВидНазначенногоДня = НСтр("ru=' рабочим предпраздничным днем';uk=' робочим передсвятковим днем'");
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого СтрокаТаблицыПеренесенныхДней Из ТаблицаПеренесенныхДней Цикл
		...
		Если ДеньНедели = 1 или ДеньНедели = 2 или ДеньНедели = 4 Тогда
			СловоПеренесен = НСтр("ru=' перенесен на ';uk=' перенесений на '");
		ИначеЕсли ДеньНедели = 7 Тогда
			СловоПеренесен = НСтр("ru=' перенесено на ';uk=' перенесений на '");
		Иначе
			СловоПеренесен = НСтр("ru=' перенесена на ';uk=' перенесена на '");
		КонецЕсли;
	...
	КонецЦикла;
	ЭлементыФормы.СведенияОПеренесенныхДнях.Заголовок = ТекстЗаголовка + НСтр("ru='Для переноса выходного дня выберите день и нажмите правую кнопку мыши';uk='Для перенесення вихідного дня, виберіть день і натисніть праву кнопку миші'");
...
Процедура ПриОткрытии()
	...
	Заголовок = НСтр("ru='Регламентированный производственный календарь на ';uk='Регламентований виробничий календар на '") + Формат(ГодВФорме,"ЧГ=0") + НСтр("ru=' год';uk=' рік'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ТаблицыКалендаряИдентичны() Тогда
		...
		Если Вопрос(НСтр("ru='Записать измененные данные за ';uk='Записати змінені дані за '") + Формат(ЗаКакойГод,"ЧГ=0") + НСтр("ru=' год?';uk=' рік?'"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
...
Процедура ДействияФормыПервоначальноеЗаполнение(Кнопка)
	...
	Если Вопрос(НСтр("ru='Восстановить первоначальное заполнение производственного календаря?';uk='Відновити первісне заповнення виробничого календаря?'"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
...
Процедура ДействияФормыПечать(Кнопка)
	...
	Если Модифицированность тогда
		Если Вопрос(НСтр("ru='Перед печать календаря необходимо записать внесенные изменения. Записать?';uk='Перед друком календаря необхідно записати внесені зміни. Записати?'"), РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да тогда
	...
	МакетПечати.КодЯзыкаМакета = КодЯзыкаПечать;
	...
	Пока ВыборкаПоГоду.Следующий() Цикл
		...
		Пока ВыборкаПоКварталу.Следующий() Цикл
			...
			НомерКвартала.Параметры.НомерКвартала = "" + ВыборкаПоКварталу.КварталКалендаря + НСтр("ru=' квартал';uk=' квартал'", КодЯзыкаПечать);
			...
			Пока ВыборкаПоМесяцу.Следующий() Цикл
				...
				КолонкаМесяца.Параметры.ИмяМесяца 		= Формат(Дата(2000,ВыборкаПоМесяцу.МесяцКалендаря, 1), "Л="+Локализация.ОпределитьКодЯзыкаДляФормат(КодЯзыкаПечать)+"; ДФ='ММММ'");
			...
			КолонкаМесяца.Параметры.ИмяМесяца 		= "" + ВыборкаПоКварталу.КварталКалендаря + НСтр("ru=' квартал';uk=' квартал'", КодЯзыкаПечать);
			ПечатнаяФорма.Присоединить(КолонкаМесяца);
			...
			Если ВыборкаПоКварталу.КварталКалендаря = 2 или ВыборкаПоКварталу.КварталКалендаря = 4 тогда
				...
				КолонкаМесяца.Параметры.ИмяМесяца 		= "" + ВыборкаПоКварталу.КварталКалендаря/2 + НСтр("ru=' полугодие';uk=' півріччя'", КодЯзыкаПечать);
				ПечатнаяФорма.Присоединить(КолонкаМесяца);
			КонецЕсли;
		...
		КолонкаМесяца.Параметры.ИмяМесяца 		= "" + Формат(ВыборкаПоГоду.ГодКалендаря, "ЧЦ=4; ЧГ=0") + НСтр("ru=' год';uk=' рік'", КодЯзыкаПечать);
		ПечатнаяФорма.Присоединить(КолонкаМесяца);
	...
	КолонкаМесяца.Параметры.РабочееВремя36 	= РабочееВремя36Год;
	КолонкаМесяца.Параметры.РабочееВремя24 	= РабочееВремя24Год;
	КолонкаМесяца.Параметры.ИмяМесяца 		= "" + Формат(ГодВФорме, "ЧЦ=4; ЧГ=0") + НСтр("ru=' год';uk=' рік'", КодЯзыкаПечать);
	...
	КолонкаМесяца.Параметры.ИмяМесяца 		= НСтр("ru='среднемесячное количество';uk='середньомісячна кількість'", КодЯзыкаПечать);
	ПечатнаяФорма.Присоединить(КолонкаМесяца);
	
	УниверсальныеМеханизмы.НапечататьДокумент(ПечатнаяФорма, 1, Ложь, НСтр("ru='Производственный календарь';uk='Виробничий календар'"));
...
Процедура ДействияПеренестиВыходнойДеньСубботу(Элемент)
	...
	ТекстКомментария = НСтр("ru='Выберите рабочий день, на который необходимо перенести выходной день, ';uk='Виберіть робочий день, на який необхідно перенести вихідний день, '")+Формат(ПроизводственныйКалендарь, "ДФ=""д ММММ, дддд""");
...
Процедура ДействияПеренестиРабочийДень(Элемент)
	...
	ТекстКомментария = НСтр("ru='Выберите выходной день, на который необходимо перенести рабочий день, ';uk='Виберіть вихідний день, на який необхідно перенести робочий день, '")+Формат(ПроизводственныйКалендарь, "ДФ=""д ММММ, дддд""");
...
Процедура ДействияНазначитьВоскресеньем(Элемент)
	
	ТекущийВидДня = СписокВидовДней.НайтиПоЗначению(ВидДняВоскресенье);
	ЭлементыФормы.ПроизводственныйКалендарь.Обновить();
	ОбновитьСтрокуСведенийОПеренесенныхДнях();
...
Процедура ПроизводственныйКалендарьПриИзменении(Элемент)
	...
	Если СтрокаТаблицыРегистра <> Неопределено Тогда
		...
		Если ВидДня = ВидДняРабочий Тогда
			...
			ДействияКнопки.Вставить(0,"ПеренестиРабочийДень", ТипКнопки, НСтр("ru='Перенести рабочий день ...';uk='Перенести робочий день ...'"), НовоеДействие);
			КонтекстДействияКнопки.Вставить(0,"ПеренестиРабочийДень", ТипКнопки, НСтр("ru='Перенести рабочий день ...';uk='Перенести робочий день ...'"), НовоеДействие);
			
			НовоеДействие = Новый Действие("ДействияНазначитьВоскресеньем");
			ДействияКнопки.Вставить(1,"НазначитьВоскресеньем", ТипКнопки, НСтр("ru='Назначить этот день воскресеньем';uk='Призначити цей день неділею'"), НовоеДействие);
			КонтекстДействияКнопки.Вставить(1,"НазначитьВоскресеньем", ТипКнопки, НСтр("ru='Назначить этот день воскресеньем';uk='Призначити цей день неділею'"), НовоеДействие);
			
			НовоеДействие = Новый Действие("ДействияНазначитьПраздничным");
			ДействияКнопки.Вставить(2,"НазначитьПраздничным", ТипКнопки, НСтр("ru='Назначить этот день праздничным днем';uk='Призначити цей день святковим днем'"), НовоеДействие);
			КонтекстДействияКнопки.Вставить(2,"НазначитьПраздничным", ТипКнопки, НСтр("ru='Назначить этот день праздничным днем';uk='Призначити цей день святковим днем'"), НовоеДействие);
			
			НовоеДействие = Новый Действие("ДействияНазначитьПредпраздничным");
			ДействияКнопки.Вставить(3,"НазначитьПредпраздничным", ТипКнопки, НСтр("ru='Назначить этот день рабочим предпраздничным днем';uk='Призначити цей день робочим передсвятковим днем'"), НовоеДействие);
			КонтекстДействияКнопки.Вставить(3,"НазначитьПредпраздничным", ТипКнопки, НСтр("ru='Назначить этот день рабочим предпраздничным днем';uk='Призначити цей день робочим передсвятковим днем'"), НовоеДействие);
		...
		ИначеЕсли ВидДня = ВидДняПредпраздничный Тогда
			...
			Если ЭтоФедеральныйПраздник(ПроизводственныйКалендарь) Тогда
				...
				НовоеДействие = Новый Действие("ДействияНазначитьПраздничным");
				ДействияКнопки.Вставить(1,"НазначитьПраздничным", ТипКнопки, НСтр("ru='Назначить этот день праздничным днем';uk='Призначити цей день святковим днем'"), НовоеДействие);
			    КонтекстДействияКнопки.Вставить(1,"НазначитьПраздничным", ТипКнопки, НСтр("ru='Назначить этот день праздничным днем';uk='Призначити цей день святковим днем'"), НовоеДействие);
			...
			Иначе
				...
				ДействияКнопки.Вставить(1,"ОтменитьПредпраздничныйДень", ТипКнопки, НСтр("ru='Отменить предпраздничный день';uk='Скасувати передсвятковий день'"), НовоеДействие);
			    КонтекстДействияКнопки.Вставить(1,"ОтменитьПредпраздничныйДень", ТипКнопки, НСтр("ru='Отменить предпраздничный день';uk='Скасувати передсвятковий день'"), НовоеДействие);
		...
		ИначеЕсли ВидДня = ВидДняПраздник Тогда
			...
			Если ЭтоФедеральныйПраздник(ПроизводственныйКалендарь, ЭтоФедеральныйПредпраздничныйДень) 
				...
				Если ПраздникПеренесенНаДругойДень(ПроизводственныйКалендарь) Тогда
					...
					ДействияКнопки.Вставить(0,"ОтменитьПереносПраздника", ТипКнопки, НСтр("ru='Отменить перенос выходного дня';uk='Скасувати перенесення вихідного дня'"), НовоеДействие);
			  		КонтекстДействияКнопки.Вставить(0,"ОтменитьПереносПраздника", ТипКнопки, НСтр("ru='Отменить перенос выходного дня';uk='Скасувати перенесення вихідного дня'"), НовоеДействие);
					
				Иначе
					Если ДеньНедели = 6 Тогда
					
						НовоеДействие = Новый Действие("ДействияПеренестиВыходнойДеньСубботу");
						ДействияКнопки.Вставить(0,"ПеренестиВыходнойДень", ТипКнопки, НСтр("ru='Перенести выходной день ...';uk='Перенести вихідний день ...'"), НовоеДействие);
						КонтекстДействияКнопки.Вставить(0,"ПеренестиВыходнойДень", ТипКнопки, НСтр("ru='Перенести выходной день ...';uk='Перенести вихідний день ...'"), НовоеДействие);
			...
			Иначе
				...
				Иначе
					Если ДеньНедели = 6 Тогда
						...
						НовоеДействие = Новый Действие("ДействияОтменитьПраздничныйДень");
						ДействияКнопки.Вставить(1,"ОтменитьПраздничныйДень", ТипКнопки, НСтр("ru='Отменить праздничный день';uk='Скасувати святковий день'"), НовоеДействие);
						КонтекстДействияКнопки.Вставить(1,"ОтменитьПраздничныйДень", ТипКнопки, НСтр("ru='Отменить праздничный день';uk='Скасувати святковий день'"), НовоеДействие);
					...
					Иначе
						НовоеДействие = Новый Действие("ДействияОтменитьПраздничныйДень");
						ДействияКнопки.Вставить(0,"ОтменитьПраздничныйДень", ТипКнопки, НСтр("ru='Отменить праздничный день';uk='Скасувати святковий день'"), НовоеДействие);
						КонтекстДействияКнопки.Вставить(0,"ОтменитьПраздничныйДень", ТипКнопки, НСтр("ru='Отменить праздничный день';uk='Скасувати святковий день'"), НовоеДействие);
		...
		ИначеЕсли ВидДня = ВидДняСуббота Тогда
			Если ДеньНедели = 6 Тогда
				...
				НовоеДействие = Новый Действие("ДействияНазначитьПраздничным");
				ДействияКнопки.Вставить(0,"НазначитьПраздничным", ТипКнопки, НСтр("ru='Назначить этот день праздничным днем';uk='Призначити цей день святковим днем'"), НовоеДействие);
				КонтекстДействияКнопки.Вставить(0,"НазначитьПраздничным", ТипКнопки, НСтр("ru='Назначить этот день праздничным днем';uk='Призначити цей день святковим днем'"), НовоеДействие);
		...
		ИначеЕсли ВидДня = ВидДняВоскресенье Тогда	
			...
			Иначе
				...
				ДействияКнопки.Вставить(0,"НазначитьРабочим", ТипКнопки, НСтр("ru='Назначить этот день рабочим днем';uk='Призначити цей день робочим днем'"), НовоеДействие);
				КонтекстДействияКнопки.Вставить(0,"НазначитьРабочим", ТипКнопки, НСтр("ru='Назначить этот день рабочим днем';uk='Призначити цей день робочим днем'"), НовоеДействие);
				
				НовоеДействие = Новый Действие("ДействияОтменитьПереносПраздника");
				ДействияКнопки.Вставить(1,"ОтменитьПереносПраздника", ТипКнопки, НСтр("ru='Отменить перенос выходного дня';uk='Скасувати перенесення вихідного дня'"), НовоеДействие);
				КонтекстДействияКнопки.Вставить(1,"ОтменитьПереносПраздника", ТипКнопки, НСтр("ru='Отменить перенос выходного дня';uk='Скасувати перенесення вихідного дня'"), НовоеДействие);
