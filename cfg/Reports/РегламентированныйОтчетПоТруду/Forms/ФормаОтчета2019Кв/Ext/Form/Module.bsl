...
Процедура ЗаполнитьНачальныеНастройки(Макет = "СоставПоказателей2019Кв") Экспорт
...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если мСтруктураВариантыЗаполнения.Свойство(ИмяСтраницыПанели, МассивВариантовЗаполненияСтраниц) Тогда
		...
		Иначе
			...
			МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2019Кв");
...
Процедура РасчетПоказателейКоличестваРаботников()
	...
	Если    РегламентированнаяОтчетность.ИДКонфигурации() = "УПП" 
		...
		|";
		Результат = ЗапросПоШтатным.Выполнить().Выбрать();
		Если КонецГода(мДатаКонцаПериодаОтчета) = КонецКвартала(мДатаКонцаПериодаОтчета) Тогда
			Если Результат.Следующий() Тогда
...
Процедура ВосстановитьГраницыРаспределенияРаздел3()
	...
	мОтчетПоТруду.Области.Пр3.Значение    = 4500.01;
	мОтчетПоТруду.Области.Пр4.Значение    = 5000.01;
	мОтчетПоТруду.Области.Пр5.Значение    = 6000.01;
	мОтчетПоТруду.Области.Пр6.Значение    = 7000.01;
	мОтчетПоТруду.Области.Пр7.Значение    = 8000.01;
	мОтчетПоТруду.Области.Пр8.Значение    = 10000.01;
	мОтчетПоТруду.Области.Пр9.Значение    = 12000.01;
	мОтчетПоТруду.Области.Пр91.Значение   = 15000.01;
	мОтчетПоТруду.Области.Пр10.Значение   = 20000.00;
	
	мОтчетПоТруду.Области.Пр200.Значение  = 4500.00;
	мОтчетПоТруду.Области.Пр300.Значение  = 5000.00;
	мОтчетПоТруду.Области.Пр400.Значение  = 6000.00;
	мОтчетПоТруду.Области.Пр500.Значение  = 7000.00;
	мОтчетПоТруду.Области.Пр600.Значение  = 8000.00;
	мОтчетПоТруду.Области.Пр700.Значение  = 10000.00;
	мОтчетПоТруду.Области.Пр800.Значение  = 12000.00;
...
мСоставПоказателей = "СоставПоказателей2019Кв";

// ОПИСАНИЕ ПАРМЕТРОВ ФОРМЫ
//
// Версия формы
мВерсияФормы = "01/01/2019";
