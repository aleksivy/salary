...
Процедура ЗаполнитьНачальныеНастройки(Макет = "СоставПоказателей2017Кв") Экспорт
	...
	Для Инд = 0 По ТабличноеПоле.Области.Количество() - 1 Цикл
		...
		Если Для1СЗвит = Истина Тогда
			...
			Если ТипЗнч(ЗначениеПоказателя) = Тип("Число") И НЕ ЗначениеПоказателя = 0 Тогда
				...
				// теперь выгружаем в Звит "как видим"
				Сдвиг = 1;
...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если мСтруктураВариантыЗаполнения.Свойство(ИмяСтраницыПанели, МассивВариантовЗаполненияСтраниц) Тогда
		...
		Иначе
			...
			МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2017Кв");
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	Если     Организация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо
		И НЕ РегламентированнаяОтчетность.ИДКонфигурации() = "УТ" 
		И НЕ РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП" Тогда
		ОбъектКонтактнойИнформации = Организация.ИндивидуальныйПредприниматель;
	КонецЕсли;
	мОтчетПоТруду.Области.ЕлПочта.Значение = УправлениеКонтактнойИнформацией.ПолучитьEmailИзКонтактнойИнформации(ОбъектКонтактнойИнформации);

	//мОтчетПоТруду.Области.ПериодОтчета.Значение = "за січень - " + Формат(мДатаКонцаПериодаОтчета, "ДФ = 'ММММ гггг';Л = uk_UA") + " р.";
	...
	мОтчетПоТруду.Области.ПериодОтчета.Значение = "за " + НомерКварталРимск + " квартал " + Формат(мДатаКонцаПериодаОтчета, "ДФ='гггг';Л = uk_UA") + " р.";
	...
	мОтчетПоТруду.Области.ДатаРаздел7.Значение   =  Формат(Год(мДатаКонцаПериодаОтчета) - 1, "ЧДЦ=0; ЧГ=0") + " р.";
	
	мОтчетПоТруду.Области.КодПодразделения1.Значение = "";
	мОтчетПоТруду.Области.КодПодразделения2.Значение = "";
	мОтчетПоТруду.Области.КодПодразделения3.Значение = "";
	мОтчетПоТруду.Области.КодПодразделения4.Значение = "";
	
	//Заполнение поля" "Исполнитель"
	мОтчетПоТруду.Области.ИСПОЛНИТЕЛЬ.Значение    = глЗначениеПеременной("глТекущийПользователь");	
	...
	мОтчетПоТруду.Области.ИСПОЛНИТЕЛЬ.Значение = СокрЛП(ФИОСрез.Фамилия) + " " + Лев(СокрЛ(ФИОСрез.Имя), 1) + "." + Лев(СокрЛ(ФИОСрез.Отчество), 1) + ".";
	...
	Попытка
		...
		Если РС.Количество() > 0 Тогда
			МинЗП = РС[0].Размер;
			Сообщить("Розмір мінімальної оплати праці на " + Формат(мДатаКонцаПериодаОтчета, "ДФ=dd.MM.yyyy") + ": " + Формат(МинЗП, "ЧЦ=15; ЧДЦ=2"));
		КонецЕсли;
	Исключение
		Сообщить("Не заданий розмір мінімальної заробітної праці! Перевірте регістр відомостей ""Мінімальна оплата праці""");
	КонецПопытки;	
...
//
Процедура Очистить(ПерезаполнятьГраницыРаздела3 = Истина)
	
	// Очищаем содержимое ячеек поля табличного документа
	ТекТабличноеПоле = мОтчетПоТруду;
	
	Если НЕ ПерезаполнятьГраницыРаздела3 Тогда
		СтруктураГраницРаздела3 = Новый Структура;
		Для каждого ТекущаяОбласть Из ТекТабличноеПоле.Области Цикл
			...
			Если НЕ(ТекущаяОбласть.СодержитЗначение) Тогда
				Продолжить;
			КонецЕсли;
			Если Лев(ТекущаяОбласть.Имя, 2) = "Пр" Тогда
				СтруктураГраницРаздела3.Вставить(ТекущаяОбласть.Имя, ТекущаяОбласть.Значение);
			КонецЕсли;	
		КонецЦикла;
	КонецЕсли;
	...
	Если ПерезаполнятьГраницыРаздела3 Тогда
		
		ВосстановитьГраницыРаспределенияРаздел3();	
	
	Иначе	
		
		Для каждого Область Из СтруктураГраницРаздела3 Цикл
			ТекТабличноеПоле.Области[Область.ключ].Значение = Область.Значение;
		КонецЦикла;
...
Процедура РазбитьПечатнуюФорму(ПечФорма)
	...
	ИмяФормы = НСтр("ru = 'Отчет по труду'; uk = 'Звіт із праці'");
...
Процедура РасчетНачисленийПоКатегориямРаботников()
	...
	ЗапросОтработано.УстановитьПараметр("НачДата", мДатаНачалаПериодаОтчета);
...
Процедура РазбитьНачисленияПоШкале(ТаблицаНачислений)
	...
	Для каждого стр Из ТаблицаНачислений Цикл
		Если (стр.Результат <= МинЗП) Тогда 
			мОтчетПоТруду.Области.П6020_1.Значение = мОтчетПоТруду.Области.П6020_1.Значение + 1;
			
		ИначеЕсли (стр.Результат >= (МинЗП + 0.01)) и (стр.Результат <= мОтчетПоТруду.Области.Пр200.Значение) Тогда 
		...
		ИначеЕсли (стр.Результат >= мОтчетПоТруду.Области.Пр91.Значение) и (стр.Результат <= мОтчетПоТруду.Области.Пр910.Значение) Тогда 
...
Процедура РасчетПоказателейКоличестваРаботников()
	...
	ЗапросПриемУвольнение.УстановитьПараметр("ДатаНач", мДатаНачалаПериодаОтчета);
	...
	//мОтчетПоТруду.Области.П3030_1.Значение =  ТаблицаПриемУвольнение.НайтиСтроки(ПараметрыОтбора).Количество();
	...
	Если    РегламентированнаяОтчетность.ИДКонфигурации() = "УПП" 
		...
		|																  И ГрафикРаботы.СокращенноеРабочееВремя)	КАК РаботникиОрганизаций
		|
		|ГДЕ 	
		|	ЗанимаемыхСтавок > 0
		|";
		Результат = ЗапросПоШтатным.Выполнить().Выбрать();
		Если Результат.Следующий() Тогда
			мОтчетПоТруду.Области.П3080_1.Значение = Результат.КоличествоВсего;
			мОтчетПоТруду.Области.П3080_2.Значение = Результат.КоличествоЖенщин;		
		КонецЕсли;
		
	КонецЕсли;	
	
	// состояния работников организаций
	Если  НЕ РегламентированнаяОтчетность.ИДКонфигурации() = "БП" Тогда 
		
		ЗапросПоСостоянию = Новый Запрос();
		ЗапросПоСостоянию.УстановитьПараметр("Организация",         		Организация);
		ЗапросПоСостоянию.УстановитьПараметр("ОтпускПоБеременностиИРодам", 	Перечисления.СостоянияРаботникаОрганизации.ОтпускПоБеременностиИРодам);
		ЗапросПоСостоянию.УстановитьПараметр("ОтпускПоУходуЗаРебенком", 	Перечисления.СостоянияРаботникаОрганизации.ОтпускПоУходуЗаРебенком);
		ЗапросПоСостоянию.УстановитьПараметр("ОсновноеМестоРаботы", 	    Перечисления.ВидыЗанятостиВОрганизации.ОсновноеМестоРаботы);
		ЗапросПоСостоянию.УстановитьПараметр("ДатаКон",  			  		мДатаКонцаПериодаОтчета);
		...
		Если РегламентированнаяОтчетность.ИДКонфигурации() = "УТП" Тогда
			...
			|					И Сотрудник.ФизЛицо.Пол = ЗНАЧЕНИЕ(Перечисление.ПолФизическихЛиц.Женский) 			   	   ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК КоличествоОтпускЖенщин
			|
			|ИЗ
			|	РегистрСведений.СостояниеРаботниковОрганизаций.СрезПоследних(&ДатаКон, Организация = &Организация) КАК Состояние 
			|ГДЕ  Сотрудник.ВидЗанятости = &ОсновноеМестоРаботы
			|   И (Состояние = &ОтпускПоБеременностиИРодам ИЛИ Состояние = &ОтпускПоУходуЗаРебенком)
			|";
			ЗапросПоСостоянию.УстановитьПараметр("ДатаКон", мДатаКонцаПериодаОтчета - 1);
		Иначе
			ЗапросПоСостоянию.Текст = "
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	СУММА(ВЫБОР КОГДА Состояние = &ОтпускПоБеременностиИРодам 												   ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК КоличествоДекретные,
			|	СУММА(ВЫБОР КОГДА Состояние = &ОтпускПоУходуЗаРебенком И Период <= &ДатаКон И ПериодЗавершения >= &ДатаКон ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК КоличествоОтпускВсего,
			|	СУММА(ВЫБОР КОГДА Состояние = &ОтпускПоУходуЗаРебенком И Период <= &ДатаКон И ПериодЗавершения >= &ДатаКон
			|					   И Сотрудник.ФизЛицо.Пол = ЗНАЧЕНИЕ(Перечисление.ПолФизическихЛиц.Женский) 			   ТОГДА 1 ИНАЧЕ 0 КОНЕЦ) КАК КоличествоОтпускЖенщин
			|
			|ИЗ
			|	РегистрСведений.СостояниеРаботниковОрганизаций.СрезПоследних(&ДатаКон, Организация = &Организация) КАК Состояние 
		...
		Если Результат.Следующий() Тогда
			мОтчетПоТруду.Области.П3090_2.Значение = Результат.КоличествоДекретные;
			мОтчетПоТруду.Области.П3100_1.Значение = Результат.КоличествоОтпускВсего;
			мОтчетПоТруду.Области.П3100_2.Значение = Результат.КоличествоОтпускЖенщин;		
		КонецЕсли;
...
Процедура РаспределениеРаботниковПоРазмеруЗП()
	...
	Если РегламентированнаяОтчетность.ИДКонфигурации()="БП" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//НетПериодДействия","");	
	Иначе	
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//ЕстьПериодДействия","");	
	КонецЕсли;
	Запрос.УстановитьПараметр("НачДата", НачалоМесяца(мДатаКонцаПериодаОтчета));
...
Процедура РасчетКоличестваИФОПКатегорийРаботников()	
	ТаблЧисленностиССЧЖ = ПроведениеРасчетов.ПолучитьСреднююЧисленность(мДатаНачалаПериодаОтчета, мДатаКонцаПериодаОтчета, "ССЧ", "КВАРТАЛ", Организация, "ЖШТАТ"); 
	мОтчетПоТруду.Области.П7010_1.Значение = ТаблЧисленностиССЧЖ[0].Средняя;
	ТаблЧисленностиСовм = ПроведениеРасчетов.ПолучитьСреднююЧисленность(мДатаНачалаПериодаОтчета, мДатаКонцаПериодаОтчета, "ССЧ", "КВАРТАЛ", Организация, "СОВМ"); 
...
Функция ПроверитьГраницыРаспределенияРаздел4()
	...
	Если    мОтчетПоТруду.Области["Пр900"].Значение >= мОтчетПоТруду.Области["Пр91"].Значение
		ИЛИ мОтчетПоТруду.Области["Пр91"].Значение >= мОтчетПоТруду.Области["Пр910"].Значение Тогда
		    Результат = Ложь;
	КонецЕсли;
	
	Если мОтчетПоТруду.Области.Пр10.Значение <> мОтчетПоТруду.Области.Пр910.Значение Тогда
	    Результат = Ложь;
	КонецЕсли;
...
Процедура ВосстановитьГраницыРаспределенияРаздел3()
	...
	мОтчетПоТруду.Области.Пр3.Значение    = 4000.01;
	мОтчетПоТруду.Области.Пр4.Значение    = 5000.01;
	мОтчетПоТруду.Области.Пр5.Значение    = 5500.01;
	мОтчетПоТруду.Области.Пр6.Значение    = 6000.01;
	мОтчетПоТруду.Области.Пр7.Значение    = 7000.01;
	мОтчетПоТруду.Области.Пр8.Значение    = 8000.01;
	мОтчетПоТруду.Области.Пр9.Значение    = 10000.01;
	мОтчетПоТруду.Области.Пр91.Значение   = 15000.01;
	мОтчетПоТруду.Области.Пр10.Значение   = 20000.00;
	
	мОтчетПоТруду.Области.Пр200.Значение  = 4000.00;
	мОтчетПоТруду.Области.Пр300.Значение  = 5000.00;
	мОтчетПоТруду.Области.Пр400.Значение  = 5500.00;
	мОтчетПоТруду.Области.Пр500.Значение  = 6000.00;
	мОтчетПоТруду.Области.Пр600.Значение  = 7000.00;
	мОтчетПоТруду.Области.Пр700.Значение  = 8000.00;
	мОтчетПоТруду.Области.Пр800.Значение  = 10000.00;
	мОтчетПоТруду.Области.Пр900.Значение  = 15000.00;
	мОтчетПоТруду.Области.Пр910.Значение  = 20000.00;
...
Процедура РаспределениеОтПриИзменении(Элемент)
	...
	Если ИмяПоказателя = "Пр91" Тогда
		ИмяИзменяемогоПоказателя = "Пр900";
	КонецЕсли;
...
Процедура РаспределениеДоПриИзменении(Элемент)
	...
	Если ИмяПоказателя = "Пр900" Тогда
		ИмяИзменяемогоПоказателя = "Пр91";
	КонецЕсли;
	
	Если ИмяПоказателя = "Пр910" Тогда
		ИмяИзменяемогоПоказателя = "Пр10";
	КонецЕсли;
...
мСоставПоказателей = "СоставПоказателей2017Кв";
