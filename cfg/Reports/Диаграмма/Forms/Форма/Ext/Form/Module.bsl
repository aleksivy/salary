...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Если ТаблицаИсходныхДанных.Количество() = 0 И ТаблицаИсходныхДанных.Колонки.Количество() = 0 Тогда
		ТекстВопроса = НСтр("ru='Нет данных для построения диаграммы. Отчет используется в качестве расширения представления других отчетов."
"Для построения диаграммы можно воспользоваться отчетом ""Диаграмма универсальная"". Открыть универсальную диаграмму?';uk='Немає даних для побудови діаграми. Звіт використовується в якості розширення представлення інших звітів."
"Для побудови діаграми можна скористатися звітом ""Діаграма універсальна"". Відкрити універсальну діаграму?'");
...
Процедура ПриЗакрытии()
	...
	Если ТипЗнч(ЭтаФорма.ВладелецФормы) = Тип("Форма") И ЭтаФорма.ВладелецФормы.ЭтотОбъект.Метаданные().Реквизиты.Найти("НастройкиДиаграммы") <> Неопределено Тогда
