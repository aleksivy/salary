...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ФиктивнаяСтрокаГруппировка = КН.Настройки.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
	...
	Если Режим = "Отбор" Тогда
		...
		Для каждого РодительскоеПоле Из РодителиПоля Цикл
			...
			Ограничение = ЭлементыФормы.Отбор.ОграниченияИспользования.Добавить();
			Ограничение.Доступность = Ложь;
			Ограничение.Поле = Поле;
		КонецЦикла;
	ИначеЕсли Режим = "Порядок" Тогда
		ЭлементыФормы.ПанельОсновная.ТекущаяСтраница = ЭлементыФормы.ПанельОсновная.Страницы.Порядок;
		Заголовок = "Выбор поля порядка";
		Для каждого РодительскоеПоле Из РодителиПоля Цикл
			Поле = Новый ПолеКомпоновкиДанных(РодительскоеПоле.Поле);
			Ограничение = ЭлементыФормы.Порядок.ОграниченияИспользования.Добавить();
			Ограничение.Доступность = Ложь;
			Ограничение.Поле = Поле;
		КонецЦикла;
	ИначеЕсли Режим = "Группировка" Тогда
		...
		Для каждого РодительскоеПоле Из РодителиПоля Цикл
			...
			Ограничение.Поле = Поле;
		КонецЦикла;
	ИначеЕсли Режим = "ВыбранныеПоля" Тогда
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	КН.Настройки.Структура.Удалить(ФиктивнаяСтрокаГруппировка);
...
Функция ВыборПоля()
	...
	ИначеЕсли Режим = "Порядок" Тогда
		ТекущиеДанные = ЭлементыФормы.Порядок.ТекущиеДанные;
	ИначеЕсли Режим = "ВыбранныеПоля" Тогда
	...
	Если Режим = "ЧислоИСтрока" ИЛИ Режим = "Дата" Тогда
		ТипЗначенияПоля = КН.Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ТекущиеДанные.Поле)).Тип;
...
Процедура ДоступныеПоляПриПолученииДанных(Элемент, ОформленияСтрок)
	...
	Для Каждого ОформлениеСтроки Из ОформленияСтрок Цикл
		ТипЗначенияПоля = КН.Настройки.ДоступныеПоляВыбора.НайтиПоле(Новый ПолеКомпоновкиДанных(ОформлениеСтроки.ДанныеСтроки.Поле)).Тип;
		Если Режим = "ЧислоИСтрока" Тогда
			Если ТипЗначенияПоля.Типы().Найти(Тип("Строка")) <> Неопределено ИЛИ ТипЗначенияПоля.Типы().Найти(Тип("Число")) <> Неопределено Тогда
