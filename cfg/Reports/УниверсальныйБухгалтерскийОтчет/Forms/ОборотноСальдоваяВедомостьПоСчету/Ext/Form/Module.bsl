...
Процедура ПередВыводомЭлементаРезультата(МакетКомпоновки, ДанныеРасшифровки, ЭлементРезультата, Отказ = Ложь) Экспорт
	...
	Попытка
		Если ТипЗнч(ЭлементРезультата.ЗначенияПараметров.П1.Значение) = Тип("Строка") Тогда
			Если Найти(Счет.Код, ЭлементРезультата.ЗначенияПараметров.П1.Значение) > 0 Тогда
				Отказ = истина;
			КонецЕсли;
		КонецЕсли;
	Исключение
...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	Для Каждого ПолеГруппировки Из ДанныеОтчета.ДополнительныеПоля Цикл 
		...
		КонецЕсли;
	КонецЦикла;
	КН.Настройки.ПараметрыВывода.Элементы.Найти("РасположениеРеквизитов").Значение = РасположениеРеквизитовКомпоновкиДанных.Отдельно;
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = ПредставлениеОтчета + " " + Счет + мПолучитьПредставлениеПериода(ЭтаФорма);
...
Процедура КоманднаяПанельСортировкаДобавитьЭлемент(Кнопка)
	...
	мДобавитьЭлементСортировки(ЭтаФорма);
...
ИдентификаторОбъекта  = "ОтчетОбъект.ОборотноСальдоваяВедомостьПоСчету";
ПредставлениеОтчета   = "Оборотно-сальдовая ведомость по счету";

СхемаКД = ПолучитьМакет("ОборотноСальдоваяВедомостьПоСчету");
КН.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКД));
