...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если мСтруктураВариантыЗаполнения.Свойство(ИмяСтраницыПанели, МассивВариантовЗаполненияСтраниц) Тогда
		...
		Иначе
			...
			МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2012Кв1");
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	Попытка
		...
		мОтчетФССУтратаТрудосп.Области.РС.Значение     = БанковскийСчетДляРасчетовСФСС.НомерСчета;
		мОтчетФССУтратаТрудосп.Области.Банк.Значение   = БанковскийСчетДляРасчетовСФСС.Банк;
		мОтчетФССУтратаТрудосп.Области.МФО.Значение    = БанковскийСчетДляРасчетовСФСС.Банк.Код;
	Исключение
	КонецПопытки;
	Если ЗначениеЗаполнено(СведенияОбОрганизации.ДатаРегистрации)
		И СведенияОбОрганизации.ДатаРегистрации >= '2011-01-01' Тогда
		мОтчетФССУтратаТрудосп.Области.DERZHREESTR.Значение = СведенияОбОрганизации.НомерРегистрации;
	КонецЕсли;
...
Процедура Расчет()
	...
	мОтчетФССУтратаТрудосп.Области.S121.Значение = мОтчетФССУтратаТрудосп.Области.S284.Значение;
												 
	//Всього
	//На початок періоду
	мОтчетФССУтратаТрудосп.Области.S109.Значение   = ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S101.Значение) 
	...
	мОтчетФССУтратаТрудосп.Области.S110.Значение   = ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S112.Значение) 
	                                               + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S113.Значение) 
	                                               + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S121.Значение) 
	                                               - ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S106.Значение); 
	
	мОтчетФССУтратаТрудосп.Области.S122.Значение   = ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S112.Значение) 
	                                               + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S113.Значение) 
												   + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S114.Значение) 
												   + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S121.Значение); 
												   
	мОтчетФССУтратаТрудосп.Области.S123.Значение   = ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S101.Значение) 
	                                               + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S102.Значение) 
	                                               + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S103.Значение) 
	                                               + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S104.Значение) 
	                                               + ОкруглитьЧислоПоФормату(мОтчетФССУтратаТрудосп.Области.S105.Значение) 
...
Процедура РассчитатьСуммыЗадолженностей()
	Если Не ЭтоЗУП Тогда
		...
		УстановитьЗначениеПоказателя("S112",  мБухИтогиОст.ПолучитьИтог("СуммаНачальныйОстатокДт", "Счет",СчетФСС) 
	...
	Иначе
		...
		|ВЫБРАТЬ
		|	ЕстьNULL(СУММА(ВзаимоРасчеты.РезультатОстаток), 0)	КАК Результат
		|
		|ИЗ
		...
		Если РезультатЗапроса.Следующий() Тогда
			ОстатокСтарый = РезультатЗапроса.Результат;
		КонецЕсли;
		...
		Запрос.Текст = "ВЫБРАТЬ
						|	ЕстьNULL(ВзаиморасчетыПоНачислениямЗаСчетФССОстатки.СуммаОстаток, 0) КАК Остаток
						|ИЗ
						|	РегистрНакопления.ВзаиморасчетыПоНачислениямЗаСчетФСС.Остатки(&НачалоПериода, Организация = &Организация И СтатьяРасчетовСФСС.Родитель = &СтатьяДекларации) КАК ВзаиморасчетыПоНачислениямЗаСчетФССОстатки
						|";
		
		Выборка = Запрос.Выполнить().Выбрать();
		ОстатокЗаСчетФСС = 0;
		Если Выборка.Следующий() Тогда
			ОстатокЗаСчетФСС = - Выборка.Остаток;
		КонецЕсли;
		
		
		Если ОстатокЗаСчетФСС + ОстатокСтарый > 0 Тогда
			S101 = ОстатокЗаСчетФСС + ОстатокСтарый;
			S112 = 0;
		Иначе
			S101 = 0;
			S112 = - (ОстатокЗаСчетФСС + ОстатокСтарый);
		КонецЕсли;
		
		УстановитьЗначениеПоказателя("S101",S101);
		УстановитьЗначениеПоказателя("S112",S112);
...
Процедура РассчитатьЗачтенныеПеречисленныеИПрочиеСуммы()	
	...
	Если Не ЭтоЗУП Тогда
		...
		УстановитьЗначениеПоказателя("S103", S103_ошибки_самост + S103_ошибки_пеня);
		...
		УстановитьЗначениеПоказателя("S104", S104_Проверки_акты + S104_Проверки_штарфы + S104_Проверки_Пеня);									
											
		////////не принято к зачету
		//////НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_НеПринятоКЗачет;
		//////УстановитьЗначениеПоказателя("???", мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС, НужнаяСтатья)
		//////									+мБухИтоги.ПолучитьИтог("СуммаОборотКт", "Счет, Субконто1", СчетФСС378, НужнаяСтатья));
		...
		УстановитьЗначениеПоказателя("S114", Перечислено);
		...
		Для каждого ВидПеречисления Из СуммыПеречислений Цикл
			УстановитьЗначениеПоказателя(ВидПеречисления.Ключ, ВидПеречисления.Значение);
		КонецЦикла;
		...
		//получено от фонда на начало квартала
		УстановитьЗначениеПоказателя("S106", ПолученоОтФонда);
	...
	Иначе
		...
		Если Выборка.Следующий() Тогда
			...
			УстановитьЗначениеПоказателя("S106", Выборка.ПолученоОтФСС);
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьНачальныеНастройки("СоставПоказателей2012Кв1");
