...
Перем СчетФСС, СчетРасчСчет1, СчетРасчСчет2, СчетЗП, СчетПрочихНачислений, СчетПФ;
...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если мСтруктураВариантыЗаполнения.Свойство(ИмяСтраницыПанели, МассивВариантовЗаполненияСтраниц) Тогда
		...
		Иначе
			...
			МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2006Кв1");
...
Процедура Расчет()
   ...
   мОтчетФССУтратаТрудосп.Области.S101.Значение  = мОтчетФССУтратаТрудосп.Области.S102.Значение
	...
	мОтчетФССУтратаТрудосп.Области.S72.Значение = мОтчетФССУтратаТрудосп.Области.S61.Значение 
	                                             ...
	                                             + мОтчетФССУтратаТрудосп.Области.S71.Значение 
	                                             + мОтчетФССУтратаТрудосп.Области.S105.Значение 
	...
	мОтчетФССУтратаТрудосп.Области.S94.Значение = мОтчетФССУтратаТрудосп.Области.S140.Значение							
	                                            + мОтчетФССУтратаТрудосп.Области.S141.Значение
												+ мОтчетФССУтратаТрудосп.Области.S142.Значение;
...
Процедура РассчитатьСуммыЗадолженностей()
	Если Не ЭтоЗУП Тогда
		...
		S185 = мБухИтогиОст.ПолучитьИтог("СуммаНачальныйОстатокКт", "Счет",СчетЗП); 
	...
	Иначе
		...
		ЗапросЗадолжПоЗП = Новый Запрос;
		ЗапросЗадолжПоЗП.Текст = "
		|Выбрать
		|	СуммаВзаиморасчетовОстаток		КАК Результат
		|
		|Из
		|	РегистрНакопления.ВзаиморасчетыСРаботникамиОрганизаций.Остатки(&НачалоГода, Организация = &Организация)
		|
		|";
		
		ЗапросЗадолжПоЗП.УстановитьПараметр("НачалоГода", НачалоГода(мДатаНачалаПериодаОтчета));
		ЗапросЗадолжПоЗП.УстановитьПараметр("Организация", Организация);
		
		
		РезультатЗапроса = ЗапросЗадолжПоЗП.Выполнить().Выбрать();
		Если РезультатЗапроса.Следующий() Тогда
			S185 = РезультатЗапроса.Результат;	
		КонецЕсли;
		УстановитьЗначениеПоказателя("S185", S185);
	...
	ЗапросСуммаЗадолжКонец.УстановитьПараметр("КонДата", КонецКвартала(мДатаКонцаПериодаОтчета));
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		S186 = ТаблицаРезультатовЗадолжЗПКон[0].Результат;
...
Процедура РассчитатьЗачтенныеПеречисленныеИПрочиеСуммы()	
	Если Не ЭтоЗУП Тогда
		...
		S30 = мБухИтоги.ПолучитьИтог("СуммаКорОборотКт", "Счет, КорСчет", СчетФСС, СчетРасчСчет1)
		+ мБухИтоги.ПолучитьИтог("СуммаКорОборотКт", "Счет, КорСчет", СчетФСС, СчетРасчСчет2);
		УстановитьЗначениеПоказателя("S30", S30);
		...
		УстановитьЗначениеПоказателя("S275", МассивПоВзносамПеречислЗачтено[0][0]);
...
Процедура  РассчитатьСуммыРасходовПоСтрахованию()
	...
	Если Не ЭтоЗУП Тогда
		...
		НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_УходРебенок;
		СуммаВыплаты = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто2,КорСчет, ", СчетФСС, НужнаяСтатья,СчетЗП)
		+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто2,КорСчет", СчетФСС, НужнаяСтатья,СчетПрочихНачислений);
		
		S71 = СуммаВыплаты;
		УстановитьЗначениеПоказателя("S71", S71);
		
		НужнаяСтатья = СтрокиДекларации.ПФ_Декр;
		S105 = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто2,КорСчет", СчетФСС, НужнаяСтатья,СчетПФ);
		УстановитьЗначениеПоказателя("S105", S105);
		
		// при рождении ребенка
		Попытка
			НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_Рождение_Единораз;
			S69 = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто2,КорСчет", СчетФСС, НужнаяСтатья,СчетЗП)
			+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто2,КорСчет", СчетФСС, НужнаяСтатья,СчетПрочихНачислений);
			УстановитьЗначениеПоказателя("S69", S69);
			
			НужнаяСтатья = СтрокиДекларации.ФССУтрТрудосп_Рождение_Ежемес;
			S284 = мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто2,КорСчет", СчетФСС, НужнаяСтатья,СчетЗП)
			+ мБухИтоги.ПолучитьИтог("СуммаКорОборотДт", "Счет, Субконто2,КорСчет", СчетФСС, НужнаяСтатья,СчетПрочихНачислений);
			УстановитьЗначениеПоказателя("S284", S284);
		Исключение
	...
	Иначе
		...
		Пока РезультатЗапроса.Следующий() Цикл
			...
			ИначеЕсли(РезультатЗапроса.СубконтоДт2 = СтрокиДекларации.ФССУтрТрудосп_Берем) Тогда
				...
				// по уходу за ребенком	
			ИначеЕсли(РезультатЗапроса.СубконтоДт2 = СтрокиДекларации.ФССУтрТрудосп_УходРебенок) Тогда
				СуммаВыплаты = РезультатЗапроса.Результат;
				S71 = РезультатЗапроса.Результат;
				// при рождении ребенка единоразово	
			ИначеЕсли(РезультатЗапроса.СубконтоДт2 = СтрокиДекларации.ФССУтрТрудосп_Рождение_Единораз) Тогда
				S69 = РезультатЗапроса.Результат;
				// при рождении ребенка	ежемесячно
			ИначеЕсли(РезультатЗапроса.СубконтоДт2 = СтрокиДекларации.ФССУтрТрудосп_Рождение_Ежемес) Тогда
				S284 = РезультатЗапроса.Результат;
				// на погребение	
			...
			ИначеЕсли(РезультатЗапроса.СубконтоДт2 =СтрокиДекларации.ФССУтрТрудосп_Другое) Тогда
				...
				// взносы в ПФ за мат. помощь по уходу за ребенком	
			ИначеЕсли(РезультатЗапроса.СубконтоДт2 = СтрокиДекларации.ПФ_Декр) Тогда
				S105 = РезультатЗапроса.Результат;	
			КонецЕсли;
		КонецЦикла;	
...
Процедура ЗаполнитьАвто()  Экспорт
   ...
   УстановитьЗначениеПоказателя("S2", S2);
	...
	Если ЭтоЗУП  Тогда
		...
		|СУММА(НеподтвержденнаяСуммаРасход)	    КАК УплаченоНеПодтверждено
		|";
		

//	Иначе
//		ДатаНачалаВЗапрос = мДатаНачалаПериодаОтчета;
	КонецЕсли;
			...
			Если (НайденныеСтроки.Количество() = 1 ) Тогда
				...
				Если (Месяц(ТекМесяц) < Месяц(мДатаНачалаПериодаОтчета)) Тогда
					...
					Если ЭтоЗУП Тогда
						...
						+ НайденныеСтроки[0].УплаченоНеПодтверждено;
					КонецЕсли;	
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьНачальныеНастройки("СоставПоказателей2006Кв1");
