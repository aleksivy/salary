...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
	 	мОтчетОВакансиях.Области["M" + Формат(НомерСтроки,"ЧЦ=4;ЧВН=0;ЧГ=;") + "G15"].Значение = ФИОИсполнителя;
	КонецЦикла;
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьНачальныеНастройки("СоставПоказателей2013");
	...
	Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл                    
		...
		Если НЕ НомерПроставлен Тогда	
			...
			ИначеЕсли ИмяГруппы = мИдГруппы2 Тогда
				ОбластьТекстовыйНомер = ТекТабличноеПоле.Область(ОбластьЯчейки.Верх + 2,10,ОбластьЯчейки.Верх + 2,13);
				ОбластьТекстовыйНомер.Текст = Строка(КолСтрок + 1);
			КонецЕсли;
			...
			Для НСтр = НомУдаляемойСтроки + 1 По СтруктураГруппы.Количество() Цикл
				...
				Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
					...
					Если НЕ НомерПроставлен Тогда
						...
						ИначеЕсли ИмяГруппы = мИдГруппы2 Тогда
							// это колонка MG_
							ОбластьТекстовыйНомер = ТекТабличноеПоле.Область(ОбластьСдвинутойЯчейки.Верх+2,10,ОбластьСдвинутойЯчейки.Верх+2,13);
							ОбластьТекстовыйНомер.Текст = Строка(НСтрНов);
						КонецЕсли;					
					КонецЕсли;
...
Процедура ПолеТабличногоДокументаОтчетОВакансияхВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если     (Лев(ИмяОбласти, 1) = "R" И Прав(ИмяОбласти, 2) = "G5") 
		 ИЛИ (Лев(ИмяОбласти, 1) = "M" И Лев(Прав(ИмяОбласти, 4), 2) = "G4") 
		 ИЛИ (Лев(ИмяОбласти, 1) = "M" И Лев(Прав(ИмяОбласти, 4), 2) = "G6") 
		 ИЛИ (Лев(ИмяОбласти, 1) = "M" И Лев(Прав(ИмяОбласти, 4), 2) = "G7") 
		 ИЛИ (Лев(ИмяОбласти, 1) = "M" И Лев(Прав(ИмяОбласти, 4), 2) = "G8") 
		 ИЛИ (Лев(ИмяОбласти, 1) = "M" И Лев(Прав(ИмяОбласти, 4), 2) = "G9") 
		 ИЛИ (Лев(ИмяОбласти, 1) = "M" И Лев(Прав(ИмяОбласти, 5), 3) = "G10") 
		 ИЛИ (Лев(ИмяОбласти, 1) = "M" И Лев(Прав(ИмяОбласти, 5), 3) = "G11") Тогда
		
		мОтчетОВакансиях.Области[ИмяОбласти].Значение = НЕ мОтчетОВакансиях.Области[ИмяОбласти].Значение;
		
		Если (Лев(ИмяОбласти, 1) = "R" И Прав(ИмяОбласти, 2) = "G5") Тогда
			СинхронизацияРазделов1и2();
		КонецЕсли;
...
Процедура СинхронизацияРазделов1и2(ПересчитыватьКоличествоСтраниц = Истина)
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		...
		Должность 		   = мОтчетОВакансиях.Области["R" + Формат(НомерСтроки,"ЧЦ=4;ЧВН=0;ЧГ=;") + "G1"].Значение;
		КоличествоВакансий = мОтчетОВакансиях.Области["R" + Формат(НомерСтроки,"ЧЦ=4;ЧВН=0;ЧГ=;") + "G2"].Значение;
		
		ВыводитьДопРаздел  = мОтчетОВакансиях.Области["R" + Формат(НомерСтроки,"ЧЦ=4;ЧВН=0;ЧГ=;") + "G5"].Значение;
		
		мОтчетОВакансиях.Области["M" + Формат(НомерСтроки,"ЧЦ=4;ЧВН=0;ЧГ=;") + "G1"].Значение = Должность;
...
СтруктураГруппы1.Колонки.Добавить("RG5", ОписаниеТиповБулево);

СтруктураГруппы2 = Новый ТаблицаЗначений;
СтруктураГруппы2.Колонки.Добавить("MG_");
СтруктураГруппы2.Колонки.Добавить("MG1",  ОписаниеТиповСтрока);
СтруктураГруппы2.Колонки.Добавить("MG1_1",ОписаниеТиповСтрока);
...
СтруктураГруппы2.Колонки.Добавить("MG7_6", ОписаниеТиповБулево);
...
СтруктураГруппы2.Колонки.Добавить("MG10_5", ОписаниеТиповБулево);
СтруктураГруппы2.Колонки.Добавить("MG10_6", ОписаниеТиповБулево);
