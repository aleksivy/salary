...
Перем Счет661, СчетФССНесч, СчетРасчСчет1, СчетРасчСчет2, СчетРасчСчет3, СчетРасчСчет4;
...
Процедура ФормироватьСтруктуруСтраницОтчета() Экспорт
	...
	СтрокаУровня1.Представление            = НСтр("ru = 'Отчет в ФСС от несчастных случаев'; uk = 'Звіт в ФСС від несчастних випадків'");
...
Функция УстановитьДатуПодписи()
	...
	Если ЧастиДатыПодписи.Свойство("ДвеПоследниеЦифрыГода") Тогда
		мОтчетФССНесчастныйСлучай.Области.ГодДаты.Значение 	= ЧастиДатыПодписи.ДвеПоследниеЦифрыГода;
	Иначе
		мОтчетФССНесчастныйСлучай.Области.ГодДаты.Значение 	= Формат(ДатаПодписи, "ДФ=yy");
	Конецесли;
...
//	мОтчетФССНесчастныйСлучай.Области.КодЗКГНГ.Значение = КодЗКГНГ;
	...
	Если Не РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП" Тогда
		...
		Если НЕ УчетнаяПолитикаНеЗадана Тогда 
			...
			//СистНалОблож = ?(СхемаНалогообл.НалогНаПрибыль, "Загальна", "Спрощена");
			СистНалОблож = ?(СхемаНалогообл.НалогНаПрибыль, "Платник збору на загальних підставах", "Платник єдиного податку");
			мОтчетФССНесчастныйСлучай.Области.СистНалОбложения.Значение = СистНалОблож;
		КонецЕсли;
	Иначе
...
Процедура Расчет()
	...
	мОтчетФССНесчастныйСлучай.Области.ПИт1.Значение     = ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_1.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_4.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_7.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_10.Значение);
														
	мОтчетФССНесчастныйСлучай.Области.ПИт2.Значение     = ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_2.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_5.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_8.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_11.Значение);
														
	мОтчетФССНесчастныйСлучай.Области.ПИт3.Значение     = ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_3.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_6.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_9.Значение) 
	                                                    + ОкруглитьЧислоПоФормату(мОтчетФССНесчастныйСлучай.Области.Пмес_12.Значение);
...
Процедура РассчитатьСостояниеВзаиморасчетовСФондом()
	...
	ИначеЕсли (РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП")  Тогда
		...
		|ВЫБРАТЬ
		|	СУММА(РезультатОстаток)	КАК Результат
		|
		|ИЗ
		...
		ЗапросСуммаНаНачалоГода.УстановитьПараметр("НачДата", НачалоГода(мДатаНачалаПериодаОтчета));
...
Процедура РассчитатьСуммыЗадолженностиИФОТПоквартально()
	...
	|
	|Из
	|	РегистрНакопления.ВзаиморасчетыСРаботникамиОрганизаций.Остатки(&КонецКвартала,Организация = &Организация И ПериодВзаиморасчетов <> &ПоследнийМесяцКвартала) КАК ВзаиморасчетыСРаботниками
	|
	|";              
	...
	Для НомКв = 1 По Месяц(КонецКвартала(мДатаКонцаПериодаОтчета))/3 Цикл
		...
		// на первую секунду следующего квартала
		ЗапросЗадолжПоквартально.УстановитьПараметр("КонецКвартала",   			ДобавитьМесяц(НачалоГода(мДатаНачалаПериодаОтчета), НомКв*3));
		// первый день последнего месяца кваратала
		ЗапросЗадолжПоквартально.УстановитьПараметр("ПоследнийМесяцКвартала",   ДобавитьМесяц(НачалоГода(мДатаНачалаПериодаОтчета), НомКв*3 - 1));
		
		РезультатЗапроса = ЗапросЗадолжПоквартально.Выполнить().Выбрать();
		Если РезультатЗапроса.Следующий() Тогда
			МассивКварталов[1][НомКв - 1] = РезультатЗапроса.Результат;
		КонецЕсли;
	...
	ЗапросФОТ.Текст ="
	|Выбрать
	|	КОНЕЦПЕРИОДА(ПериодВзаиморасчетов,КВАРТАЛ) КАК Период,
	|	Сумма(СуммаВзаиморасчетовПриход) КАК Результат
	|
	|Из 
	|	РегистрНакопления.ВзаиморасчетыСРаботникамиОрганизаций.Обороты(&Начало, &Конец, , ((Организация = &Организация) И (СчетУчета = &парамСчет)) )
	|
	|СГРУППИРОВАТЬ ПО 
	|   КОНЕЦПЕРИОДА(ПериодВзаиморасчетов,КВАРТАЛ)
	|";
	...
	ЗапросФОТ.УстановитьПараметр("парамСчет",   Счет661);
	...
	Пока РезультатЗапрос.Следующий() Цикл
		// РезультатЗапрос.период = последний месяц квартала
		ИндексМассива = Месяц(РезультатЗапрос.Период)/3 - 1;
		МассивКварталов[2][ИндексМассива] = РезультатЗапрос.Результат;
	КонецЦикла;
	...
	Для Сч = 0 по 3 Цикл  
		...
		Если МассивКварталов[2][Сч] <> Неопределено Тогда
...
Процедура РассчитатьЗачтенныеПеречисленныеИПрочиеСуммы()
	...
	ИначеЕсли (РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП") Тогда
		...
		|ВЫБРАТЬ
		|	Взаиморасчеты.Период          КАК НалоговыйПериод,
		...
		Пока Результат.Следующий() Цикл
			...
			ИначеЕсли Месяц(Результат.НалоговыйПериод) = Месяц(мДатаКонцаПериодаОтчета)	 Тогда
				...
				П21_3 = Результат.УплаченоПодтверждено;
			...
			Иначе	
				...
				П21_2 = Результат.УплаченоПодтверждено;
...
Процедура ЗаполнитьОтчетФССНесчастныйСлучай()
	...
	Пока Истина Цикл
		Если КонДата <= мДатаКонцаПериодаОтчета Тогда
			ЧисленностьССЧ = ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачДата, КонДата, "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", Организация, "Штат");
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЭлементыФормы.ПолеТабличногоДокументаОтчетФССНесчастныйСлучай.ОбластьПечати = ЭлементыФормы.ПолеТабличногоДокументаОтчетФССНесчастныйСлучай.Области.Найти("ПредупреждениеОПечати");
...
Процедура ПолеТабличногоДокументаОтчетФССНесчастныйСлучайR9C45НачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
