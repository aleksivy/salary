...
Процедура ИзменитьПериод()
	...
	Если Месяц(НачПериода) = 1 Тогда 
		Мес = " Января"
	ИначеЕсли Месяц(НачПериода) = 2 Тогда
		Мес = " Февраля"
	ИначеЕсли Месяц(НачПериода) = 3 Тогда
		Мес = " Марта"	
	ИначеЕсли Месяц(НачПериода) = 4 Тогда
		Мес = " Апреля"	
	ИначеЕсли Месяц(НачПериода) = 5 Тогда
		Мес = " Мая"	
	ИначеЕсли Месяц(НачПериода) = 6 Тогда
		Мес = " Июня"	
	ИначеЕсли Месяц(НачПериода) = 7 Тогда
		Мес = " Июля"	
	ИначеЕсли Месяц(НачПериода) = 8 Тогда
		Мес = " Августа"	
	ИначеЕсли Месяц(НачПериода) = 9 Тогда
		Мес = " Сентября"	
	ИначеЕсли Месяц(НачПериода) = 10 Тогда
		Мес = " Октября"	
	ИначеЕсли Месяц(НачПериода) = 11 Тогда
		Мес = " Ноября"
	ИначеЕсли Месяц(НачПериода) = 12 Тогда
		Мес = " Декабря"
	КонецЕсли;
		
		
		
	Если НачПериода < '20110822' Тогда
	
		Если (День(НачПериода) <= 10) Тогда
			ЭлементыФормы.НадписьПериодСоставленияОтчета.Заголовок = "1 - 10 " + Мес + " " + Формат(Год(НачПериода), "ЧГ=");
			мДатаНачалаПериодаОтчета = Дата((Формат(Год(НачПериода),"ЧГ=0"))+ Строка(Формат(НачПериода,"ДФ=ММ")) + "01");
			мДатаКонцаПериодаОтчета  = КонецДня(Дата((Формат(Год(НачПериода),"ЧГ=0"))+ Строка(Формат(НачПериода,"ДФ=ММ")) + "10"));
		ИначеЕсли (День(НачПериода) <= 20) Тогда
			ЭлементыФормы.НадписьПериодСоставленияОтчета.Заголовок = "10 - 20 " + Мес + " " + Формат(Год(НачПериода), "ЧГ=");
			мДатаНачалаПериодаОтчета = Дата((Формат(Год(НачПериода),"ЧГ=0"))+ Строка(Формат(НачПериода,"ДФ=ММ")) + "11");
			мДатаКонцаПериодаОтчета  = КонецДня(Дата((Формат(Год(НачПериода),"ЧГ=0"))+ Строка(Формат(НачПериода,"ДФ=ММ")) + "20"));
		ИначеЕсли (День(НачПериода) >= 21) Тогда
			ЭлементыФормы.НадписьПериодСоставленияОтчета.Заголовок = "21 - " + День(КонецМесяца(НачПериода)) + Мес + " " + Формат(Год(НачПериода), "ЧГ=");
			мДатаНачалаПериодаОтчета = Дата((Формат(Год(НачПериода),"ЧГ=0"))+ Строка(Формат(НачПериода,"ДФ=ММ")) + "21");
			мДатаКонцаПериодаОтчета  = КонецДня(Дата((Формат(Год(НачПериода),"ЧГ=0"))+ Строка(Формат(НачПериода,"ДФ=ММ")) + Строка(День(КонецМесяца(НачПериода)))));
		КонецЕсли;
	...
	Иначе	
		...
		ЭлементыФормы.НадписьПериодСоставленияОтчета.Заголовок = Формат(мДатаНачалаПериодаОтчета,"ДФ=dd.MM.yyyy") + " - " + Формат(мДатаКонцаПериодаОтчета,"ДФ=dd.MM.yyyy") ;		
