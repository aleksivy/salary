...
Процедура ПоказатьПериод()
	...
	ДАТАОТЧЕТА = "на  "+ Формат(мДатаКонцаПериодаОтчета,"ДЛФ=ДД;Л = uk_UA");
	ДАТАОТЧЕТА = Лев(ДАТАОТЧЕТА, СтрДлина(ДАТАОТЧЕТА) -2) + " року";    // р. -> року
	мОтчетОПринятыхНаРаботу.Области.ДАТАОТЧЕТА.Значение = ДАТАОТЧЕТА;
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	мОтчетОПринятыхНаРаботу.Области.ФИОДиректора.Значение   = РегламентированнаяОтчетность.ПолучитьСведенияОбОрганизации(Организация, ДатаПодписи, 				СписокПоказателей).ФИОРук;		
	...
	мОтчетОПринятыхНаРаботу.Области.ФАКС.Значение 	    = УправлениеКонтактнойИнформацией.ПолучитьФаксИзКонтактнойИнформации(Организация);
	мОтчетОПринятыхНаРаботу.Области.ЭЛПОЧТА.Значение 	= УправлениеКонтактнойИнформацией.ПолучитьEmailИзКонтактнойИнформации(Организация);
...
Процедура Печать(ВидПечати)
	...
	ОбластьПечати = мОтчетОПринятыхНаРаботу.ПолучитьОбласть("ОбластьПечатиПоВысоте1");
	ПечатнаяФорма.Вывести(ОбластьПечати);
	
	ОбластьПечати = мОтчетОПринятыхНаРаботу.ПолучитьОбласть("ОбластьПечатиПоВысоте2");
	ПечатнаяФорма.Вывести(ОбластьПечати);// два раза выводим номера колонок, чтобы разрыв страницы выглядел хорошо.
	ПечатнаяФорма.Вывести(ОбластьПечати);
	
	ОбластьПечати = мОтчетОПринятыхНаРаботу.ПолучитьОбласть("ОбластьПечатиПоВысоте3");
	ПечатнаяФорма.Вывести(ОбластьПечати);
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьНачальныеНастройки("СоставПоказателей2011");
...
СтруктураГруппы1.Колонки.Добавить("RG6", ОписаниеТиповСтрока);
СтруктураГруппы1.Колонки.Добавить("RG6D", ОписаниеТиповДата);
СтруктураГруппы1.Колонки.Добавить("RG7",  ОписаниеТиповДата);
