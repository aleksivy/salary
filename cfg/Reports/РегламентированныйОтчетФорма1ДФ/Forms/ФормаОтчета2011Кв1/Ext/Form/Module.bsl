...
Функция ПолучитьНастройкиПроверкиЗаполенения(Макет = "СоставПоказателей2011кв1") Экспорт
...
Перем СтруктураГруппы;
	...
	Макет        = ПолучитьМакет("ПечатнаяФорма2011");
	...
	Если мФорма1ДФ.Области.R00G04.Значение <> 0 Тогда
		ОбластьМакета.Параметры.СумЗадолжГрн = Цел(мФорма1ДФ.Области.R00G04.Значение);
	  	КопейкиВСтроку(мФорма1ДФ.Области.R00G04.Значение, ОбластьМакета.Параметры.СумЗадолжКоп);
	Иначе
	  	ОбластьМакета.Параметры.СумЗадолжГрн = "" + "-"; 
	 	ОбластьМакета.Параметры.СумЗадолжКоп= "" + "-";
	КонецЕсли;
...
Процедура СохранитьВФорматеDBF()
	...
	xB.Поля.Добавить("OZN_DOX","N",3,0);
	...
	СтруктураМногострочныхРазделов.Свойство("T1R",СтруктураГруппы);
	...
	Для ТекСтрока = 1 По СтрокКол Цикл
		...
		xB.УстановитьЗначениеПоля("TIN",     СтруктураГруппы[ТекСтрока - 1]["T1RG02"]); 
		xB.УстановитьЗначениеПоля("S_NAR",   СтруктураГруппы[ТекСтрока - 1]["T1RG03A"]);
		xB.УстановитьЗначениеПоля("S_DOX",   СтруктураГруппы[ТекСтрока - 1]["T1RG03"]);
		xB.УстановитьЗначениеПоля("S_TAXN",  СтруктураГруппы[ТекСтрока - 1]["T1RG04A"]);
		xB.УстановитьЗначениеПоля("S_TAXP",  СтруктураГруппы[ТекСтрока - 1]["T1RG04"]);
		xB.УстановитьЗначениеПоля("OZN_DOX", СтруктураГруппы[ТекСтрока - 1]["T1RG05"]);
		xB.УстановитьЗначениеПоля("D_PRIYN", Дата(СтруктураГруппы[ТекСтрока - 1]["T1RG06D"]));
		xB.УстановитьЗначениеПоля("D_ZVILN", Дата(СтруктураГруппы[ТекСтрока - 1]["T1RG07D"]));
		xB.УстановитьЗначениеПоля("OZN_PILG",СтруктураГруппы[ТекСтрока - 1]["T1RG08"]);
		xB.УстановитьЗначениеПоля("OZNAKA",  СтруктураГруппы[ТекСтрока - 1]["T1RG09"]);
		xB.Записать();
	КонецЦикла;
...
Функция Сохранить()  Экспорт
	...
	СписокСохранения.Вставить("ПоказыватьВзносыФОТКакДоход133", ПоказыватьВзносыФОТКакДоход133);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("ПоказыватьВзносыФОТКакДоход133", ПоказыватьВзносыФОТКакДоход133);
...
Процедура ЗаполнитьАвто()  Экспорт
	...
	Иначе	
		...
		Если ПоказыватьВзносыФОТКакДоход133 Тогда
			Запрос.Текст = "
			|ВЫБРАТЬ
			|	ВзаиморасчетыПоВзносамВФонды.Сотрудник.ФизЛицо КАК Сотрудник,
			|	""133"" КАК ДоходНДФЛ,
			|	СУММА(ВзаиморасчетыПоВзносамВФонды.Результат) КАК ДоходПриход,
			|	СУММА(ВзаиморасчетыПоВзносамВФонды.Результат) КАК ДоходРасход,
			|	СУММА(0) КАК НалогПриход,
			|	СУММА(0) КАК НалогРасход
			|	ПОМЕСТИТЬ ТаблицаВзносыФОТ
			|ИЗ
			|	РегистрНакопления.ВзаиморасчетыПоВзносамВФонды КАК ВзаиморасчетыПоВзносамВФонды
			|	ГДЕ 
			|		 Период МЕЖДУ &ДатаНач И &ДатаКон
			|  	   И ВзаиморасчетыПоВзносамВФонды.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
			|  	   И Организация = &Организация
			|  	   И Налог.Родитель = ЗНАЧЕНИЕ(Справочник.Налоги.ВзносыФОТ)
	        |	   " + УсловиеНаПодразделение + "
			|
			|   СГРУППИРОВАТЬ ПО
			|		ВзаиморасчетыПоВзносамВФонды.Сотрудник.ФизЛицо
			|";
			Запрос.Выполнить();
		КонецЕсли;	
		...
		Если ПоказыватьВзносыФОТКакДоход133 Тогда
			...
			|
			|ВЫБРАТЬ 
			|	ТаблицаВзносыФОТ.Сотрудник КАК Сотрудник,
			|	ТаблицаВзносыФОТ.Сотрудник.КодПоДРФО КАК ИННСотрудника,
			|	ТаблицаВзносыФОТ.ДоходНДФЛ КАК ДоходНДФЛ,
			|	ЗНАЧЕНИЕ(Справочник.ВидыЛьготРаботниковОрганизаций.ПустаяСсылка)  КАК Льгота,
			|	ТаблицаВзносыФОТ.ДоходПриход КАК ДоходПриход,
			|	ТаблицаВзносыФОТ.ДоходРасход КАК ДоходРасход,
			|	ТаблицаВзносыФОТ.НалогПриход КАК НалогПриход,
			|	ТаблицаВзносыФОТ.НалогРасход КАК НалогРасход
			|ИЗ ТаблицаВзносыФОТ КАК ТаблицаВзносыФОТ
			|";
		КонецЕсли;		
	...
	Если мДатаНачалаПериодаОтчета = '20110101' Тогда
		...
		Запрос.УстановитьПараметр("НеВключатьЧПников", НеВключатьЧПников);
		Запрос.УстановитьПараметр("ДатаКонцаКвартала", мДатаКонцаПериодаОтчета);
		Запрос.УстановитьПараметр("ДатаНачалаКваратала",мДатаНачалаПериодаОтчета);
		Запрос.УстановитьПараметр("Декабрь2010",'20101201');
		
		
		Запрос.Текст = "ВЫБРАТЬ
		               |	ЕСТЬNULL(СУММА(ВзаиморасчетыПоНДФЛОстатки.НалогОстаток), 0) КАК ЗадолженностьПоНалогу
		               |ИЗ
		               |	РегистрНакопления.ВзаиморасчетыПоНДФЛ.Остатки(&ДатаНачалаКваратала, Организация = &Организация) КАК ВзаиморасчетыПоНДФЛОстатки
		               |ГДЕ
		               |	НАЧАЛОПЕРИОДА(ВзаиморасчетыПоНДФЛОстатки.ПериодВзаиморасчетов, МЕСЯЦ) <> &Декабрь2010
					   |	   " + УсловиеНаПодразделение + "
					   |";
		...
		Если Выборка.Следующий() Тогда
			Если Выборка.ЗадолженностьПоНалогу > 0 Тогда
				R00G04 = Выборка.ЗадолженностьПоНалогу;
			КонецЕсли;
		КонецЕсли;			
		
	КонецЕсли;
	мФорма1ДФ.Области.R00G04.Значение = R00G04;
	...
	Для Каждого стр ИЗ ТаблицаНДФЛ Цикл
		...
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[0].Имя,4);
		// номер строки
		
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[1].Имя,4);
		// область первой ячейки последней строки многострочного раздела
		...
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + "GFIO";
		ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
		ОбластьЯчейки.Значение = стр.Сотрудник;
		
		
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[2].Имя,4);
		ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
		ОбластьЯчейки.Значение = стр.ДоходПриход;
		
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[3].Имя,4);
		ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
		ОбластьЯчейки.Значение = стр.ДоходРасход;
		
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[4].Имя,4);
		ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
		ОбластьЯчейки.Значение = стр.НалогПриход;
		
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[5].Имя,4);
		ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
		ОбластьЯчейки.Значение = стр.НалогРасход;
		
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[6].Имя,4);
		ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
		ОбластьЯчейки.Значение = стр.ДоходНДФЛ;
		
		
		ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[9].Имя,4);
		ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
		Льгота = "";
				...
				Для каждого ЗаписьПриемУвольнение Из ПриемУвольнение Цикл
					...
					Если НЕ РаботалВПредыдущемПериоде 
						...
						Если ДатаПриема > '00010101' ИЛИ ДатаУвольнения > '00010101' Тогда
							...
							ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[7].Имя,4);
							ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
							ОбластьЯчейки.Значение = ДатаПриема;
							
							ИмяНужнойЯчейкиНовойСтроки = "T1R" + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[8].Имя,4);
							ОбластьЯчейки = мФорма1ДФ.Области[ИмяНужнойЯчейкиНовойСтроки];
	...
	Если мСохраненныйДок = Неопределено Тогда
		...
		Иначе
			...
			Если мПоказыватьВзносыФОТКакДоход133 = Неопределено Тогда
				ПоказыватьВзносыФОТКакДоход133 = Ложь;
			Иначе
...
спПараметрыСтраницы.Вставить("ВысотаПодвала",27);
спПараметрыСтраницы.Вставить("КоличествоСтрокТаблицыНаЕдинстеннойСтранице",12);
