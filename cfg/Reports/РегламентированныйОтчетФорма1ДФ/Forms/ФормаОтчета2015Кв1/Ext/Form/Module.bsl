...
Функция ПолучитьНастройкиПроверкиЗаполенения(Макет = "СоставПоказателей2015кв1") Экспорт
...
Процедура ПроверитьЗаполнениеИНН(ТаблицаНезаполненныхПоказателей, ДанныеОтчета, ОбработкаЭкспортXML)
	...
	Пока Истина Цикл
		...
		ИмяПоказателя = "T1R" + Формат(НомерСтроки , "ЧЦ=4; ЧВН=; ЧГ=0") + "G02";
		ЗначениеПоказателя = "";
...
Функция ОбновитьСлужебныеДанные()  
	...
	мСлужебныеДанные.Области.HZKV.Значение = Месяц(КонецКвартала(мДатаКонцаПериодаОтчета))/3;
...
Процедура ПоказатьПериод()
	...
	мФорма1ДФ.Области.ПериодОтчетаКвартал.Значение = Месяц(КонецКвартала(мДатаКонцаПериодаОтчета))/3;
	мФорма1ДФ.Области.ПериодОтчетаГод.Значение 	   = Год(мДатаКонцаПериодаОтчета);
...
Процедура ВставитьДополнительнуюСтроку()
	...
	ИмяПервойЯчейкиПоследнейСтроки = Сред(СтруктураГруппы.Колонки[0].Имя,1,3) + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[0].Имя,4,3);
	...
	Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
		...
		ИмяОбластиЯчейки    = Сред (ИмяКолонки,1,3) + Формат(КолСтрок, "ЧЦ=4; ЧВН=; ЧГ=0")    + Сред (ИмяКолонки,4,СтрДлина(ИмяКолонки)-3);
		ИмяНовОбластиЯчейки = Сред (ИмяКолонки,1,3) + Формат(КолСтрокНов, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (ИмяКолонки,4,СтрДлина(ИмяКолонки)-3);
	...
	ИмяТекущейОбласти = Сред (СтруктураГруппы.Колонки[0].Имя,1,3) + Формат(КолСтрокНов, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (СтруктураГруппы.Колонки[0].Имя,4,3);
...
"(Увага! дана операція необоротна)'"), РежимДиалогаВопрос.ДаНетОтмена, 60, КодВозвратаДиалога.Да);
	...
	Если СтруктураГруппы.Количество() > ИсходноеКоличествоСтрокГруппы Тогда
		...
		ИмяПервойЯчейкиУдаляемойСтроки = "T1R" + Формат(НомУдаляемойСтроки , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[0].Имя,4);
		...
		Если КолвоСдвигаемыхСтрок > 0 Тогда
			Для НСтр = НомУдаляемойСтроки + 1 По СтруктураГруппы.Количество() Цикл
				...
				НСтрНов = НСтр - 1;
				Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
					ИмяСдвинутойЯчейки      = "T1R" + Формат(НСтр ,    "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (Колонка.Имя,4);
					НовоеИмяСдвинутойЯчейки = "T1R" + Формат(НСтрНов , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (Колонка.Имя,4);
	...
	Иначе
		...
		Для Каждого СтрокаТаблицы Из СтруктураГруппы Цикл
			Для Каждого КолонкаТаблицы Из СтруктураГруппы.Колонки Цикл
				...
				ИмяЯчейки    = "T1R" + Формат(НомерСтрокиВФормеТаблицы , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(КолонкаТаблицы.Имя,4);
	...
	ИмяТекущейОбласти = "T1R" + Формат(НомТекущейСтроки , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (СтруктураГруппы.Колонки[0].Имя,4);
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	мФорма1ДФ.Области.R00G03I.Значение = 1;	
	...
	ИННРук       		= РегламентированнаяОтчетность.ПолучитьСведенияОбОрганизации(Организация, ДатаПодписи, СписокПоказателей).ИННРук;
	ИННБух       		= РегламентированнаяОтчетность.ПолучитьСведенияОбОрганизации(Организация, ДатаПодписи, СписокПоказателей).ИННБух;
	...
	Если (НалоговаяИнспекция <> Неопределено) Тогда 
		...
		Иначе	
			...
			мФорма1ДФ.Области.HSTI.Значение  = НалоговаяИнспекция_Наименование;
		КонецЕсли;	
	...
	мФорма1ДФ.Области.HLOC.Значение  = УправлениеКонтактнойИнформацией.ПолучитьАдресИзКонтактнойИнформации(Организация,"Юридический");
...
"Виконується відновлення користувальницьких настройок.'"), СтатусСообщения.Информация);
	...
	Если ВерсияФормы = "01/01/2011" Тогда
		...
		Если СписокСохранения.Свойство( "ПоказателиОтчета", ПоказателиОтчета) Тогда
			Для Каждого ЭлементСтруктуры Из ПоказателиОтчета Цикл
				...
				Если ИмяГруппы = "ПолеТабличногоДокументаФорма1ДФ" Тогда
					СписокПоказателей.Вставить("R00G03I", 1);
				КонецЕсли;
		...
		СписокСохранения.ВерсияФормы = "13/04/2011";
	    Модифицированность = Истина;
		
	КонецЕсли;
	
	//ВерсияФормы = СписокСохранения.ВерсияФормы;
	//Если ВерсияФормы = "13/04/2011" Тогда
	//
...
Функция ОбновитьВариантыЗаполнения()
	...
	МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2015кв1");
...
Процедура ВывестиРазделВТабличныйДокумент(ИмяГруппы, СтруктураГруппы)
	...
	Если ТекущееКоличествоСтрокГруппы > ИсходноеКоличествоСтрокГруппы Тогда
		...
		Для НомНовойСтроки = ИсходноеКоличествоСтрокГруппы + 1 По ТекущееКоличествоСтрокГруппы Цикл
			...
			ИмяПервойЯчейкиПоследнейСтроки = "T1R" + Формат(НомИсходнойСтроки , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[0].Имя,4);
			...
			Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
				ИмяКолонки          = Колонка.Имя;
				ИмяОбластиЯчейки    = "T1R" + Формат(НомИсходнойСтроки , "ЧЦ=4; ЧВН=; ЧГ=0")  + Сред (ИмяКолонки,4);
				ИмяНовОбластиЯчейки = "T1R" + Формат(НомНовойСтроки , "ЧЦ=4; ЧВН=; ЧГ=0")     + Сред (ИмяКолонки,4);
				ОбластьЯчейки = ТекТабличноеПоле.Области[ИмяОбластиЯчейки];
				ОбластьЯчейки.Имя = ИмяНовОбластиЯчейки;
				// определяем границы ячейки исходной области
				НомерВерхСтрокиЯчейки  = ОбластьЯчейки.Верх - СдвигСтрок;
...
//  выводим значения строк
	Для Каждого СтрокаТаблицы Из СтруктураГруппы Цикл
		Для Каждого КолонкаТаблицы Из СтруктураГруппы.Колонки Цикл
			...
			ИмяЯчейки    = "T1R" + Формат(НомерСтрокиВФормеТаблицы , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (ИмяКолонки,4);
			ОбластьЯчейкиТаблицы     = ТекТабличноеПоле.Области[ИмяЯчейки];
		...
		Если ТекущееКоличествоСтрок > ИсходноеКоличествоСтрокГруппы Тогда
			...
			Пока НСтр > ИсходноеКоличествоСтрокГруппы Цикл
				...
				Если (ТекТабличноеПоле = мФорма1ДФ) Тогда
					ИмяПервойЯчейкиУдаляемойСтроки = Сред(СтруктураГруппы.Колонки[0].Имя,1,3) + Формат(НСтр,"ЧЦ=4;ЧВН=0;ЧГ=;") + Сред(СтруктураГруппы.Колонки[0].Имя,4);
				КонецЕсли;
...
Перем СтруктураГруппы;
	...
	ОбластьМакета.Параметры.ОргЮрАдрес = мФорма1ДФ.Области.HLOC.Значение;
	...
	ЗначениеВКлетку(ФОрмат(мФорма1ДФ.Области.ПериодОтчетаКвартал.Значение,"ЧГ=0"),0, 1 ,"ПерКв",ОбластьМакета);
	ЗначениеВКлетку(ФОрмат(мФорма1ДФ.Области.ПериодОтчетаГод.Значение,"ЧГ=0"),0, 4 ,"ПерГод",ОбластьМакета);
	
	Если мФорма1ДФ.Области.R00G01I.Значение <> 0 Тогда
		ЗначениеВКлетку(ФОрмат(мФорма1ДФ.Области.R00G01I.Значение,"ЧГ=0"),0, 7 ,"Шт",ОбластьМакета);
	Иначе
		ОбластьМакета.Параметры.Шт7= "-";
	КонецЕсли;
	
	Если мФорма1ДФ.Области.R00G02I.Значение <> 0 Тогда
		ЗначениеВКлетку(ФОрмат(мФорма1ДФ.Области.R00G02I.Значение,"ЧГ=0"),0, 7 ,"свм",ОбластьМакета);
	Иначе
		ОбластьМакета.Параметры.свм7= "-";
	КонецЕсли;
	
	Если мФорма1ДФ.Области.R00G03I.Значение <> 0 Тогда
		ЗначениеВКлетку(ФОрмат(мФорма1ДФ.Области.R00G03I.Значение,"ЧГ=0"),0, 4 ,"Порция",ОбластьМакета);
	Иначе
  ...
  Для НСтр = 1 По СтруктураГруппы.Количество() Цикл
	  ...
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[1].Имя,4);
	  // область первой ячейки последней строки многострочного раздела
	  ИНН = Формат(мФорма1ДФ.Области[ИмяНужнойЯчейки].Значение, "ЧГ=");
	  Для Инд = 1 По 10 Цикл
		  ОбластьМакета.Параметры["ИНН" + Инд] = Сред(ИНН, Инд, 1);
	  КонецЦикла; 
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[2].Имя,4);
	  ...
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[3].Имя,4);
	  Если (мФорма1ДФ.Области[ИмяНужнойЯчейки].Значение <> 0) Тогда
	  ...
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[4].Имя,4);
	  Если мФорма1ДФ.Области[ИмяНужнойЯчейки].Значение <> 0 Тогда
	  ...
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[5].Имя,4);
	  Если (мФорма1ДФ.Области[ИмяНужнойЯчейки].Значение <> 0) Тогда
	  ...
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[6].Имя,4);
	  ОбластьМакета.Параметры.Пр = мФорма1ДФ.Области[ИмяНужнойЯчейки].Значение;
	  
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[9].Имя,4);
	  ...
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[10].Имя,4);
	  ОбластьМакета.Параметры.Озн  = мФорма1ДФ.Области[ИмяНужнойЯчейки].Значение;
	  
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[7].Имя,4);
	  ...
	  ИмяНужнойЯчейки = "T1R" + Формат(НСтр , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред(СтруктураГруппы.Колонки[8].Имя,4);
	  Если мФорма1ДФ.Области[ИмяНужнойЯчейки].Значение <> '00010101' Тогда
  ...
  Для СчРазд2 = 1 По 6 Цикл
	  ...
	  Если (мФорма1ДФ.Области["R020" + СчРазд2 + "G03A"].Значение <> 0) Тогда
		  ОбластьМакета.Параметры["СумНачислДохГрн" + СчРазд2] = Цел(мФорма1ДФ.Области["R020" + СчРазд2 + "G03A"].Значение);
		  КопейкиВСтроку(мФорма1ДФ.Области["R020" + СчРазд2 + "G03A"].Значение, ОбластьМакета.Параметры["СумНачислДохКоп" + СчРазд2]);
	  Иначе
	  ...
	  Если СчРазд2 >=3  Тогда
		  Если (мФорма1ДФ.Области["R020" + СчРазд2 + "G03"].Значение <> 0) Тогда
			  ОбластьМакета.Параметры["СумВыплачДохГрн" + СчРазд2] = Цел(мФорма1ДФ.Области["R020" + СчРазд2 + "G03"].Значение);
			  КопейкиВСтроку(мФорма1ДФ.Области["R020" + СчРазд2 + "G03"].Значение, ОбластьМакета.Параметры["СумВыплачДохКоп" + СчРазд2]);
		  Иначе
	  ...
	  Если (мФорма1ДФ.Области["R020" + СчРазд2 + "G04A"].Значение <> 0) Тогда
		  ОбластьМакета.Параметры["СумНачислНДФЛГрн" + СчРазд2] = Цел(мФорма1ДФ.Области["R020" + СчРазд2 + "G04A"].Значение);
		  КопейкиВСтроку(мФорма1ДФ.Области["R020" + СчРазд2 + "G04A"].Значение, ОбластьМакета.Параметры["СумНачислНДФЛКоп" + СчРазд2]);
	  Иначе
		  ОбластьМакета.Параметры["СумНачислНДФЛГрн" + СчРазд2] = "" + "-"; 
		  ОбластьМакета.Параметры["СумНачислНДФЛКоп" + СчРазд2] = "" + "-"; 
	  КонецЕсли;
	  
	  Если (мФорма1ДФ.Области["R020" + СчРазд2 + "G04"].Значение <> 0) Тогда
		  ОбластьМакета.Параметры["СумПеречислНДФЛГрн" + СчРазд2] = Цел(мФорма1ДФ.Области["R020" + СчРазд2 + "G04"].Значение);
		  КопейкиВСтроку(мФорма1ДФ.Области["R020" + СчРазд2 + "G04"].Значение, ОбластьМакета.Параметры["СумПеречислНДФЛКоп" + СчРазд2]);
	  Иначе
  ...
  ЗначениеВКлетку(Формат(мФорма1ДФ.Области.R02G01I.Значение, "ЧГ=0"), 0, 6,"Строк",ОбластьМакета);
  ЗначениеВКлетку(Формат(мФорма1ДФ.Области.R02G02I.Значение, "ЧГ=0"), 0, 6,"ФЛ",ОбластьМакета);
  ...
  Если Организация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
	  ЗначениеВКлетку(мФорма1ДФ.Области.HTIN.Значение, 0, 10,"ИННЧЛ",ОбластьМакета);
  КонецЕсли;
  ...
  ОбластьМакета.Параметры.ТелЧЛ        = мФорма1ДФ.Области.HTELFO.Значение;
...
Процедура ОбновитьСтруктуруМногострочныхРазделов()
	...
	Для Каждого ЭлементСтруктуры Из мСтруктураМногострочныхРазделов Цикл
		...
		Для НСтр = 1 По СтруктураГруппы.Количество() Цикл
			...
			Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
				ИмяКолонки    = Колонка.Имя;
				ИмяЯчейки    		= "T1R" + Формат(НСтр , 	 "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (ИмяКолонки,4);
				ИмяНовОбластиЯчейки = "T1R" + Формат(НСтрНов , "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (ИмяКолонки,4);
...
Процедура Расчет()
	...
	мФорма1ДФ.Области.R01G03A.Значение = СтруктураГруппы1.Итог("T1RG03A");
	мФорма1ДФ.Области.R01G03.Значение = СтруктураГруппы1.Итог("T1RG03");
	мФорма1ДФ.Области.R01G04A.Значение = СтруктураГруппы1.Итог("T1RG04A");
	мФорма1ДФ.Области.R01G04.Значение = СтруктураГруппы1.Итог("T1RG04");
...
Функция СформироватьТекстЗапросаПоШтатнымСовместителям(ДопУсловие = "")
	...
	ТекстЗапросаПоШтатнымСовместителям = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	                                     ...
	                                     |ИЗ
	                                     |	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ДатаНач, ) КАК РаботникиОрганизаций
	                                     |ГДЕ
...
Процедура ЗаполнитьАвто()  Экспорт
	...
	МетаданныеВидыДоходов = Справочники.ВидыДоходовНДФЛ.ПолучитьСсылку().Метаданные();
	Если МетаданныеВидыДоходов.Реквизиты.Найти("ВидСтавкиВоенныйСбор") = Неопределено Тогда
	...
	|	ЕСТЬNULL(ТаблицаЛьгот.Льгота,ЗНАЧЕНИЕ(Справочник.ВидыЛьготРаботниковОрганизаций.ПустаяСсылка))  КАК Льгота,
	...
	|		ЕСТЬNULL(ТаблицаЛьгот.Льгота,ЗНАЧЕНИЕ(Справочник.ВидыЛьготРаботниковОрганизаций.ПустаяСсылка))
	...
	//////	ЗапросРаботникиНаНачало.УстановитьПараметр("ДатаНач", Новый Граница(мДатаНачалаПериодаОтчета-1,ВидГраницы.Включая));
	//////Иначе	
	//////    ЗапросРаботникиНаНачало.УстановитьПараметр("ДатаНач", мДатаНачалаПериодаОтчета);
	//////КонецЕсли;
	//////
	//////
	//////
	...
	//////
	//////Если ЭтоУТП Тогда
	//////	ЗапросДатыКадровыхИзменений.УстановитьПараметр("ДатаКон", мДатаКонцаПериодаОтчета);
	//////КонецЕсли;
	//////
	...
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	РаботникиОрганизаций.Сотрудник.Физлицо КАК Сотрудник,
	|	СУММА(РаботникиОрганизаций.ЗанимаемыхСтавок) КАК Ставок
	|ПОМЕСТИТЬ РаботникиНаНачало
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(РаботникиОрганизаций.Период, СЕКУНДА, -1), ДЕНЬ) КАК Период,
	|	РаботникиОрганизаций.Сотрудник.Физлицо КАК Сотрудник,
	|	ДОБАВИТЬКДАТЕ(РаботникиОрганизаций.Период, СЕКУНДА, -1) КАК ДатаУвольнения
	|ПОМЕСТИТЬ ДатыУвольнений
	|ИЗ
	|	РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|ГДЕ
	|	РаботникиОрганизаций.Период <= &ДатаКон
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НаКонец.Сотрудник КАК Сотрудник,
	|	ЕСТЬNULL(ДатыПриемов.Период, ДатыУвольнений.Период) КАК Период,
	|	МАКСИМУМ(ВЫБОР КОГДА ЕСТЬNULL(НаНачало.Ставок, 0) > 0 ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ КОНЕЦ) КАК РаботалНаНачало,
	|	МАКСИМУМ(ЕСТЬNULL(ДатыПриемов.ДатаПриема, ДАТАВРЕМЯ(1, 1, 1))) 		  КАК ДатаПриема,
	|	МАКСИМУМ(ЕСТЬNULL(ДатыУвольнений.ДатаУвольнения, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаУвольнения
	|ИЗ
	|	РаботникиНаКонец КАК НаКонец
	|		ЛЕВОЕ СОЕДИНЕНИЕ РаботникиНаНачало КАК НаНачало
	|		ПО НаКонец.Сотрудник = НаНачало.Сотрудник
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДатыПриемов КАК ДатыПриемов
	|			ПОЛНОЕ СОЕДИНЕНИЕ ДатыУвольнений КАК ДатыУвольнений
	|			ПО ДатыПриемов.Сотрудник = ДатыУвольнений.Сотрудник
	|				И ДатыПриемов.Период = ДатыУвольнений.Период
	|		ПО (ЕСТЬNULL(ДатыПриемов.Сотрудник, ДатыУвольнений.Сотрудник) = НаКонец.Сотрудник)
	|
	|СГРУППИРОВАТЬ ПО 
	|	НаКонец.Сотрудник,
	|	ЕСТЬNULL(ДатыПриемов.Период, ДатыУвольнений.Период)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Сотрудник,
	|	Период";
	...
	ЗапросПоШтатнымСовместителям.УстановитьПараметр("ДатаНач", мДатаНачалаПериодаОтчета);
	ЗапросПоШтатнымСовместителям.УстановитьПараметр("ДатаКон", мДатаКонцаПериодаОтчета);
	...
	Для Каждого стр ИЗ ТаблицаНДФЛ Цикл
		...
		Если Номер > 0 Тогда
			...
			Если Номер > 9999 Тогда
				...
				Иначе
					мМенеджерЗвит1С.ПредупреждениеЗвит1С(ТекстСообщенияОшибкаКолСтрок, "Завантаження даних з 1С");
				КонецЕсли;		
				Прервать;
			КонецЕсли;
		...
		// номер строки
		СтруктураГруппы[КолСтрок-1][0] = КолСтрок;
		...
		КонецЕсли;
		СтруктураГруппы[КолСтрок-1][1] = стр.ИННСотрудника;
		
		
		// отобразим ФИО сотрудника
		СтруктураГруппы[КолСтрок-1]["T1RGFIO"] = стр.Сотрудник;
		
		
		СтруктураГруппы[КолСтрок-1][2] = стр.ДоходПриход;
		
		СтруктураГруппы[КолСтрок-1][3] = стр.ДоходРасход;
		
		СтруктураГруппы[КолСтрок-1][4] = стр.НалогПриход;
		
		СтруктураГруппы[КолСтрок-1][5] = стр.НалогРасход;
	
		СтруктураГруппы[КолСтрок-1][6] = стр.ДоходНДФЛ;
		
		Льгота = "";
		Если (СтрДлина(стр.Льгота) > 0)  Тогда
			Льгота = стр.Льгота.КодОтчетностиНДФЛ;	
		КонецЕсли;	
		СтруктураГруппы[КолСтрок-1][9] = Льгота;
		...
		Если (стр.Сотрудник <> СотрудникПредыдущий)  Тогда
			...
			Если НЕ (ТипЗнч(стр.Сотрудник) = Тип("СправочникСсылка.Контрагенты")) Тогда
				...
				ПриемУвольнение = РаботавшиеВОрганизации.НайтиСтроки(ПараметрыОтбора);
				...
				Если ПриемУвольнение.Количество() > 0 Тогда
					// 1.1 определяем, что сотрудник ранее работал и был уволен
					РаботалНаНачало = ПриемУвольнение[0].РаботалНаНачало;
					// ищем макс. дату увольнения для сотрудника в предыдущем периоде
					Для каждого ЗаписьПриемУвольнение Из ПриемУвольнение Цикл
						Если ЗаписьПриемУвольнение.ДатаУвольнения < мДатаНачалаПериодаОтчета И ЗаписьПриемУвольнение.ДатаУвольнения > ДатаУвольнения Тогда
							ДатаУвольнения = ЗаписьПриемУвольнение.ДатаУвольнения;
						КонецЕсли;
					КонецЦикла;
					Если РаботалНаНачало Тогда
						//если дата увольнения это последний день прошлого квартала, значит сотрудник не работал в этом периоде.
						Если НачалоДня(ДатаУвольнения) = НачалоДня(мДатаНачалаПериодаОтчета - 1) Тогда
							РаботалНаНачало = Ложь;	
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				...
				Если НЕ РаботалНаНачало 
					...
					Для каждого ЗаписьПриемУвольнение Из ПриемУвольнение Цикл
						Если ЗначениеЗаполнено(ЗаписьПриемУвольнение.ДатаПриема) И (ЗаписьПриемУвольнение.ДатаПриема <= мДатаКонцаПериодаОтчета) Тогда
							ДатаУвольнения = '00010101';
							Прервать;
						КонецЕсли;
					КонецЦикла;	
				...
				Для каждого ЗаписьПриемУвольнение Из ПриемУвольнение Цикл
					...
					Если НЕ РаботалВПредыдущемПериоде 
						И ЗначениеЗаполнено(ЗаписьПриемУвольнение.ДатаПриема) Тогда
						
						Если ЗначениеЗаполнено(ЗаписьПриемУвольнение.ДатаУвольнения) Тогда 
							// прием и увольнение в один день показывать не будем
							Продолжить;
						КонецЕсли;
						
						Если ЗаписьПриемУвольнение.ДатаПриема > мДатаКонцаПериодаОтчета Тогда 
						...
						Если ДатаПриема > '00010101' ИЛИ ДатаУвольнения > '00010101' Тогда
							...
							СтруктураГруппы[КолСтрок-1][7] = ДатаПриема;
							
							СтруктураГруппы[КолСтрок-1][8] = ДатаУвольнения;
						...
						ДатаПриема 		= ЗаписьПриемУвольнение.ДатаПриема;
						ДатаУвольнения  = '00010101';
						РаботалВПредыдущемПериоде = Истина;
						
					ИначеЕсли РаботалВПредыдущемПериоде
						//И ЗначениеЗаполнено(ЗаписьПриемУвольнение.ДатаУвольнения) Тогда
						И ЗаписьПриемУвольнение.ДатаУвольнения >= мДатаНачалаПериодаОтчета Тогда
						
						Если ЗначениеЗаполнено(ЗаписьПриемУвольнение.ДатаПриема) Тогда 
							// прием и увольнение в один день показывать не будем
							Продолжить;
						КонецЕсли;
	...
	КонецЦикла;
	ВывестиРазделВТабличныйДокумент(мИдГруппы1, СтруктураГруппы);
	
	
	мФорма1ДФ.Области.R0205G03A.Значение = R0205G03A;
	мФорма1ДФ.Области.R0205G03.Значение  = R0205G03;
	мФорма1ДФ.Области.R0205G04A.Значение = R0205G04A;
	мФорма1ДФ.Области.R0205G04.Значение  = R0205G04;
	...
	Если мСохраненныйДок = Неопределено Тогда
		...
		Для Каждого ЭлементСтруктуры Из мСтруктураМногострочныхРазделов Цикл
			...
			Для НСтр = 1 По  ИсходноеКоличествоСтрокГруппы Цикл
				...
				Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
					ИмяКолонки    = Колонка.Имя;
					КодПоказателя    = Сред (ИмяКолонки,1,3) + Формат(НСтр, "ЧЦ=4; ЧВН=; ЧГ=0") + Сред (ИмяКолонки,4,СтрДлина(ИмяКолонки)-3);
...
Процедура КоманднаяПанельДействийДействиеXML(Кнопка) Экспорт
  ...
  Если Организация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
  	ДанныеОтчета.ПолеТабличногоДокументаФорма1ДФ.HKBOS 		= ДанныеОтчета.ПолеТабличногоДокументаФорма1ДФ.HKFO;
  	ДанныеОтчета.ПолеТабличногоДокументаФорма1ДФ.HBOS 		= ДанныеОтчета.ПолеТабличногоДокументаФорма1ДФ.HFO;
	ДанныеОтчета.ПолеТабличногоДокументаФорма1ДФ.HTELBOS 	= ДанныеОтчета.ПолеТабличногоДокументаФорма1ДФ.HTELFO;
  КонецЕсли;
  ...
  // вставляем данные многострочных разделов
  КопияСтруктурыМногострочныхРазделов = Новый Структура();
  Для каждого МнР Из мСтруктураМногострочныхРазделов Цикл
	  КопияСтруктурыМногострочныхРазделов.Вставить(МнР.Ключ, МнР.Значение.Скопировать());
  КонецЦикла;
  Если ЗначениеЗаполнено(ДанныеОтчета.ПолеТабличногоДокументаФорма1ДФ.HZ) Тогда
	...
	Для каждого Строка Из КопияСтруктурыМногострочныхРазделов.T1R Цикл
		Строка.T1RG09 = "&НеЗаписывать";
	КонецЦикла;
  КонецЕсли;
  //спПараметры.Вставить("ДанныеМногострочныхРазделов", мСтруктураМногострочныхРазделов);
  спПараметры.Вставить("ДанныеМногострочныхРазделов", КопияСтруктурыМногострочныхРазделов);
...
Процедура ПолеТабличногоДокументаФорма1ДФR28C3НачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ТекущийНомерСтроки = Сред(ТекущаяОбласть.Имя,4,4);
	...
	Если НЕ РезультатВыбора = Неопределено Тогда
		...
		Если НЕ РезультатВыбора.ЭтоГруппа Тогда
			...
			ЭлементыФормы.ПолеТабличногоДокументаФорма1ДФ.Области["T1R" + ТекущийНомерСтроки + "GFIO"].Значение = РезультатВыбора;		
		КонецЕсли;
...
Процедура ПолеТабличногоДокументаФорма1ДФПриИзмененииСодержимогоОбласти(Элемент, Область)
	
	 ПоказатьИННЧЛ();
	
КонецПроцедуры

Процедура ПоказатьИННЧЛ()

	Если Организация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
	
		//мФорма1ДФ.Области.ИННЧЛ.Значение = мФорма1ДФ.Области.HTIN.Значение;
		мФорма1ДФ.Области.HKFO.Значение = мФорма1ДФ.Области.HTIN.Значение;
		
	Иначе
		
		//мФорма1ДФ.Области.ИННЧЛ.Значение = "";
		мФорма1ДФ.Области.HKFO.Значение = "";
...
СтруктураГруппы1.Колонки.Добавить("T1RG01",  ОписаниеТиповСтрока);
СтруктураГруппы1.Колонки.Добавить("T1RG02",  ОписаниеТиповСтрока);
СтруктураГруппы1.Колонки.Добавить("T1RG03A", ОписаниеТиповЧисло);
СтруктураГруппы1.Колонки.Добавить("T1RG03",  ОписаниеТиповЧисло);
СтруктураГруппы1.Колонки.Добавить("T1RG04A", ОписаниеТиповЧисло);
СтруктураГруппы1.Колонки.Добавить("T1RG04",  ОписаниеТиповЧисло);
СтруктураГруппы1.Колонки.Добавить("T1RG05",  ОписаниеТиповЧисло3);
СтруктураГруппы1.Колонки.Добавить("T1RG06D", ОписаниеТиповДата);
СтруктураГруппы1.Колонки.Добавить("T1RG07D", ОписаниеТиповДата);
СтруктураГруппы1.Колонки.Добавить("T1RG08",  ОписаниеТиповСтрока);
СтруктураГруппы1.Колонки.Добавить("T1RG09",  ОписаниеТиповСтрока);
СтруктураГруппы1.Колонки.Добавить("T1RGFIO",  ОписаниеТиповСтрока);
...
мСоставПоказателей = "СоставПоказателей2015кв1";

// ОПИСАНИЕ ПАРМЕТРОВ ФОРМЫ
//
мВерсияФормы = "01/01/2015";
