...
Перем МассивШтатных, МассивЖенщин;
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	мОтчетОКоличествеРаботников.Области.КодКОАТУУ.Значение             = КодКОАТУУ;
	мОтчетОКоличествеРаботников.Области.КодКФВ.Значение                = КодКФВ;
	мОтчетОКоличествеРаботников.Области.КодКВЕД.Значение               = КодКВЕД;
	мОтчетОКоличествеРаботников.Области.ОрганГУ.Значение               = ОрганГУ;
	мОтчетОКоличествеРаботников.Области.КодЕДРПОУ.Значение             = КодЕДРПОУ;
	
	мОтчетОКоличествеРаботников.Области.ОргПочтАдрес.Значение = УправлениеКонтактнойИнформацией.ПолучитьАдресИзКонтактнойИнформации(Организация,"Юридический");
    ...
    УстановитьЗначениеПоказателя( "ОргБухгалтер", ФИОГлавБуха);
...
Процедура РассчитатьЧислоЧеловекВОтпусках()
	Если (РегламентированнаяОтчетность.ИДКонфигурации() = "УПП") или (РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП") Или (РегламентированнаяОтчетность.ИДКонфигурации() = "УТП") Тогда
		Запрос=Новый Запрос;
		ТекстЗапроса ="ВЫБРАТЬ
		|	СостояниеРаботниковОрганизацийСрезПоследних.Физлицо КАК КолОбщее,
		|	ВЫБОР
		|		КОГДА СостояниеРаботниковОрганизацийСрезПоследних.Физлицо.Пол = &Пол
		|			ТОГДА ""Женский""
...
//в процедуре осуществляется расчет количества женщин с детьми до 15 лет или
//с детьми-инвалидами
////Процедура ОпределитьКоличествоЖенщинСДетьмиДо15()
//	//сначала запрос или просто обход результат самомго первого запроса
//	//с целью формирования массива женщин
//		Текст = "ВЫБРАТЬ
//				|	Семья.Ссылка КАК ФизЛицо,
//				|	СУММА(ВЫБОР
//				|			КОГДА Семья.СтепеньРодства = &парамРебенок
//				|					И ГОД(&парамДата) - ГОД(Семья.ФизЛицо.ДатаРождения) < 15
//				|				ТОГДА 1
//				|			ИНАЧЕ 0
//				|		КОНЕЦ) КАК КвоДетей
//				|ИЗ
//				|	Справочник.ФизическиеЛица.СоставСемьи КАК Семья
//				|ГДЕ
//				|	Семья.Ссылка В(&парамФизЛицо)
//				|
//				|СГРУППИРОВАТЬ ПО
//				|	Семья.Ссылка
//				|	";
//	
//	ЗапросОКоличествеДетей = Новый Запрос;
//	ЗапросОКоличествеДетей.Текст = Текст;
//	ЗапросОКоличествеДетей.УстановитьПараметр("парамРебенок",  Справочники.СтепениРодстваФизЛиц.Ребенок);
//	ЗапросОКоличествеДетей.УстановитьПараметр("парамДата",  мДатаКонцаПериодаОтчета);
//	ЗапросОКоличествеДетей.УстановитьПараметр("парамФизЛицо",  МассивЖенщин);
//	ТаблицаЗапроса = ЗапросОКоличествеДетей.Выполнить().Выгрузить();

//	УстановитьЗначениеПоказателя("П12180_2",ТаблицаЗапроса.Количество());
//	
//КонецПроцедуры	

// Процедура рассчитывает количество инвалидов общее и среди них женщин
Процедура РассчитатьКоличествоИнвалидов()
	Если (РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП") или (РегламентированнаяОтчетность.ИДКонфигурации() = "УПП") Тогда
		Группа=Перечисления.ГруппыИнвалидности.ТретьяГруппа;
		ЗапросИнв = Новый Запрос;
		ТекстЗапросаИнв ="ВЫБРАТЬ
		|	СведенияОбИнвалидностиФизлицСрезПоследних.Физлицо КАК Физлицо,
		|	ВЫБОР
		...
		|	КОНЕЦ КАК ГруппаЖен
		|ИЗ
		|	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&НаДату, (ЗанимаемыхСтавок > 0) И (Организация = &Организация)) КАК РаботникиОрганизацийСрезПоследних
...
Процедура ЗаполнитьАвто() Экспорт
	...
	Запрос=Новый Запрос;
	ТекстЗапроса ="ВЫБРАТЬ
	|	РаботникиОрганизацийСрезПоследних.Физлицо КАК КолОбщее,
	|	ВЫБОР
	|		КОГДА РаботникиОрганизацийСрезПоследних.Физлицо.Пол = &Пол
	|			ТОГДА ""Женский""
	|	КОНЕЦ КАК ОбщееКолЖен,
	|	ВЫБОР
	|		КОГДА ВЫБОР
	|				КОГДА ГОД(РаботникиОрганизацийСрезПоследних.Физлицо.ДатаРождения) = 1
	|					ТОГДА 0
	|				ИНАЧЕ &ДатаАктуальности_Год - ГОД(РаботникиОрганизацийСрезПоследних.Физлицо.ДатаРождения) 
	|			  КОНЕЦ МЕЖДУ 15 И 34
	|			ТОГДА ВЫБОР
	|                     КОГДА МЕСЯЦ(РаботникиОрганизацийСрезПоследних.Физлицо.ДатаРождения) * 100 + ДЕНЬ(РаботникиОрганизацийСрезПоследних.Физлицо.ДатаРождения) < &ДатаАктуальности_Месяц * 100 + &ДатаАктуальности_День
	|                       ТОГДА ВЫБОР
	|					          КОГДА РаботникиОрганизацийСрезПоследних.Физлицо.Пол = &Пол  
	|							    ТОГДА ""Женский""
	|                              КОНЕЦ
	...
	|ИЗ
	|	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&НаДату, ((Приказ.ВидЗанятости = &ОсновноеМестоРаботы) И (Приказ.Организация = &Организация))) 
	...
	|		ПО РаботникиОрганизацийСрезПоследних.Физлицо = СведенияОПенсионерахСрезПоследних.ФизЛицо
	...
	МассивЖенщин = Новый Массив;
	
	Для каждого стр Из ВыборкаЗапроса Цикл
	  Если стр.ОбщееКолЖен = "Женский" Тогда
		  МассивЖенщин.Добавить(стр.КолОбщее);
	  КонецЕсли
	КонецЦикла; 		
	...
	//ОпределитьКоличествоЖенщинСДетьмиДо15();
