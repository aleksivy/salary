...
Процедура ЗаполнитьНачальныеНастройкиЛок() Экспорт
...
КонецПроцедуры // ЗаполнитьНачальныеНастройкиЛок()
...
Процедура ПоказатьПериод()
	...
	ПериодОтчета = Формат(мДатаКонцаПериодаОтчета, "ДФ = ' гггг';Л = uk_UA") + " рік";
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	КонецЕсли;
	УстановитьЗначениеПоказателя ("ЭлПочта", УправлениеКонтактнойИнформацией.ПолучитьEmailИзКонтактнойИнформации(ОбъектКонтактнойИнформации));
	
	мОтчетОКоличествеРаботников.Области.КодПодразделения3.Значение = "";
	
	//Заполнение поля" "Исполнитель"
	мОтчетОКоличествеРаботников.Области.ИСПОЛНИТЕЛЬ.Значение    = глЗначениеПеременной("глТекущийПользователь");	
	...
	мОтчетОКоличествеРаботников.Области.ИСПОЛНИТЕЛЬ.Значение = СокрЛП(ФИОСрез.Фамилия) + " " + Лев(СокрЛ(ФИОСрез.Имя), 1) + "." + Лев(СокрЛ(ФИОСрез.Отчество), 1) + ".";
...
//		ИЛИ (РегламентированнаяОтчетность.ИДКонфигурации() = "УТП") Тогда
//		
//		Группа3=Перечисления.ГруппыИнвалидности.ТретьяГруппа;
//		
//	КонецЕсли;
...
//		ИЛИ (РегламентированнаяОтчетность.ИДКонфигурации() = "УПП") 
//		ИЛИ (РегламентированнаяОтчетность.ИДКонфигурации() = "УТП") Тогда
//		ТекстЗапросаИнв = ТекстЗапросаИнв  + "
//		|	ВЫБОР
...
//	|	 ГДЕ (ЗанимаемыхСтавок > 0)) КАК РаботникиОрганизацийСрезПоследних
//	|
...
//	ЗапросИнв.УстановитьПараметр("Группа", Группа3);
...
//		УстановитьЗначениеПоказателя("П12130_2",ГруппаЖен);
//	КонецЕсли;
...
Процедура ЗаполнитьАвто() Экспорт
	...
	////|                     КОГДА МЕСЯЦ(РаботникиОрганизацийСрезПоследних.Сотрудник.Физлицо.ДатаРождения) * 100 + ДЕНЬ(РаботникиОрганизацийСрезПоследних.Сотрудник.Физлицо.ДатаРождения) < &ДатаАктуальности_Месяц * 100 + &ДатаАктуальности_День
	////|                       ТОГДА ВЫБОР
	////|					          КОГДА НЕ РаботникиОрганизацийСрезПоследних.Сотрудник.Физлицо.Пол = &Пол  
	////|							    ТОГДА ""Мужской""
	////|                              КОНЕЦ
	////|				КОНЕЦ
	////|	КОНЕЦ КАК КолМуж55_59,
	////|	ВЫБОР
	...
	////|	КОЛИЧЕСТВО(КолЖен51_55),
	////|	КОЛИЧЕСТВО(КолМуж55_59),
	...
	УстановитьЗначениеПоказателя("П12030_1",Кол15_35);
	УстановитьЗначениеПоказателя("П12030_2",КолЖен15_35);
	
	////КолЖен51_55 = ВыборкаЗапроса.Итог("КолЖен51_55");
	////УстановитьЗначениеПоказателя("П12050_2",КолЖен51_55);
	////
	////КолМуж55_59    = ВыборкаЗапроса.Итог("КолМуж55_59");
	////УстановитьЗначениеПоказателя("П12060_1",КолМуж55_59);
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьНачальныеНастройкиЛок();     
