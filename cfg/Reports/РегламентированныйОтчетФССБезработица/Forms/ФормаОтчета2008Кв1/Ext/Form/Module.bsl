...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если мСтруктураВариантыЗаполнения.Свойство(ИмяСтраницыПанели, МассивВариантовЗаполненияСтраниц) Тогда
		...
		Иначе
			...
			МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2008Кв1");
		...
		Пока Результат.Следующий() Цикл
			Если Месяц(Результат.НалоговыйПериод) = Месяц(мДатаНачалаПериодаОтчета) Тогда
				...
				Если (Результат.СтатьяНалоговойДекларации = МассивСтатей[0]) Тогда
					Н1Нар15 = Н1Нар15 + Результат.Начислено;
					Н1Пер15 = Н1Пер15 + Результат.УплаченоПодтверждено;
				Иначе
					Н1Нар05 = Н1Нар05 + Результат.Начислено;
					Н1Пер05 = Н1Пер05 + Результат.УплаченоПодтверждено;
				КонецЕсли;
				
			ИначеЕсли Месяц(Результат.НалоговыйПериод) = Месяц(мДатаКонцаПериодаОтчета)	 Тогда
				//третий месяц	
				Если (Результат.СтатьяНалоговойДекларации = МассивСтатей[0]) Тогда
					Н3Нар15 = Н3Нар15 + Результат.Начислено;
					Н3Пер15 = Н3Пер15 + Результат.УплаченоПодтверждено;
				Иначе	
					Н3Нар05 = Н3Нар05 + Результат.Начислено;
					Н3Пер05 = Н3Пер05 + Результат.УплаченоПодтверждено;
				КонецЕсли;
				
			ИначеЕсли Месяц(Результат.НалоговыйПериод) = Месяц(ДобавитьМесяц(мДатаКонцаПериодаОтчета,-1))	 Тогда	
				//второй месяц	
				Если (Результат.СтатьяНалоговойДекларации = МассивСтатей[0]) Тогда
					Н2Нар15 = Н2Нар15 + Результат.Начислено;
					Н2Пер15 = Н2Пер15 + Результат.УплаченоПодтверждено;
				Иначе
					Н2Нар05 = Н2Нар05 + Результат.Начислено;
					Н2Пер05 = Н2Пер05 + Результат.УплаченоПодтверждено;
				КонецЕсли;	
				
			Иначе//Если Месяц(Результат.НалоговыйПериод) < Месяц(мДатаНачалаПериодаОтчета) Тогда	
				
				Если (Результат.СтатьяНалоговойДекларации = МассивСтатей[0]) Тогда
					ПочНар15 = ПочНар15 + ОкруглитьЧислоПоФормату(Результат.Начислено);
					ПочПер15 = ПочПер15 + ОкруглитьЧислоПоФормату(Результат.УплаченоПодтверждено);
					
				Иначе					
					ПочНар05 = ПочНар05 + ОкруглитьЧислоПоФормату(Результат.Начислено);
					ПочПер05 = ПочПер05 + ОкруглитьЧислоПоФормату(Результат.УплаченоПодтверждено);
				КонецЕсли;
...
Процедура ЗаполнитьАвто() Экспорт
	...
	Пока Истина Цикл
		Если КонДата <= мДатаКонцаПериодаОтчета Тогда
			ЧисленностьЭПЗ = ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачДата, КонДата, "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", Организация, "ИНВ");
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьНачальныеНастройки("СоставПоказателей2008Кв1");
