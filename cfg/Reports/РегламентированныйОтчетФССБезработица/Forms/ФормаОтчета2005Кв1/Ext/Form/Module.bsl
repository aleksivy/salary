...
Процедура Расчет()
	...
	мОтчетФССБезработица.Области.П23.Значение = мОтчетФССБезработица.Области.П21.Значение; 						
...
Процедура  РассчитатьСуммыСтраховыхВзносов()
			...
			Для каждого НужнаяСтатья из МассивСтатей Цикл	
				...
				Если Месяц(мес) < Месяц(мДатаНачалаПериодаОтчета) Тогда 
					Если (i = 0) Тогда
						...
						ПочПер15 = ПочПер15 + мБухИтогиРС.ПолучитьИтог("СуммаКорОборотДт", "Период,Счет,Субконто1,Субконто2,КорСчет",мес, СчетФСС, НужнаяСтатья,РасчСчет1)
						+ мБухИтогиРС.ПолучитьИтог("СуммаКорОборотДт", "Период,Счет,Субконто2,КорСчет",мес, СчетФСС, НужнаяСтатья,РасчСчет2);
						
					Иначе					
						ПочПер05 = ПочПер05 + мБухИтогиРС.ПолучитьИтог("СуммаКорОборотДт", "Период,Счет,Субконто2,КорСчет",мес, СчетФСС, НужныйВидСбора,НужнаяСтатья,РасчСчет1)
						+ мБухИтогиРС.ПолучитьИтог("СуммаКорОборотДт", "Период,Счет,Субконто2,КорСчет",мес, СчетФСС, НужнаяСтатья,РасчСчет2);
	...
	Иначе
		...
		|	НеподтвержденнаяСуммаРасход УплаченоНеПодтверждено
		|	
		|ИЗ
		...
		Пока Результат.Следующий() Цикл
			...
			Если Месяц(Результат.НалоговыйПериод) = Месяц(мДатаНачалаПериодаОтчета) Тогда
				Если (Результат.СтатьяНалоговойДекларации = МассивСтатей[0]) Тогда
					...
					Н1Пер15 = Результат.УплаченоПодтверждено + Результат.УплаченоНеПодтверждено;
				Иначе
					Н1Нар05 = Результат.Начислено;
					Н1Пер05 = Результат.УплаченоПодтверждено + Результат.УплаченоНеПодтверждено;
				КонецЕсли;
			...
			ИначеЕсли Месяц(Результат.НалоговыйПериод) = Месяц(мДатаКонцаПериодаОтчета)	 Тогда
				Если (Результат.СтатьяНалоговойДекларации = МассивСтатей[0]) Тогда
					...
					Н3Пер15 = Результат.УплаченоПодтверждено + Результат.УплаченоНеПодтверждено;
				Иначе	
					Н3Нар05 = Результат.Начислено;
					Н3Пер05 = Результат.УплаченоПодтверждено + Результат.УплаченоНеПодтверждено;
				КонецЕсли;
			...
			ИначеЕсли Месяц(Результат.НалоговыйПериод) = Месяц(ДобавитьМесяц(мДатаКонцаПериодаОтчета,-1))	 Тогда	
				Если (Результат.СтатьяНалоговойДекларации = МассивСтатей[0]) Тогда
					...
					Н2Пер15 = Результат.УплаченоПодтверждено + Результат.УплаченоНеПодтверждено;
				Иначе
					Н2Нар05 = Результат.Начислено;
					Н2Пер05 = Результат.УплаченоПодтверждено + Результат.УплаченоНеПодтверждено;
				КонецЕсли;	
			...
			ИначеЕсли Месяц(Результат.НалоговыйПериод) < Месяц(мДатаНачалаПериодаОтчета) Тогда	
				...
				Если (Результат.СтатьяНалоговойДекларации = МассивСтатей[0]) Тогда
					...
					ПочПер15 = ПочПер15 + Результат.УплаченоПодтверждено + Результат.УплаченоНеПодтверждено;
					
				Иначе					
					ПочПер05 = ПочНар05 + Результат.Начислено;
					ПочНар05 = ПочПер05 + Результат.УплаченоПодтверждено + Результат.УплаченоНеПодтверждено;
				КонецЕсли;
...
Процедура РассчитатьСуммыЗадолженностей()
	...
	Иначе
		...
		|Выбрать
		|	Сумма(РезультатОстаток +  НеподтвержденнаяСуммаОстаток) КАК ЗадолженностьРезультат
		|
		|Из
		|	РегистрНакопления.ВзаиморасчетыПоВзносамВФонды.Остатки(&Период, СтатьяНалоговойДекларации В (&МассивСтатей) И Организация = &Организация)	
		|";
...
// Инициализация
	...
	Для мес = Месяц (НачалоГода(мДатаНачалаПериодаОтчета)) по Месяц(мДатаКонцаПериодаОтчета) Цикл
		...
		Для каждого НужнаяСтатья из МассивСтатей Цикл
			...
			Если (НайденныеСтроки.Количество() = 1 ) Тогда
				...
				мОтчетФССБезработица.Области[ИмяЯчейкиМес].Значение = НайденныеСтроки[0].БазаОборот;
			КонецЕсли;
			 j = j + 1;
		КонецЦикла;
		i = i + 1;
	...
	Пока Истина Цикл
		Если КонДата <= мДатаКонцаПериодаОтчета Тогда
			ЧисленностьЭПЗ = ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачДата, КонДата, "ЭПЗ", "КВАРТАЛ", Организация, "ПЕНСИОНЕРЫ");		
	...
	Пока Истина Цикл
		Если КонДата <= мДатаКонцаПериодаОтчета Тогда
			ЧисленностьЭПЗ = ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачДата, КонДата, "ЭПЗ", "КВАРТАЛ", Организация, "ИНВ");
	...
	//Округляем значение ССЧ по новым правилам округления: Наказ Державного комітету статистики України від 28 вересня 2005 року N 286 
   	ТаблЧисленностиЭПЗ[Ном].Средняя = ОкруглитьССЧ(ТаблЧисленностиЭПЗ[Ном].Средняя);
	УстановитьЗначениеПоказателя("ПСерГод",ТаблЧисленностиЭПЗ[Ном].Средняя);
	
    ТаблЧисленностиПенс = ПроведениеРасчетов.ПолучитьСреднююЧисленность(мДатаНачалаПериодаОтчета, мДатаКонцаПериодаОтчета,"ЭПЗ", "МЕСЯЦСНАЧАЛАГОДА", Организация, "ПЕНСИОНЕРЫ"); 
	Ном = ТаблЧисленностиЭПЗ.Количество() - 1;
	//Округляем значение ССЧ по новым правилам округления: Наказ Державного комітету статистики України від 28 вересня 2005 року N 286 
   	ТаблЧисленностиПенс[Ном].Средняя = ОкруглитьССЧ(ТаблЧисленностиПенс[Ном].Средняя);
	УстановитьЗначениеПоказателя("ПенсГод",ТаблЧисленностиПенс[Ном].Средняя);
	
    ТаблЧисленностиИнв = ПроведениеРасчетов.ПолучитьСреднююЧисленность(мДатаНачалаПериодаОтчета, мДатаКонцаПериодаОтчета, "ЭПЗ", "МЕСЯЦСНАЧАЛАГОДА", Организация, "ИНВ"); 
	Ном = ТаблЧисленностиИнв.Количество() - 1;
	//Округляем значение ССЧ по новым правилам округления: Наказ Державного комітету статистики України від 28 вересня 2005 року N 286 
   	ТаблЧисленностиИнв[Ном].Средняя = ОкруглитьССЧ(ТаблЧисленностиИнв[Ном].Средняя);
	УстановитьЗначениеПоказателя("ИнвГод",ТаблЧисленностиИнв[Ном].Средняя);
