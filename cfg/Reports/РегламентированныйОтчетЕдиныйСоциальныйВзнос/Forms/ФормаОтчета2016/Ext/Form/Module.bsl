...
Процедура КоманднаяПанельДействийДействиеXML(Кнопка) Экспорт
	...
	СтруктураТаблицы = КопияСтруктурыМногострочныхРазделов[мИдГруппы5];
	СтруктураТаблицы.Колонки.Добавить("T1RG111");
	СтруктураТаблицы.Колонки.Добавить("T1RG112");
	Для каждого СтрокаТаблицы Из СтруктураТаблицы Цикл
		СтрокаТаблицы["T1RG111"] = ?(ЗначениеЗаполнено(СтрокаТаблицы["T1RG111D"]), День(СтрокаТаблицы["T1RG111D"]), "");
		СтрокаТаблицы["T1RG112"] = ?(ЗначениеЗаполнено(СтрокаТаблицы["T1RG112D"]), День(СтрокаТаблицы["T1RG112D"]), "");
	КонецЦикла;

	СтруктураТаблицы = КопияСтруктурыМногострочныхРазделов[мИдГруппы6];
	СтруктураТаблицы.Колонки.Добавить("T1RG111");
	СтруктураТаблицы.Колонки.Добавить("T1RG112");
	Для каждого СтрокаТаблицы Из СтруктураТаблицы Цикл
		СтрокаТаблицы["T1RG111"] = Месяц(СтрокаТаблицы["T1RG111D"]);
		СтрокаТаблицы["T1RG112"] = Год(СтрокаТаблицы["T1RG111D"]);
	КонецЦикла;

	СтруктураТаблицы = КопияСтруктурыМногострочныхРазделов[мИдГруппы7];

	СтруктураТаблицы.Колонки.T1RG10.Имя = "_T1RG10";
	СтруктураТаблицы.Колонки.T1RG11.Имя = "_T1RG11";
	СтруктураТаблицы.Колонки.Добавить("T1RG10");
	СтруктураТаблицы.Колонки.Добавить("T1RG11");

	Для каждого СтрокаТаблицы Из СтруктураТаблицы Цикл
		СтрокаТаблицы["T1RG10"] = День(СтрокаТаблицы["_T1RG10"]);
		СтрокаТаблицы["T1RG11"] = День(СтрокаТаблицы["_T1RG11"]);
	КонецЦикла;
	...
	Если ТипЗнч(ЗначениеВыбора) = Тип("Структура") Тогда
		...
		Если  ЗначениеВыбора.Свойство("ИмяПоказателя", 		ИмяПоказателя)
			...
			Если  Лев(ИмяПоказателя, 3) = "T1R" Тогда
				...
				Если Прав(ИмяПоказателя, 4) = "G111" Тогда
			    	ИмяПоказателя = СтрЗаменить(ИмяПоказателя, "G111", "G111D");
				КонецЕсли;
			
				Если Прав(ИмяПоказателя, 4) = "G112" Тогда
			    	ИмяПоказателя = СтрЗаменить(ИмяПоказателя, "G112", "G111D");
				КонецЕсли;
