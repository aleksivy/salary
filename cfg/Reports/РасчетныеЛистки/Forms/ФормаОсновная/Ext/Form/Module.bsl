...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Для каждого Ячейка из ЯчейкиМакета Цикл
		НастройкаШириныКолонокРасчетногоЛистка.Вставить(ЯчейкиМакета.Индекс(Ячейка), Ячейка.Оформление.Элементы.Найти("MinimumWidth").Значение);
		НастройкаРазмТекстаКолонокРасчетногоЛистка.Вставить(ЯчейкиМакета.Индекс(Ячейка), ПолучитьТипРазмещенияТекста(Ячейка.Оформление.Элементы.Найти("Placement").Значение));
	КонецЦикла;
	
	НастройкаРазмТекстаКолонокРасчетногоЛистка[1] = ТипРазмещенияТекстаТабличногоДокумента.Авто;
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ВидРасчетногоЛистка = "Кратко" и Сотрудник <> Неопределено и ПериодРегистрации <> Неопределено тогда
		...
		Если Сотрудник <> Неопределено и ПериодРегистрации <> Неопределено тогда
			...
			ОтчетРасшифровка = Отчеты.РасчетныеЛистки.Создать();
