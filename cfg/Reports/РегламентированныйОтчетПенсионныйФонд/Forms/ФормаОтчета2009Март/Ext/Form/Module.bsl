...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2008Дек");
...
Процедура ПоказатьПериод()
	...
	мОтчетПФ.Области.ПериодОтчета.Значение   = "за " + Формат(мДатаКонцаПериодаОтчета, "ДФ = 'ММММ';Л = uk_UA") +" місяць "+ Формат(мДатаКонцаПериодаОтчета, "ДФ = 'гггг р.';Л = uk_UA");
	
	мОтчетПФ.Области.ПериодОтчетаПриложение.Значение   = "за " + Формат(мДатаКонцаПериодаОтчета, "ДФ = 'ММММ';Л = uk_UA") +" місяць "+ Формат(мДатаКонцаПериодаОтчета, "ДФ = 'гггг р.';Л = uk_UA");
	...
	Если ЧастиДатыПодписи.Свойство("ДвеПоследниеЦифрыГода") Тогда
		мОтчетПФ.Области.ГодДаты.Значение 	= ЧастиДатыПодписи.ДвеПоследниеЦифрыГода;
	Иначе
		мОтчетПФ.Области.ГодДаты.Значение 	= Формат(ДатаПодписи, "ДФ=yy");
	Конецесли;
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	Если Не РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП" Тогда
		...
		Если НЕ УчетнаяПолитикаНеЗадана Тогда 
			...
			мОтчетПФ.Области.СистНалОблож.Значение = СистНалОблож;
		КонецЕсли;
	Иначе
...
"Виконується відновлення користувльницьких настройок.'"), СтатусСообщения.Информация);
	...
	ВерсияФормы = СписокСохранения.ВерсияФормы;
	Если ЕстьИзмененияВариантыЗаполнения ИЛИ ВерсияФормы = "10/06/2008" Тогда
		
		Если  ВерсияФормы = "10/06/2008" Тогда
			Если СписокСохранения.Свойство( "ПоказателиОтчета", ПоказателиОтчета) Тогда
				Для Каждого ЭлементСтруктуры Из ПоказателиОтчета Цикл
					...
					Если ИмяГруппы = "ПолеТабличногоДокументаОтчетПФ" Тогда
...
Процедура ВыгрузитьВАРМЗС ()
	...
	Значение = Код + Формат(День(мДатаКонцаПериодаОтчета),"ЧЦ=2;ЧГ=;") + Формат(Месяц(мДатаКонцаПериодаОтчета), "ЧЦ=2;ЧГ=;") + Формат(Год(мДатаКонцаПериодаОтчета),"ЧЦ=4;ЧГ=0;")+ "P0100115" + ".ZDI";
...
Процедура СоздатьТаблицуСоСхемамиВыгрузки() Экспорт
	...
	ТипКолИмяСхемы = СоздатьОписанияТипов("Строка",30);
	...
	НовСтрока.ИмяСхемы = "ПоказателиАРМЗС2008Дек"; 
...
мСоставПоказателей = "СоставПоказателей2008Дек";
