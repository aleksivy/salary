...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2007Янв");
...
Процедура РассчитатьЧислоЗастрахованныхЛиц (ТаблицаБазВзносов)
	...
	ТаблицаКоличестваЗастрахованныхЛиц = ТаблицаБазВзносов.Скопировать();
	...
	Для каждого строка Из НайденныеСтроки Цикл
		...
		НоваяСтрока.ФизЛицо = строка.ЗастрЛицо.ФизЛицо;
	КонецЦикла;	
...
Процедура  РассчитатьНачисленияЗаПрошлыеИБудущиеПериоды()
	...
	Если  РегламентированнаяОтчетность.ИДКонфигурации() = "БП" 
	  ИЛИ РегламентированнаяОтчетность.ИДКонфигурации() = "УТП" Тогда
		...
		ТаблицаНачисленийПрошлБудущ.Свернуть("ЗастрЛицо,НалоговыйПериод,СтатьяНалоговойДекларации", "СуммаНачисления");
		...
		Для каждого стр Из ТаблицаНачисленийПрошлБудущ Цикл
			Если (стр.ЗастрЛицо <> СотрПред) или ((стр.ЗастрЛицо = СотрПред) и (стр.СтатьяНалоговойДекларации = СтрокиДекларации.ПФ_Декр)) Тогда
				Если  (стр.НалоговыйПериод <> NULL) Тогда
					СтруктураМаксБаза = МаксБазаСведения.ПолучитьПоследнее(стр.НалоговыйПериод,Отбор);
					МаксБаза = СтруктураМаксБаза.Предел;

					Если ((стр.НалоговыйПериод < мДатаНачалаПериодаОтчета) и (стр.НалоговыйПериод = мДатаНачалаПериодаОтчета)) Тогда 
					...
					ИначеЕсли ((стр.НалоговыйПериод > мДатаКонцаПериодаОтчета) и (стр.НалоговыйПериод = мДатаНачалаПериодаОтчета)) Тогда
						НоваяСтрока = тбНачисления.Добавить();
						НоваяСтрока.Месяц = стр.НалоговыйПериод;
...
Процедура РассчитатьСуммуФактическихЗатратНаОплатуТруда()
	Если (РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП") Тогда
		...
		Запрос.Текст = "	
		|ВЫБРАТЬ
		|	СУММА(Взаиморасчеты.СуммаВзаиморасчетовОборот) КАК КО66	
		|ИЗ
		|	РегистрНакопления.ВзаиморасчетыСРаботникамиОрганизаций.Обороты(&ПериодНачало, &ПериодКонец, МЕСЯЦ, СчетУчета = &СчетУчета И Организация = &Организация)	КАК Взаиморасчеты
		|";
...
Процедура ЗаполнитьАвто()  Экспорт
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|ВзаиморасчетыПоВзносам.Сотрудник        КАК ЗастрЛицо,
	|";
	
	Если (РегламентированнаяОтчетность.ИДКонфигурации() = "УПП") Или (РегламентированнаяОтчетность.ИДКонфигурации() = "ЗУП") Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|ВзаиморасчетыПоВзносам.БазаВзноса              КАК База,
		|ВзаиморасчетыПоВзносам.ПериодВзаиморасчетов    КАК НалоговыйПериод,
		|";
	ИначеЕсли (РегламентированнаяОтчетность.ИДКонфигурации() = "БП") ИЛИ  (РегламентированнаяОтчетность.ИДКонфигурации() = "УТП") Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|ВзаиморасчетыПоВзносам.БазаВзноса          КАК База,
		|ВзаиморасчетыПоВзносам.База                КАК СуммаНачисления,
		|ВзаиморасчетыПоВзносам.ПериодВзаиморасчетов КАК НалоговыйПериод,
		|";
	КонецЕсли;	   
...
Процедура ВыгрузитьВАРМЗС ()
	...
	Значение = Организация.Наименование + Формат(День(мДатаКонцаПериодаОтчета),"ЧЦ=2;ЧГ=;") + Формат(Месяц(мДатаКонцаПериодаОтчета), "ЧЦ=2;ЧГ=;") + Формат(Год(мДатаКонцаПериодаОтчета),"ЧЦ=4;ЧГ=0;")+ "P0100111" + ".ZDI";
...
Процедура СоздатьТаблицуСоСхемамиВыгрузки() Экспорт
	...
	ТипКолИмяСхемы = СоздатьОписанияТипов("Строка",16);
	...
	НовСтрока.ИмяСхемы = "ПоказателиАРМЗС"; 
...
ЭлементыФормы.ПолеТабличногоДокументаОтчетПФ.Области.Мес_1.ЭлементУправления.СписокВыбора = СписокВыбораМесяца ;
ЭлементыФормы.ПолеТабличногоДокументаОтчетПФ.Области.Мес_2.ЭлементУправления.СписокВыбора = СписокВыбораМесяца ;
ЭлементыФормы.ПолеТабличногоДокументаОтчетПФ.Области.Мес_3.ЭлементУправления.СписокВыбора = СписокВыбораМесяца ;
ЭлементыФормы.ПолеТабличногоДокументаОтчетПФ.Области.Мес_4.ЭлементУправления.СписокВыбора = СписокВыбораМесяца ;
...
мСоставПоказателей = "СоставПоказателей2007Янв";
