		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИндексацияЗарплаты Тогда
			...
			Иначе	
				...
				Если КомментироватьРасчет Тогда
					...
					ОбщегоНазначения.КомментарийРасчета(НСтр("ru='Коэффициент индексации пропорционально: ';uk='Коефіцієнт індексації пропорційно: '") + Окр(КоэффициентИндексацииПропорционально, 4), КомментарийВидаРасчета);
...
// Серна - Начало
				|		СУММА(ВЫБОР
				|					КОГДА Начисления.ВидРасчета.ЯвляетсяДоходомВНатуральнойФорме
				|						ТОГДА &КоэффициентМатериальнойФормы * Начисления.Результат
				|					ИНАЧЕ Начисления.Результат
				|				КОНЕЦ) КАК Доход,
				|		СУММА(Начисления.Результат) КАК ДоходВС
// Серна - Конец
...
// Серна - Конец
			...
			Запрос.Текст = ТекстЗапросаВТ;
// Серна - Начало
			Запрос.УстановитьПараметр("КоэффициентМатериальнойФормы", 1.21951);
// Серна - Конец
			Запрос.Выполнить();
